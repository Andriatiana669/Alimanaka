...
class CongeAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'jours_deduits_badge', 'statut_badge', 'validation_info', 'date_creation'
    ]
    list_filter = ['statut', 'type_conge', 'date_debut', 'valide_par', 'refuse_par']
    search_fields = [
        'utilisateur__username', 'utilisateur__first_name', 
        'utilisateur__last_name', 'motif', 'commentaire_refus'
    ]
    readonly_fields = ['jours_deduits', 'date_creation', 'date_modification', 'date_validation', 'date_refus']
    ...
    fieldsets = (
        ('Informations générales', {
            'fields': ('utilisateur', 'type_conge', 'date_debut', 'date_fin', 'motif')
        }),
        ('Statut et calcul', {
            'fields': ('statut', 'jours_deduits')
        }),
        ('Validation', {
            'fields': ('valide_par', 'date_validation'),
            'classes': ('collapse',),
            'description': 'Informations de validation'
        }),
        ('Refus', {
            'fields': ('refuse_par', 'date_refus', 'commentaire_refus'),
            'classes': ('collapse',),
            'description': 'Informations de refus'
        }),
        ('Métadonnées', {
    ...
    @admin.display(description="Validation/Refus")
    def validation_info(self, obj):
        if obj.statut == 'approuve' and obj.valide_par:
            return format_html(
                '<span style="color: #4caf50;">✅ Par {}</span>',
                obj.valide_par.get_display_name()
            )
        elif obj.statut == 'refuse' and obj.refuse_par:
            return format_html(
                '<span style="color: #f44336;">❌ Par {}</span>',
                obj.refuse_par.get_display_name()
            )
        return '-'
    
    # Actions personnalisées
    @admin.action(description='Valider les congés sélectionnés')
    ...
    def refuser_conges(self, request, queryset):
        ...
        self.message_user(request, f'{queryset.filter(statut="en_attente").count()} congé(s) refusé(s).')
...
class DroitAdmin(admin.ModelAdmin):
    list_display = ['nom', 'est_actif', 'ordre', 'description_courte']
...
alimanaka_admin.register(Droit, DroitAdmin)