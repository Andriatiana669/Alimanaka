...
class PoleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filtre les pôles selon les permissions"""
        user = self.request.user
        
        # Super admin ou staff : voit tous les pôles actifs
        if user.is_superuser or user.is_staff:
            return Pole.objects.filter(est_actif=True)
        
        # Manager/Co-manager : voit uniquement les pôles de ses équipes et sous-équipes
        equipes_accessibles = user.sous_equipes_accessibles
        
        # Extraire les pôles uniques des équipes accessibles
        pole_ids = set()
        for equipe in equipes_accessibles:
            if equipe.pole_id:
                pole_ids.add(equipe.pole_id)
        
        # Si pas de pôles trouvés, retourner queryset vide mais valide
        if not pole_ids:
            return Pole.objects.none()
        
        return Pole.objects.filter(id__in=pole_ids, est_actif=True)
    ...
    def actifs(self, request):
        """Liste tous les pôles actifs selon les permissions"""
    ...
    def equipes(self, request, pk=None):
        """Liste les équipes d'un pôle accessibles à l'utilisateur"""
        pole = self.get_object()
        user = request.user
        
        # Toutes les équipes du pôle
        equipes_queryset = Equipe.objects.filter(pole=pole, est_actif=True)
        
        # Si super admin/staff : voit tout
        if user.is_superuser or user.is_staff:
            serializer = EquipeSerializer(equipes_queryset, many=True)
            return Response(serializer.data)
        
        # Sinon : filtrer selon les équipes accessibles
        equipes_accessibles_ids = [eq.id for eq in user.sous_equipes_accessibles]
        equipes_filtrees = equipes_queryset.filter(id__in=equipes_accessibles_ids)
        
        serializer = EquipeSerializer(equipes_filtrees, many=True)
        return Response(serializer.data)
...
class EquipeViewSet(viewsets.ModelViewSet):
    ...
    def arbre(self, request):
        ...
        equipes_accessibles = self.get_equipes_accessibles(user, inclure_inactives)

        if user.is_staff or user.is_superuser:
            # Admin → toutes les racines
            racines = equipes_accessibles.filter(equipe_parente__isnull=True)
        elif user.equipes_gerees.exists():
            # Manager → ses équipes gérées et leurs sous-équipes
            racines = equipes_accessibles.filter(
                equipe_parente__isnull=True
            ) | equipes_accessibles.filter(id__in=user.equipes_gerees.values_list('id', flat=True))
        elif user.equipe:
            # 🔑 Utilisateur normal → SON équipe comme racine
            racines = equipes_accessibles.filter(id=user.equipe.id)
        else:
            racines = Equipe.objects.none()
        ...
        serializer = EquipeTreeSerializer(
            ...
            context={
                'request': request,
                'inclure_inactives': inclure_inactives,
                'equipes_accessibles': equipes_accessibles
            }
        )
        return Response(serializer.data)
    ...
    def get_equipes_accessibles(self, user, inclure_inactives=False):
        ...
        if user.equipes_gerees.exists():
            ids = [eq.id for eq in user.sous_equipes_accessibles]
            return qs.filter(id__in=ids)

        if user.equipe:
            return qs.filter(id=user.equipe.id)
