...
class Permission(models.Model):
    ...
    def enregistrer_retour(self, heure_arrivee):
        ...
        if arrivee_dt > max_dt:
            ...
            # Total à rattraper = 2h (permission complète) + dépassement
            # Soit: (heure_max - heure_depart) + (arrivée - heure_max)
            diff_depassement = arrivee_dt - max_dt
            self.minutes_depassement = int(diff_depassement.total_seconds() / 60)
            
            # Calcul des minutes totales: 2h (120 min) + minutes_depassement
            minutes_totales = 120 + self.minutes_depassement
            self.heures_a_rattraper = Decimal(minutes_totales) / Decimal('60')
            self.heures_restantes = self.heures_a_rattraper
            
        else:
            # CAS 2: Retour AVANT l'heure max
            # Total à rattraper = durée réelle de l'absence
            # Soit: arrivée - départ
            diff_absence = arrivee_dt - datetime.combine(self.date, self.heure_depart)
            minutes_absence = int(diff_absence.total_seconds() / 60)
            
            # Vérifier si c'est moins que le forfait
            config = PermissionConfig.objects.first()
            minutes_forfait = config.rattrapage_forfaitaire_minutes if config else 30
            
            self.minutes_depassement = 0
            self.heures_a_rattraper = Decimal(max(minutes_absence, minutes_forfait)) / Decimal('60')
            self.heures_restantes = self.heures_a_rattraper
...
class PermissionConfig(models.Model):
    ...
    rattrapage_forfaitaire_minutes = models.PositiveIntegerField(
        default=30,
        verbose_name="Rattrapage forfaitaire (minutes)",
        help_text="Minutes de rattrapage obligatoire même en cas de retour dans les temps"
    )
    ...
    def __str__(self):
        return "Configuration des permissions"