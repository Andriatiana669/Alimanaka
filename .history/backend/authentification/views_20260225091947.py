...
def sso_login_view(request):
    ...
    # S'assurer que c'est un chemin relatif
...
def sso_callback_view(request):
    ...
    # Rediriger vers le frontend avec l'URL complète depuis settings.FRONTEND_URL
    redirect_path = request.session.pop('post_auth_redirect', '/dashboard')
    
    # Construire l'URL complète du frontend
    ...
    # Ajouter un paramètre pour indiquer le succès
    separator = '&' if '?' in redirect_url else '?'
    redirect_url = f"{redirect_url}{separator}auth=success"
    
    print(f"   → Redirect vers: {redirect_url}")
