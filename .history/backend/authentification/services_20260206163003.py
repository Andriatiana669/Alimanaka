...
class KeycloakService:
    ...
    def create_or_update_user(keycloak_user):
        ...
        # 1. Chercher par username d'abord (pour les superusers locaux)
        try:
        ...
        if user:
            # Mettre à jour sans toucher à is_staff/is_superuser
            if email:
                user.email = email
            ...
            if not email:
                email = f"{username}@localhost"  # Fallback si pas d'email
