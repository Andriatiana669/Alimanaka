...
class KeycloakService:
    ...
    def create_or_update_user(keycloak_user):
        ...
        try:
            ...
            # Mettre à jour SEULEMENT les champs SSO (pas is_staff/is_superuser)
            user.username = username
            user.first_name = keycloak_user.get('given_name', '')
            user.last_name = keycloak_user.get('family_name', '')
            # NE PAS toucher à is_staff et is_superuser
            user.save(update_fields=['username', 'first_name', 'last_name'])  # Spécifie les champs
            return user, False
        except User.DoesNotExist:
            # Créer un nouvel utilisateur (is_staff=False par défaut)
