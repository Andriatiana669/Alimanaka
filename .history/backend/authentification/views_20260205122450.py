...
def get_frontend_url(path='/dashboard'):
    """Retourne l'URL frontend selon l'environnement"""
    if settings.DEBUG:
        # En dev, rediriger vers le serveur Vite
        return f"http://localhost:5173{path}"
    # En prod, laisser Django servir la SPA
    return path

# Modifier home_view
def home_view(request):
    """Page d'accueil avec redirection auto"""
    if request.user.is_authenticated:
        return redirect(get_frontend_url('/dashboard'))
    
    state = str(uuid.uuid4())
    request.session['oauth_state'] = state
    request.session['next_url'] = get_frontend_url('/dashboard')  # ← URL complète ici
...
def sso_callback_view(request):
    ...
    if request.method == 'GET':
        ...
        if user:
            ...
            next_url = request.session.pop('next_url', get_frontend_url('/dashboard'))
