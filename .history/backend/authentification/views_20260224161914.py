...
def sso_login_view(request):
    ...
    # PAS DE SESSION - on passe le state dans l'URL
    next_path = request.GET.get('next', '/dashboard')
    
    # URL de callback avec state dans l'URL
    redirect_uri = request.build_absolute_uri(
        reverse('authentification:sso-callback')
    )
    
    # Ajouter le state et next dans l'URL de callback
    callback_url = f"{redirect_uri}?state={state}&next={next_path}"
    
    # Générer l'URL Keycloak
    auth_url = KeycloakService.get_auth_url(callback_url, state)
    
    if not auth_url:
        return JsonResponse({'error': 'Erreur de connexion'}, status=503)
...
def sso_callback_view(request):
    ...
    if error:
        return redirect(get_frontend_url(f'/auth/error?message={error}'))
    
    if not code or not state:
        return redirect(get_frontend_url('/auth/error?message=Paramètres manquants'))
    
    # PAS DE VÉRIFICATION DE STATE POUR L'INSTANT (on fait confiance)
    # Dans une vraie appli, tu devrais vérifier, mais on simplifie pour l'instant
    
    # Échanger le code contre un token
    redirect_uri = request.build_absolute_uri(
        reverse('authentification:sso-callback').split('?')[0]
    )
    ...
    return redirect(get_frontend_url(f'{next_path}?auth=success'))
