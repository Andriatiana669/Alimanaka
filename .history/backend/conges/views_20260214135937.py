...
class CongeViewSet(viewsets.ModelViewSet):
    ...
    def calendrier(self, request):
        ...
        for conge in conges_queryset.select_related('utilisateur'):
            ...
            # Si le congé dure plusieurs jours
            if conge.date_fin > conge.date_debut:
                # FullCalendar : end est EXCLUSIF
                # Pour afficher du 17 au 19 inclus, end doit être 20
                end_exclusive = conge.date_fin + timedelta(days=1)
                event['end'] = end_exclusive.isoformat()
                
                # DEBUG : Affiche dans la console ce qui est envoyé
                print(f"DEBUG: Conge {conge.id} du {conge.date_debut} au {conge.date_fin} -> end: {end_exclusive}")
