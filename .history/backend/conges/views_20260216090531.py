...
class CongeViewSet(viewsets.ModelViewSet):
    ...
    def _get_conge_color(self, type_conge, statut='en_attente'):
        ...
        if statut == 'approuve':
            return {'bg': '#4caf50', 'text': '#ffffff', 'border': '#388e3c'}  # Vert validation
        
        # Si refusé : rouge
        if statut == 'refuse':
            return {'bg': '#f44336', 'text': '#ffffff', 'border': '#d32f2f'}  # Rouge refus
        
        # Si annulé : gris
        if statut == 'annule':
            return {'bg': '#9e9e9e', 'text': '#ffffff', 'border': '#757575'}  # Gris
