...
from .serializers import (
    ...
    CongeSerializer, CongeCreateSerializer, DroitSerializer
)



class DroitViewSet(viewsets.ReadOnlyModelViewSet):
    """Liste des droits disponibles"""
    queryset = Droit.objects.filter(est_actif=True)
...
class CongeViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # Juste calculer les jours déduits pour info (sauf pour les droits)
        if not conge.est_droit:
            conge.calculer_deduction()
        else:
            conge.jours_deduits = Decimal('0.00')
        conge.save()
    ...
    def calendrier(self, request):
        ...
        for conge in conges_queryset.select_related('utilisateur').select_related('droit'):
            title = f"{conge.utilisateur.username.upper()} - {conge.utilisateur.get_display_name().upper()} - {'✔️' if conge.motif else ''} "
            
            # Ajouter indication si c'est un droit
            if conge.est_droit and conge.droit:
                title += f"- 🤝 - "
            
            title += f"{conge.get_type_conge_display()}"
            ...
            event = {
                ...
                'allDay': True,
                'color': self._get_conge_color(conge.type_conge, conge.statut, conge.est_droit),
                'type': 'conge',
                'user_id': conge.utilisateur.id,
                'equipe_id': conge.utilisateur.equipe_id if conge.utilisateur.equipe else None,
                'statut': conge.statut,
                'isDroit': conge.est_droit,  # ← NOUVEAU
                'droit_nom': conge.droit.nom if conge.est_droit and conge.droit else None  # ← NOUVEAU
            }
            ...
            for jf in jours_feries:
                try:
                    ...
                    events.append({
                        ...
                        'allDay': True,
                        'color': '#ffeb3b',  # Jaune de base
                        'backgroundColor': '#ffeb3b',  # Fond jaune
                        'display': 'background',  # ← Ceci crée un fond coloré
                        'classNames': ['holiday-striped'],  # Pour CSS personnalisé
                        'type': 'ferie',
                        'isBlocked': True,
                        'textColor': '#000000'  # Texte en noir pour lisibilité
                    })
                except ValueError:
                    continue
    ...
    def _get_conge_color(self, type_conge, statut='en_attente', est_droit=False):
        """Détermine la couleur d'un événement dans le calendrier"""
        
        # Couleur spéciale pour les droits
        if est_droit:
    ...
    def valider(self, request, pk=None):
        ...
        # Pour les droits, pas de vérification de solde
        if not conge.est_droit:
        ...
        if dates_specifiques:
            ...
            # Vérifier le solde (sauf pour les droits)
            if not conge.est_droit and conge.utilisateur.solde_conge_actuelle < jours_a_valider:
                return Response({'error': 'Solde insuffisant'}, status=400)
            
            # DÉDUIRE le solde (sauf pour les droits)
            if not conge.est_droit:
            ...
            for periode in nouvelles_periodes:
                if periode['valide']:
                    ...
                    if not nouveau_conge.est_droit:
                        nouveau_conge.calculer_deduction()
                    else:
                        nouveau_conge.jours_deduits = Decimal('0.00')
                    nouveau_conge.save()
            ...
            while current <= conge.date_fin:
                if current.weekday() < 5:
                    if not JourFerie.objects.filter(mois=current.month, jour=current.day, est_actif=True).exists():
                        if not JourExceptionnel.objects.filter(date=current, type_jour='exceptionnel').exists():
                            jours_valides.append(current.strftime('%Y-%m-%d'))
                current += timedelta(days=1)
        
            conge.jours_valides = jours_valides
    ...
    def refuser(self, request, pk=None):
        ...
        # Pour un refus, le solde n'a PAS été déduit (car en attente)
        if conge.jours_deduits > 0 and not conge.est_droit:
    ...
    def annuler(self, request, pk=None):
        ...
        # REMBOURSEMENT CONDITIONNEL (sauf pour les droits)
        remboursement_effectue = False
        if conge.statut == 'approuve' and conge.jours_deduits > 0 and not conge.est_droit:
    ...
    def annuler_partiel(self, request, pk=None):
        ...
        if conge.est_droit:
            return Response({'error': 'Les droits ne peuvent pas être annulés partiellement'}, status=400)
        ...
        for periode in nouvelles_periodes:
            if periode['valide']:
                jours_valides_periode = []
                current_day = periode['debut']
                while current_day <= periode['fin']:
                    if current_day.weekday() < 5:
                        jours_valides_periode.append(current_day.strftime('%Y-%m-%d'))
                ...
                nouveau_conge = Conge.objects.create(
                    ...
                    jours_valides=jours_valides_periode,
                    conge_original=conge
                )
            else:
                nouveau_conge = Conge.objects.create(
    ...
    def export(self, request):
        ...
        headers = ['Utilisateur', 'Type', 'Droit?', 'Date début', 'Date fin', 'Jours déduits', 
        ...
        conges = Conge.objects.all().select_related('utilisateur').select_related('droit').order_by('-date_debut')
        
        for conge in conges:
            type_display = f"{conge.get_type_conge_display()}"
            if conge.est_droit and conge.droit:
                type_display = f"⚖️ {conge.droit.nom}"
            
            ws.append([
                conge.utilisateur.get_display_name(),
                type_display,
                "Oui" if conge.est_droit else "Non",
    ...
    def export_mine(self, request):
        ...
        headers = ['Type', 'Droit?', 'Date début', 'Date fin', 'Jours déduits', 'Statut', 'Motif']
        ws.append(headers)
        ...
        conges = Conge.objects.filter(utilisateur=user).select_related('droit').order_by('-date_debut')
