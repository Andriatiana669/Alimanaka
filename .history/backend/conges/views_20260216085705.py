...
class CongeViewSet(viewsets.ModelViewSet):
    ...
    def calendrier(self, request):
        ...
        for conge in conges_queryset.select_related('utilisateur'):
            ...
            # backend/conges/views.py - Dans la méthode calendrier, modifie l'event :
            ...
            event = {
                ...
                'allDay': True,
                'color': self._get_conge_color(conge.type_conge, conge.statut),  # ← PASSER LE STATUT
                'type': 'conge',
                'user_id': conge.utilisateur.id,
                'equipe_id': conge.utilisateur.equipe_id if conge.utilisateur.equipe else None,
                'statut': conge.statut  # ← AJOUTER LE STATUT pour le frontend
            }
