...
class CongeViewSet(viewsets.ModelViewSet):
    ...
    def calendrier(self, request):
        ...
        for conge in conges_queryset.select_related('utilisateur'):
            # Récupérer la config du type pour avoir les heures
            try:
            ...
            # Construire les datetime avec heures pour la timeline
            from datetime import datetime
            start_datetime = datetime.combine(conge.date_debut, heure_debut)
            end_datetime = datetime.combine(conge.date_fin, heure_fin)
            ...
            event = {
                ...
                'allDay': False,  # False pour avoir les heures dans la timeline
                ...
                # Infos supplémentaires pour le modal détails
                'heure_debut': heure_debut.strftime('%H:%M'),
                'heure_fin': heure_fin.strftime('%H:%M'),
                'jours_deduits': float(conge.jours_deduits),
                'statut': conge.statut,
                'motif': conge.motif,
                'date_creation': conge.date_creation.isoformat() if conge.date_creation else None,
                'can_cancel': conge.utilisateur == user or user.is_superuser  # Seul le propriétaire peut annuler
            }
            
            if conge.date_fin > conge.date_debut:
                day_after = conge.date_fin + timedelta(days=1)
                end_datetime = datetime.combine(day_after, heure_fin)
                event['end'] = end_datetime.isoformat()
            else:
                end_datetime = datetime.combine(conge.date_debut, heure_fin)
                event['end'] = end_datetime.isoformat()
