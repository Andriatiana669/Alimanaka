...
class CongeViewSet(viewsets.ModelViewSet):
    ...
    def calendrier(self, request):
        ...
        # AJOUTER CETTE CONDITION : Si date_fin > date_debut, ajouter 'end'
    ...
    def utilisateurs_gerables(self, request):
        ...
        from users.models import User  # ← IMPORT EXPLICITE ICI
        ...
        if user.is_superuser:
            users = User.objects.filter(is_active=True)  # ← CORRIGÉ: User.objects (majuscule)
        elif user.is_staff and est_manager:
            equipes = user.sous_equipes_accessibles
            users = User.objects.filter(  # ← CORRIGÉ: User.objects (majuscule)
            ...
            users = User.objects.filter(id=user.id)  # ← CORRIGÉ: User.objects (majuscule)
