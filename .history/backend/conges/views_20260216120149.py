...
class CongeViewSet(viewsets.ModelViewSet):
    ...
    def valider(self, request, pk=None):
        ...
        # Nouveau : validation partielle ?
        dates_specifiques = request.data.get('dates', [])  # Liste de dates ISO
        ...
        if dates_specifiques:
            # Calculer le nombre de jours à valider
            from datetime import datetime, timedelta
            jours_a_valider = 0
            current = conge.date_debut
            while current <= conge.date_fin:
                if current.strftime('%Y-%m-%d') in dates_specifiques:
            ...
            if conge.utilisateur.solde_conge_actuelle < jours_a_valider:
                return Response({
                    'error': f'Solde insuffisant pour ces jours. Disponible: {conge.utilisateur.solde_conge_actuelle}'
                }, status=400)
            ...
            # Marquer ces jours comme validés (on pourrait créer un champ JSON)
            if not hasattr(conge, 'jours_valides') or not conge.jours_valides:
                conge.jours_valides = []
            conge.jours_valides.extend(dates_specifiques)
            conge.save()
            
            # Si tous les jours sont validés, marquer le congé comme approuvé
            total_jours = (conge.date_fin - conge.date_debut).days + 1
            if len(conge.jours_valides) >= total_jours:
            ...
            return Response({
                'success': True,
                'message': f'{len(dates_specifiques)} jour(s) validé(s)',
                'jours_valides': conge.jours_valides,
                'statut': conge.statut
            })
        
        # Sinon, validation complète (code existant)
        ...
        conge.valide_par = user
        conge.date_validation = timezone.now()
        conge.jours_valides = []  # Tous les jours sont validés implicitement
        conge.save()
