...
class CongeViewSet(viewsets.ModelViewSet):
    ...
    def valider(self, request, pk=None):
        ...
        # Récupérer la configuration du type de congé
        config = TypeCongeConfig.objects.get(type_conge=conge.type_conge)
        
        # Récupérer les dates spécifiques pour validation partielle
        ...
        if dates_specifiques:
            ...
            while current <= conge.date_fin:
                ...
                if date_str in dates_specifiques:
                    # Vérifier si ce jour n'est pas déjà validé
                    if conge.jours_valides and date_str in conge.jours_valides:
                        current += timedelta(days=1)
                        continue
                    
                    # Calculer la déduction pour ce jour spécifique
                    weekday = current.weekday()  # 0=Lundi, 6=Dimanche
                    
                    # Vérifier si c'est un jour ouvrable (pas weekend, pas férié, pas exceptionnel)
                    if weekday >= 5:  # Weekend
                        current += timedelta(days=1)
                        continue
                    
                    if JourFerie.objects.filter(mois=current.month, jour=current.day, est_actif=True).exists():
                        current += timedelta(days=1)
                        continue
                    
                    if JourExceptionnel.objects.filter(date=current, type_jour='exceptionnel').exists():
                        current += timedelta(days=1)
                        continue
                    
                    # Calculer la déduction selon les règles
                    deduction = config.deduction_jours
                    
                    # Règle Vendredi
                    if weekday == 4 and config.vendredi_deduction:  # Vendredi
                        deduction = config.vendredi_deduction
                    
                    # Règle Jeudi (si vendredi suivant est rayé)
                    elif weekday == 3 and config.jeudi_deduction:  # Jeudi
                        vendredi = current + timedelta(days=1)
            ...
            # Vérifier qu'il y a des jours à valider
            if jours_a_valider == 0:
                return Response({
                    'error': 'Aucun jour valide à valider (peut-être déjà validés ou non ouvrables)'
                }, status=400)
            
            # Vérifier le solde
            if conge.utilisateur.solde_conge_actuelle < jours_a_valider:
                return Response({
                    'error': f'Solde insuffisant pour ces jours. Disponible: {conge.utilisateur.solde_conge_actuelle}, Requis: {jours_a_valider}'
                }, status=400)
            
            # DÉDUIRE le solde pour ces jours
            conge.utilisateur.consommer_conge(jours_a_valider)
            conge.utilisateur.save()
            
            # Marquer ces jours comme validés
            if not conge.jours_valides:
                conge.jours_valides = []
            
            # Éviter les doublons
            for date in dates_specifiques:
                if date not in conge.jours_valides:
                    conge.jours_valides.append(date)
                ...
                if current.weekday() < 5:  # Pas weekend
                    if not JourFerie.objects.filter(mois=current.month, jour=current.day, est_actif=True).exists():
                        if not JourExceptionnel.objects.filter(date=current, type_jour='exceptionnel').exists():
                            total_jours_ouvrables += 1
                current += timedelta(days=1)
            
            # Si tous les jours sont validés, marquer le congé comme approuvé
            if len(conge.jours_valides) >= total_jours_ouvrables:
            ...
            return Response({
                'success': True,
                'message': f'{len(dates_specifiques)} jour(s) validé(s) sur {total_jours_ouvrables}',
                'jours_valides': conge.jours_valides,
                'statut': conge.statut,
                'valide_par': user.get_display_name(),
                'date_validation': conge.date_validation if conge.statut == 'approuve' else None,
                'jours_deduits': str(jours_a_valider),
                'nouveau_solde': str(conge.utilisateur.solde_conge_actuelle)
            })
        ...
        # ========== VALIDATION COMPLÈTE ==========
            ...
            if conge.utilisateur.solde_conge_actuelle < conge.jours_deduits:
                return Response({
                    'error': f'Solde insuffisant. Disponible: {conge.utilisateur.solde_conge_actuelle}, '
                            f'Requis: {conge.jours_deduits}'
                }, status=400)
            
            # DÉDUIRE le solde maintenant (à la validation)
            ...
            # Marquer tous les jours comme validés
            conge.jours_valides = []
            current = conge.date_debut
            while current <= conge.date_fin:
                if current.weekday() < 5:  # Pas weekend
                    if not JourFerie.objects.filter(mois=current.month, jour=current.day, est_actif=True).exists():
                        if not JourExceptionnel.objects.filter(date=current, type_jour='exceptionnel').exists():
                            conge.jours_valides.append(current.strftime('%Y-%m-%d'))
                current += timedelta(days=1)
            ...
            return Response({
                'success': True,
                'message': 'Congé validé et solde déduit',
                'valide_par': user.get_display_name(),
                'date_validation': conge.date_validation,
                'jours_deduits': str(conge.jours_deduits),
                'nouveau_solde': str(conge.utilisateur.solde_conge_actuelle),
                'jours_valides': conge.jours_valides
            })
