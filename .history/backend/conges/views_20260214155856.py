    ...
    def calendrier(self, request):
        ...
        for conge in conges_queryset.select_related('utilisateur'):
            ...
            try:
                ...
                heure_fin = time(17, 00)
                
            # Construire les datetime avec heures pour la timeline
            from datetime import datetime
            start_datetime = datetime.combine(conge.date_debut)
            end_datetime = datetime.combine(conge.date_fin)
            
            
            event = {
                'id': f"conge_{conge.id}",
                'title': f"{conge.utilisateur.get_display_name().upper()} - {conge.get_type_conge_display()} - {'🟢' if conge.motif else '❌'}",
