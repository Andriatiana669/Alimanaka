...
class CongeViewSet(viewsets.ModelViewSet):
    ...
    def _get_conge_colors(self, type_conge, statut='en_attente'):
        """Retourne les couleurs (fond et texte) selon le type et statut"""
        
        if statut == 'approuve':
            return {'bg': '#4caf50', 'text': '#ffffff', 'border': '#388e3c'}
        
        if statut == 'refuse':
            return {'bg': '#f44336', 'text': '#ffffff', 'border': '#d32f2f'}
        
        if statut == 'annule':
            return {'bg': '#9e9e9e', 'text': '#ffffff', 'border': '#757575'}
        
        # En attente - couleurs selon type
        colors = {
            'matin': {'bg': '#c8e6c9', 'text': '#1b5e20', 'border': '#81c784'},
            'midi': {'bg': '#ffe0b2', 'text': '#e65100', 'border': '#ffb74d'},
            'journee': {'bg': '#bbdefb', 'text': '#0d47a1', 'border': '#64b5f6'}
        }
        return colors.get(type_conge, {'bg': '#e3f2fd', 'text': '#1565c0', 'border': '#90caf9'})
