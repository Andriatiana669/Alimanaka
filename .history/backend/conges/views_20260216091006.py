    ...
    def calendrier(self, request):
        ...
        for conge in conges_queryset.select_related('utilisateur'):
            ...
            event = {
                ...
                'title': f"{conge.utilisateur.username.upper()} - {conge.utilisateur.get_display_name().upper()} - {conge.get_type_conge_display()}",
                ...
                'extendedProps': {  # ← AJOUTÉ pour FullCalendar si besoin
                    'statut': conge.statut,
                    'motif': conge.motif
                }
            }
