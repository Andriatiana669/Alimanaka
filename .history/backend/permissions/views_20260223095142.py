...
class PermissionViewSet(viewsets.ModelViewSet):
    ...
    @action(description='Marquer comme approuvé')
    def marquer_approuve(self, request, queryset):
        from django.utils import timezone
        
        # Récupérer les IDs avant modification
        permission_ids = list(queryset.filter(statut__in=['retourne', 'rattrapage']).values_list('id', flat=True))
        
        # Mettre à jour
        queryset.filter(statut__in=['retourne', 'rattrapage']).update(
            statut='approuve',
        ...
        # Recharger avec les relations pour l'affichage admin
        updated_permissions = Permission.objects.filter(
            id__in=permission_ids
        ).select_related('valide_par')
        
        self.message_user(request, f'{len(permission_ids)} permission(s) approuvée(s).')
