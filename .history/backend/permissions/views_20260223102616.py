...
class PermissionViewSet(viewsets.ModelViewSet):
    ...
    def approuver(self, request, pk=None):
        """Approuver une permission (par un manager/admin)"""
        permission = self.get_object()
        user = request.user
        
        # Vérifier les permissions
        if not self._can_manage_user(user, permission.utilisateur.id):
            return Response(
                {'error': 'Vous n\'avez pas la permission d\'approuver cette permission'},
        ...
        if permission.statut not in ['retourne', 'rattrapage']:
            return Response(
                {'error': 'Seules les permissions en retour ou rattrapage peuvent être approuvées'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        permission.valide_par = user
        permission.date_validation = timezone.now()
        ...
        return Response({
            ...
            'date_validation': permission.date_validation
        })
