...
class Permission(models.Model):
    ...
    def enregistrer_retour(self, heure_arrivee):
        ...
        if arrivee_dt > max_dt:
            ...
            # Retour dans les temps : imposer un rattrapage forfaitaire
            ...
            self.heures_a_rattraper = Decimal(minutes_forfait) / Decimal('60')
            self.heures_restantes = self.heures_a_rattraper
    ...
    def ajouter_rattrapage(self, date_rattrapage, heure_debut, heure_fin, commentaire="", user=None):
        ...
        from django.utils import timezone  # ← Ajoute ceci
        ...
        if self.heures_restantes <= 0:
            ...
            # ← AJOUTE CECI : Enregistrer qui a approuvé (le user qui a fait le dernier rattrapage)
            if user:
                self.valide_par = user
                self.date_validation = timezone.now()  # ← Utilise timezone.now() pour Django
        else:
