...
class RetardViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_staff or user.is_superuser:
            return Retard.objects.all().select_related('utilisateur').prefetch_related('rattrapages')
        ...
        if hasattr(user, 'equipes_gerees') and user.equipes_gerees.exists():
            ...
            ).select_related('utilisateur').prefetch_related('rattrapages')
        
        # Utilisateur normal voit ses retards
        return Retard.objects.filter(utilisateur=user).select_related('utilisateur').prefetch_related('rattrapages')
    ...
    def calendrier(self, request):
        """Retourne les événements pour le calendrier (retards + weekends + fériés)"""
        from datetime import timedelta
        from conges.models import JourFerie  # ← Import du modèle congés
        ...
        # 1. Récupérer les retards selon les permissions
        retard_queryset = self.get_queryset().filter(date__year=annee)
        ...
        # 2. Ajouter les weekends (tout le monde les voit)
        current = datetime(annee, 1, 1).date()
        while current.year == annee:
            if current.weekday() >= 5:  # Samedi=5, Dimanche=6
                events.append({
        ...
        # 3. Ajouter les jours fériés (tout le monde les voit)
...
class RattrapageViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_staff or user.is_superuser:
            return Rattrapage.objects.all()
        
        # Ne voir que ses propres rattrapages
        return Rattrapage.objects.filter(retard__utilisateur=user)