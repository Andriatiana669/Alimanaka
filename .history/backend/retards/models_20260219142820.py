...
class TypeRetardConfig(models.Model):
    ...
    nom = models.CharField(
        ...
        verbose_name="Nom du type de retard",
        help_text="Ex: Retard léger, Retard matinal, etc."
    )
    
    # SUPPRIMÉ : type_retard avec choices - on utilise seulement le nom
...
class Retard(models.Model):
    ...
    # Configuration utilisée (au lieu du type_retard_config)
    type_retard = models.ForeignKey(
        TypeRetardConfig,
    ...
    # Heures (on garde une copie pour l'historique)
    heure_debut_prevue = models.TimeField(
        verbose_name="Heure de début prévue"
    )
    ...
    # SUPPRIMÉ : type_retard (charfield) - on utilise la FK
    ...
    def calculer_retard(self):
        ...
        if self.type_retard:
            ...
            self.heures_a_rattraper = (heures_brutes * self.type_retard.multiplicateur).quantize(
    ...
    def to_calendar_event(self):
        ...
        if self.statut == 'approuve':
            ...
            color = self.type_retard.couleur if self.type_retard else '#ff9800'  # Couleur du type ou orange
        else:
            color = self.type_retard.couleur if self.type_retard else '#ff9800'
        
        return {
            'id': f"retard_{self.id}",
            'title': f"⏰ {self.utilisateur.username.upper()} - {self.utilisateur.username.upper()} - {self.minutes_retard}min ({self.heures_restantes}h restantes)",
            'start': self.date.isoformat(),
            'allDay': True,
            'color': color,  # ← Couleur dynamique selon statut
            'type': 'retard',
            'user_id': self.utilisateur.id,
            'statut': self.statut,  # ← Important pour le frontend
