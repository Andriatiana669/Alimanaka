...
class Equipe(models.Model):
    ...
    # NOUVEAU : Co-managers (ManyToMany)
    co_managers = models.ManyToManyField(
        'User',
        blank=True,
        related_name='equipes_co_gerees',
    ...
    def all_managers(self):
        """Retourne tous les managers (principal + co-managers)"""
        managers = []
        if self.manager:
            managers.append(self.manager)
        managers.extend(self.co_managers.all())
        return managers
...
class User(AbstractUser):
    ...
    # CONGES
    solde_conge_recue_par_mois = models.DecimalField(
        max_digits=4,
        decimal_places=2,
        default=2.5,
        verbose_name="Solde congé reçu par mois",
        help_text="Nombre de jours de congé ajoutés chaque mois (défaut: 2.5)"
    )
    
    solde_conge_actuelle = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=0.0,
        verbose_name="Solde congé actuel",
        help_text="Solde disponible actuellement"
    )
    
    solde_conge_consomme = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=0.0,
        verbose_name="Solde congé consommé",
        help_text="Total des congés pris"
    )
    
    motif_conge = models.TextField(
        blank=True,
        null=True,
        verbose_name="Motif du dernier congé",
        help_text="Raison de la demande de congé"
    )
    ...
    def ajouter_solde_mensuel(self):
        """Ajoute le solde mensuel au solde actuel"""
        self.solde_conge_actuelle += self.solde_conge_recue_par_mois
        self.save()
        
    def consommer_conge(self, nombre_jours: float):
        """Consomme des jours de congé"""
        if self.solde_conge_actuelle >= nombre_jours:
            self.solde_conge_actuelle -= nombre_jours
            self.solde_conge_consomme += nombre_jours
            self.save()
            return True
        return False
...
# backend/users/models.py - Ajoute à la fin

class CronJobLog(models.Model):
    """Log des exécutions de jobs planifiés"""
    ...
    def __str__(self):
        return f"{self.job_name} - {self.last_run}"
