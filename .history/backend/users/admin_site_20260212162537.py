...
class PoleAdmin(admin.ModelAdmin):
    ...
    list_filter = ['nom', EstActifFilter]
...
class EquipeAdmin(admin.ModelAdmin):
    ...
    list_filter = ['nom', 'pole', EstActifFilter, 'date_creation']
...
class CustomUserAdmin(admin.ModelAdmin):
    list_display = [
        'username_badge', 'display_name', 'role_badge', 'solde_conge_badge',
    ...
    readonly_fields = [
        ...
        'equipes_co_gerees_list', 'role_display', 'solde_conge_actuelle', 'solde_conge_consomme',
    ]
    ...
    fieldsets = (
        ...
        #Solde congé
        ('Solde de Congés', {
            'fields': ('solde_conge_recue_par_mois', 'solde_conge_actuelle', 'solde_conge_consomme', 'motif_conge'),
            'classes': ('collapse',),
            'description': format_html(
                '<div style="background: #f8f9fa; padding: 10px; border-radius: 5px; '
                'margin-bottom: 10px;">'
                '<strong>🏖️ Gestion des congés :</strong> '
                'Solde mensuel = {}, Actuel = {}, Consommé = {}'
                '</div>',
                '2.5 jours/mois par défaut', 'Calculé automatiquement', 'Cumul des prises'
            )
        }),
    )
    ...
    def role_badge(self, obj):
        if obj.is_superuser:
            return format_html(
                ...
                'border-radius: 12px; font-weight: bold;">👑 Super Admin</span>'
            )
        elif obj.is_staff:
            return format_html(
                '<span style="background: #3498db; color: white; padding: 4px 10px; '
                'border-radius: 12px; font-weight: bold;">⚙️ Admin / Staff</span>'
            )
        elif obj.equipes_gerees.exists() or obj.equipes_co_gerees.exists():
            return format_html(
                '<span style="background: #e67e22; color: white; padding: 4px 10px; '
                'border-radius: 12px; font-weight: bold;">👨‍💼 Chef équipe / CM</span>'
            )
        else:
            return format_html(
                '<span style="background: #2ecc71; color: white; padding: 4px 10px; '
                'border-radius: 12px; font-weight: bold;">👤 Utilisateur</span>'
            )
    ...
    def pole_equipe_badge(self, obj):
        ...
        if obj.pole:
            info.append(f"{obj.pole.nom}")
        
        if obj.equipe:
            # Rôle dans l'équipe
            if obj.equipes_gerees.filter(id=obj.equipe.id).exists():
                info.append(f"👑 {obj.equipe.nom[:20]}")
            elif obj.equipes_co_gerees.filter(id=obj.equipe.id).exists():
                info.append(f"🛡️ {obj.equipe.nom[:20]}")
            else:
                info.append(f"👤 {obj.equipe.nom[:20]}")
        
        if not info:
            return format_html('<span style="color: #95a5a6;">🚫 Non assigné</span>')
    ...
    @admin.display(description="Solde Congés")
    def solde_conge_badge(self, obj):
        return format_html(
            '<div style="display: flex; flex-direction: column; gap: 2px;">'
            '<span style="background: #2ecc71; color: white; padding: 2px 6px; '
            'border-radius: 10px; font-size: 0.8em;">🏖️ {} dispo</span>'
            '<span style="background: #e74c3c; color: white; padding: 2px 6px; '
            'border-radius: 10px; font-size: 0.75em;">⛔ {} pris</span>'
            '</div>',
            obj.solde_conge_actuelle, obj.solde_conge_consomme
        )
