...
class EquipeViewSet(viewsets.ModelViewSet):
    ...
    def arbre(self, request):
        """Retourne l'arborescence des équipes selon le rôle de l'utilisateur"""
        user = request.user

        # Inclure ou non les équipes inactives
        ...
        # Obtenir toutes les équipes que l'utilisateur peut voir
        equipes_accessibles = self.get_queryset()  # ton get_queryset() gère les filtres par rôle et activité

        # Déterminer les racines à renvoyer selon le rôle
        if user.is_staff or user.is_superuser:
            # Admin → toutes les racines
            racines = equipes_accessibles.filter(equipe_parente__isnull=True)

        elif user.equipes_gerees.exists():
            # Manager → ses équipes gérées + sous-équipes (récursivement via serializer)
            racines = equipes_accessibles.filter(
                Q(id__in=user.equipes_gerees.values_list('id', flat=True)) |
                Q(equipe_parente__isnull=True)
            )

        elif user.equipe:
            # Utilisateur normal → sa propre équipe comme racine
