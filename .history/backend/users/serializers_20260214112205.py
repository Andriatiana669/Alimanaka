...
class EquipeTreeSerializer(serializers.ModelSerializer):
    ...
    def get_sous_equipes(self, obj):
        ...
        equipes_accessibles = self.context.get('equipes_accessibles')

        if equipes_accessibles is None:
            return []

        sous_equipes = equipes_accessibles.filter(equipe_parente=obj)

        if not inclure_inactives:
            sous_equipes = sous_equipes.filter(est_actif=True)

        return EquipeTreeSerializer(
            sous_equipes,
            many=True,
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = [
            'id', 'username', 'first_name', 'last_name', 'pseudo', 
            'pseudo_format', 'pseudo_format_display', 'email',
            'keycloak_id', 'is_active', 'is_staff', 'is_superuser',
            'date_joined', 'last_login', 'last_updated', 'full_name', 'display_name',
            
            'pole', 'pole_details', 'equipe', 'equipe_details', 'est_chef_equipe',
            
            'solde_conge_recue_par_mois', 'solde_conge_actuelle', 'solde_conge_consomme', 'motif_conge',
        ]
        read_only_fields = [
            'id', 'username', 'first_name', 'last_name', 'email', 'solde_conge_actuelle', 'solde_conge_consomme',
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'pole', 'pole_details', 'equipe', 'equipe_details', 'equipes_managerisees',
        ...
        read_only_fields = [
            ...
            'solde_conge_actuelle', 'solde_conge_consomme', 'motif_conge'
        ]
...
class UserListSerializer(serializers.ModelSerializer):
    ...
    solde_conge_actuelle = serializers.DecimalField(max_digits=5, decimal_places=2, read_only=True)
    
    class Meta:
        model = User
        fields = [
            'id', 'display_name', 'pseudo', 
            'username', 'first_name', 'last_name', 'pole_nom',
            'equipe_nom', 'solde_conge_actuelle', 'is_staff', 'date_joined'
        ]
