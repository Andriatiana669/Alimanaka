...
class PoleAdmin(admin.ModelAdmin):
    ...
    def actions(self, obj):
        view_url = reverse('admin:users_pole_changelist') + f'?pole__id__exact={obj.id}'
        return format_html(
...
class SousEquipeInline(admin.TabularInline):
    ...
    def nom_link(self, obj):
        url = reverse('admin:users_equipe_change', args=[obj.id])
...
class EquipeAdmin(admin.ModelAdmin):
    ...
    def pole_link(self, obj):
        if obj.pole:
            url = reverse('admin:users_pole_change', args=[obj.pole.id])
            return format_html(
    ...
    def co_managers_badge(self, obj):
        ...
        return format_html(
            ...
            "".join([
                format_html(
                    '<span style="background: #f39c12; color: white; padding: 1px 6px; '
                    'border-radius: 10px; font-size: 11px;" title="{}">🛡️ {}</span>',
                    cm.get_display_name(), cm.username[:3].upper()
    ...
    def sous_equipes_badge(self, obj):
        ...
        if count > 0:
            url = reverse('admin:users_equipe_changelist') + f'?equipe_parente__id__exact={obj.id}'
            return format_html(
    ...
    @admin.display(description="Actions")
    def actions(self, obj):
        view_url = reverse('admin:users_user_changelist') + f'?equipe__id__exact={obj.id}'
        return format_html(
...
class CustomUserAdmin(admin.ModelAdmin):
    # ✅ CORRECTION: Définir actions comme une liste
    actions = ['activate_users', 'deactivate_users', 'make_staff', 'remove_staff']
    
    list_display = [
        'username_badge', 'display_name', 'role_badge',
        'pole_equipe_badge', 'last_login_badge', 'is_active_badge', 'admin_actions_column'
    ]
    ...
    def make_staff(self, request, queryset):
        ...
        self.message_user(request, f"⚙️ {updated} utilisateur(s) promu(s) admin.")
    
    def remove_staff(self, request, queryset):
        updated = queryset.update(is_staff=False)
        self.message_user(request, f"👤 {updated} utilisateur(s) retiré(s) des admins.")
    ...
    make_staff.short_description = "Donner les droits admin"
    remove_staff.short_description = "Retirer les droits admin"
    ...
    def role_badge(self, obj):
        if obj.is_superuser:
            return format_html(
                '<span style="background: linear-gradient(45deg, #e74c3c, #c0392b); '
                'color: white; padding: 4px 10px; border-radius: 12px; font-weight: bold;">'
                '👑 Super Admin</span>'
            )
        elif obj.is_staff:
            return format_html(
                '<span style="background: linear-gradient(45deg, #3498db, #2980b9); '
                'color: white; padding: 4px 10px; border-radius: 12px; font-weight: bold;">'
                '⚙️ Admin</span>'
            )
        elif obj.equipes_gerees.exists() or obj.equipes_co_gerees.exists():
            return format_html(
                '<span style="background: linear-gradient(45deg, #e67e22, #d35400); '
                'color: white; padding: 4px 10px; border-radius: 12px; font-weight: bold;">'
                '👨‍💼 Chef d\'équipe</span>'
            )
        else:
            return format_html(
                '<span style="background: linear-gradient(45deg, #2ecc71, #27ae60); '
                'color: white; padding: 4px 10px; border-radius: 12px; font-weight: bold;">'
                '👤 Utilisateur</span>'
            )
    ...
    @admin.display(description="Actions")  # Renommé pour éviter conflit
    def admin_actions_column(self, obj):
        equipe_url = reverse('admin:users_equipe_changelist') + f'?manager__id__exact={obj.id}'
        return format_html(
    ...
    def equipes_gerees_list(self, obj):
        ...
        for equipe in equipes:
            url = reverse('admin:users_equipe_change', args=[equipe.id])
