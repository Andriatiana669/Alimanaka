...
export const congesApi = {
  ...
  annulerCongePartiel(id: number, dates: string[]): Promise<any> {
    return api.post(`/api/conges/conges/${id}/annuler_partiel/`, { dates }).then(res => res.data)
  },

  validerCongePartiel(id: number, dates: string[]): Promise<any> {
  ...
  validerConge(id: number): Promise<{ 
    success: boolean; 
    message: string;
    valide_par: string;
  ...
  refuserConge(id: number, commentaire?: string): Promise<{
    success: boolean;
    message: string;
    refuse_par: string;
    date_refus: string;
