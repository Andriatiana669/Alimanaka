...
export const polesApi = {
  ...
  getAll(): Promise<Pole[]> {
    return api.get('/api/poles/').then(res => res.data)
  },

  // Récupérer un pôle par ID
  getById(id: number): Promise<Pole> {
    return api.get(`/api/poles/${id}/`).then(res => res.data)
  },

  // Créer un pôle (admin)
  create(data: Omit<Pole, 'id' | 'date_creation'>): Promise<Pole> {
    return api.post('/api/poles/', data).then(res => res.data)
  },

  // Mettre à jour un pôle (admin)
  update(id: number, data: Partial<Pole>): Promise<Pole> {
    return api.patch(`/api/poles/${id}/`, data).then(res => res.data)
  },

  // Supprimer un pôle (admin)
  delete(id: number): Promise<void> {
    return api.delete(`/api/poles/${id}/`)
  },

  // Récupérer seulement les pôles actifs
  getActifs(): Promise<Pole[]> {
    return api.get('/api/poles/actifs/').then(res => res.data)
  }
}