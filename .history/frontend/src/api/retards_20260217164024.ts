...
export const retardsApi = {
  ...
  getTypesRetard(): Promise<TypeRetardConfig[]> {
    return api.get('/retards/types-retard/').then(res => res.data)
  },
  ...
  getMesRetards(annee?: number, statut?: string): Promise<Retard[]> {
    return api.get('/retards/retards/mes_retards/', {
      params: { annee, statut }
    }).then(res => res.data)
  },

  getRetardDetails(id: number): Promise<Retard> {
    return api.get(`/retards/retards/${id}/`).then(res => res.data)
  },

  createRetard(data: CreateRetardData & { user_id?: number }): Promise<Retard> {
    return api.post('/retards/retards/', data).then(res => res.data)
  },

  annulerRetard(id: number, commentaire?: string): Promise<AnnulationRetardResponse> {
    return api.post(`/retards/retards/${id}/annuler/`, { commentaire }).then(res => res.data)
  },
  ...
  getCalendrier(params?: CalendrierRetardsParams): Promise<RetardCalendarEventFromAPI[]> {
    ...
    return api.get('/retards/retards/calendrier/', {
  ...
  ajouterRattrapage(retardId: number, data: CreateRattrapageData): Promise<RattrapageResponse> {
    return api.post(`/retards/retards/${retardId}/ajouter_rattrapage/`, data).then(res => res.data)
  },

  getRattrapages(): Promise<Rattrapage[]> {
    return api.get('/retards/rattrapages/').then(res => res.data)
  },

  getRattrapageDetails(id: number): Promise<Rattrapage> {
    return api.get(`/retards/rattrapages/${id}/`).then(res => res.data)
  },
  ...
  getUtilisateursGerables(): Promise<GerableUserRetard[]> {
    return api.get('/retards/retards/utilisateurs_gerables/').then(res => res.data)
  }
}