...
// ============================================
// Fonction helper pour chercher le display name dans tous les stores
// ============================================
const getUserDisplayNameFromStore = (userId: number): string | undefined => {
  // Congés
    ...
    congesStore.calendrierEvents.forEach((e: any) => {
      if (e.user_id) {
        demandes.push({
          ...
          type: 'conge',
          user_id: e.user_id,
          user_display_name: e.user_display_name || getUserDisplayNameFromStore(e.user_id),
          statut: e.statut,
...
const getUserDisplayName = (demande: CalendarEvent): string => {
  ...
  if (userId) {
    // Créer un Map de tous les utilisateurs pour recherche rapide
    const allUsers = new Map()
    
    // Congés
    congesStore.utilisateursGerables?.forEach(u => allUsers.set(u.id, u))
    // Retards
    retardsStore.utilisateursGerables?.forEach(u => allUsers.set(u.id, u))
    // Permissions
    permissionsStore.utilisateursGerables?.forEach(u => allUsers.set(u.id, u))
    // Repos médicaux
    reposStore.utilisateursGerables?.forEach(u => allUsers.set(u.id, u))
    // OSTIES
    ostieStore.utilisateursGerables?.forEach(u => allUsers.set(u.id, u))
    ...
    // Si toujours pas trouvé, retourner l'ID
...
const getDemandeId = (demande: CalendarEvent): number | null => {
  // Essayer de récupérer depuis originalEvent d'abord
  if (demande.originalEvent?.id) {
    const id = demande.originalEvent.id
    if (typeof id === 'number') return id
    if (typeof id === 'string') {
      // Pour les IDs comme "conge_123"
      const matches = id.match(/\d+/)
      if (matches) return parseInt(matches[0])
      
      // Pour les IDs numériques en string
      const num = parseInt(id)
      if (!isNaN(num)) return num
    }
  }
  
  // Essayer depuis l'ID du demande
  if (demande.id) {
    const matches = demande.id.match(/\d+/)
    if (matches) return parseInt(matches[0])
    
    const num = parseInt(demande.id)
    if (!isNaN(num)) return num
  }
...
const openDemandeDetails = (demande: CalendarEvent) => {
  ...
  console.log('Ouverture des détails:', {
    ...
    user: getUserDisplayName(demande)
  })
  ...
  try {
    ...
    alert('Erreur lors de la navigation vers les détails')
  }
}
