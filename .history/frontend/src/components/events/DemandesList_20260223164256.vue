<template>
  <div class="demandes-container">
    ...
    <div v-else class="demandes-list">
      ...
      >
        ...
        <div class="demande-content">
          ...
          <!-- Actions rapides (uniquement pour les demandes modifiables) -->
    ...
    <!-- NOTE: Les modals seront ajoutés ultérieurement -->
  </div>
</template>
...
// ============================================
// Options de statut (tous les statuts possibles)
// ============================================
...
// Toutes les demandes (pas de filtre par statut)
const allDemandes = computed<CalendarEvent[]>(() => {
  const events = eventsStore.eventsForCalendar
  console.log('📊 eventsForCalendar:', events)
  
  const filtered = events.filter(e => 
    e.user_id && 
    ['conge', 'retard', 'permission', 'repos_medical', 'ostie'].includes(e.type as string)
  )
  console.log('📋 allDemandes filtrées:', filtered)
  
  return filtered as CalendarEvent[]
})

// Demandes filtrées par type, statut et année
const filteredDemandes = computed(() => {
  ...
  console.log('📦 allDemandes dans filtered:', demandes.length)
  ...
  console.log('📅 Après filtre année:', demandes.length)

  // Filtre par type
  if (selectedType.value) {
    demandes = demandes.filter(d => d.type === selectedType.value)
    console.log('🎯 Après filtre type:', demandes.length)
  }

  // Filtre par statut
  if (selectedStatus.value) {
    demandes = demandes.filter(d => d.statut === selectedStatus.value)
    console.log('🏷️ Après filtre statut:', demandes.length)
  }
...
const canManageThisDemande = (demande: CalendarEvent): boolean => {
  ...
  // Ne pas afficher les actions pour les demandes déjà finalisées
...
const openDemandeDetails = (demande: CalendarEvent) => {
  ...
  console.log('Ouverture des détails pour:', demande)
  alert(`Détails de la demande ${getTypeLabel(demande.type)} (${getStatusLabel(demande.statut, demande.type)}) - À implémenter`)
}
...
// Initialisation
onMounted(async () => {
  console.log('Montage de DemandesList')
  loading.value = true
  try {
    await eventsStore.fetchCalendrier({ annee: selectedYear.value })
    console.log('✅ Calendrier chargé, events:', eventsStore.calendrierEvents.length)
  } catch (error) {
