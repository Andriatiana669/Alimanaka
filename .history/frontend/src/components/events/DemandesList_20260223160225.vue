          ...
          <div class="demande-details">
            ...
            <template v-if="demande.type === 'conge'">
              <span class="detail-chip">
                ...
                {{ formatDateRange(demande.date_debut, demande.date_fin) }}
              </span>
              <span class="detail-chip">
                <i class="bi bi-hourglass-split"></i>
                {{ demande.jours_deduits || 0 }}j
              </span>
            </template>
            ...
            <template v-else-if="demande.type === 'retard'">
              <span class="detail-chip">
                ...
                {{ demande.minutes_retard || 0 }}min
              </span>
              <span class="detail-chip">
                <i class="bi bi-arrow-repeat"></i>
                {{ demande.heures_restantes || 0 }}h restantes
              </span>
            </template>
            ...
            <template v-else-if="demande.type === 'permission'">
              <span class="detail-chip">
                ...
                {{ formatTime(demande.heure_depart) }} → {{ formatTime(demande.heure_arrivee_max) }}
              </span>
            </template>
            ...
            <template v-else-if="demande.type === 'ostie'">
              <span class="detail-chip">
                <i class="bi bi-clock"></i>
                Début {{ formatTime(demande.heure_debut) }}
              </span>
            </template>

            <!-- Motif -->
            <span v-if="demande.motif" class="detail-chip motif">
              <i class="bi bi-chat-text"></i>
              {{ truncateMotif(demande.motif) }}
            </span>
          </div>
...
const demandesEnAttente = computed(() => {
  return eventsStore.eventsForCalendar.filter(e => 
    e.statut === 'en_attente' && 
    e.user_id && 
    ['conge', 'retard', 'permission', 'repos_medical', 'ostie'].includes(e.type)
  )
})
...
const canValidate = (demande: any): boolean => {
  if (!canManageOthers.value) return false
  return demande.statut === 'en_attente' && 
         ['conge', 'repos_medical'].includes(demande.type)
}

const canRefuse = (demande: any): boolean => {
  if (!canManageOthers.value) return false
  return demande.statut === 'en_attente' && 
         ['conge'].includes(demande.type)
}

const canTransform = (demande: any): boolean => {
  if (!canManageOthers.value) return false
  
  if (demande.type === 'permission') {
    return demande.statut === 'retourne' && demande.minutes_depassement > 0
  }
...
const canCancel = (demande: any): boolean => {
  if (!canManageOthers.value) return false
...
const openDemandeDetails = (demande: any) => {
...
const validateDemande = async (demande: any) => {
  if (!confirm(`Valider cette demande de ${getTypeLabel(demande.type)} ?`)) return
  
  try {
    const id = typeof demande.id === 'string' 
      ? parseInt(demande.id.replace(/[^0-9]/g, '')) 
      : demande.id
...
const transformDemande = (demande: any) => {
  // Ouvrir modal de transformation spécifique
  openDemandeDetails(demande)
  // Le modal gérera la transformation
}
