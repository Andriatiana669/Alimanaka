...
// Demandes en attente (statut = 'Approuvé')
...
const canValidate = (demande: CalendarEvent): boolean => {
  ...
  return demande.statut === 'Approuvé' && 
...
const canCancel = (demande: CalendarEvent): boolean => {
  if (!canManageOthers.value) return false
  return ['Approuvé', 'retourne', 'rattrapage'].includes(demande.statut || '')
}
...
const openDemandeDetails = (demande: CalendarEvent) => {
  ...
  // À implémenter : ouvrir le modal approprié
  alert(`Détails de la demande ${getTypeLabel(demande.type)} - À implémenter`)
}
...
const transformDemande = (demande: CalendarEvent) => {
  ...
  // openDemandeDetails(demande) // Le modal gérera la transformation
}
