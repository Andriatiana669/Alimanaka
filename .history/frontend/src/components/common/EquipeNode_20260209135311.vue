<template>
  <div class="equipe-node" :style="{ marginLeft: `${niveau * 2}rem` }">
    ...
    >
      ...
      <!-- Manager info - VERSION SIMPLE (un seul manager) -->
      ...
      <div v-else-if="membres.length > 0" class="membres-grid">
        ...
        >
          ...
          <div class="membre-actions">
            <!-- Bouton changer manager (visible si pas manager) -->
            <button 
              v-if="!isManager(membre)"
              class="btn-action set-manager"
              @click="setAsManager(membre)"
              title="Définir comme manager (remplace l'actuel)"
            >
              <i class="bi bi-person-up"></i>
            </button>
            <!-- Bouton retirer manager (visible si manager) -->
            <button 
              v-else
...
const removeMembre = async (membre: EquipeMembre) => {
  ...
  try {
    // Si c'est le manager, retirer aussi le rôle
    if (isManager(membre)) {
...
// CORRECTION : Remplacer le manager (un seul manager possible)
const setAsManager = async (membre: EquipeMembre) => {
  const currentManager = props.equipe.manager_details
  let message = `Définir ${getMembreDisplayName(membre)} comme manager ?`
  
  if (currentManager && currentManager.id !== membre.id) {
    message = `Remplacer ${getManagerDisplayName(currentManager)} par ${getMembreDisplayName(membre)} comme manager ?`
  }
