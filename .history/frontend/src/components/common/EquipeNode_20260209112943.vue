  ...
  <div class="equipe-node" :style="{ marginLeft: `${niveau * 2}rem` }">
    ...
    >
      ...
      <div v-if="equipe.manager_details" class="manager-info">
        ...
        <span class="manager-name">{{ generatePseudo(equipe.manager_details) }}</span>
...
const getManagerDisplayName = (manager: any) => {
  // Vérifie si manager est un objet valide
  if (!manager) return ''
  
  // Utilise le display_name s'il existe
  if (manager.display_name) return manager.display_name
  
  // Sinon, génère un pseudo à partir des infos disponibles
  return generatePseudo(manager.last_name, manager.pseudo || manager.first_name)
}
</script>
