  ...
  <div class="equipe-node" :style="{ marginLeft: `${niveau * 2}rem` }">
    ...
    >
      ...
      <div class="equipe-info">
        ...
        <span v-if="equipe.pole_details" class="pole-info">
          ...
          <span v-if="equipe.manager_details || (equipe.manager_details && equipe.manager_details.length > 0)" class="separator">|</span>
        </span>
      </div>
      
      <!-- Manager info - VERSION CORRIGÉE POUR MULTIPLE manager -->
      <div v-if="equipe.manager_details && equipe.manager_details.length > 0" class="manager-info">
        <span class="manager-label">Manager{{ equipe.manager_details.length > 1 ? 's' : '' }}:</span>
        <span v-for="(manager, index) in equipe.manager_details" :key="manager.id" class="manager-item">
          <span class="manager-name">{{ getManagerDisplayName(manager) }}</span>
          <span class="manager-matricule">{{ manager.username.toUpperCase() }}</span>
          <span v-if="index < equipe.manager_details.length - 1" class="manager-separator">, </span>
        </span>
      </div>
    ...
    <div v-if="showEditModal" class="modal-overlay" @click.self="closeEditModal">
      <div class="modal modal-large">
        ...
        <!-- Gestion des co-manager -->
        <div class="form-group" v-if="membres.length > 0">
          <label>Co-manager (maintenez Ctrl pour sélection multiple)</label>
          <select v-model="editForm.manager" multiple class="multi-select">
            <option v-for="membre in membres" :key="membre.id" :value="membre.id">
              {{ getMembreDisplayName(membre) }} ({{ membre.username.toUpperCase() }})
            </option>
          </select>
          <small class="help-text">Sélectionnez plusieurs utilisateurs pour avoir plusieurs manager</small>
        </div>
...
const editForm = ref({
  ...
  manager: [] as number[]  // NOUVEAU: pour les co-manager
})
...
const isManager = (membre: EquipeMembre) => {
  ...
  if (props.equipe.manager && props.equipe.manager.includes(membre.id)) return true
  if (props.equipe.manager_details && props.equipe.manager_details.some(m => m.id === membre.id)) return true
  return false
}
...
const canRemoveManager = (membre: EquipeMembre) => {
  ...
  // On peut retirer si c'est un co-manager OU si c'est le manager principal mais qu'il y a des co-manager
  if (membre.id === props.equipe.manager) {
    return (props.equipe.manager || []).length > 0
  }
  return true
}
...
const saveEquipe = async () => {
  ...
  try {
    ...
    // Ajouter les co-manager s'il y en a
    if (editForm.value.manager && editForm.value.manager.length > 0) {
      updateData.manager = editForm.value.manager
    }
...
const removeMembre = async (membre: EquipeMembre) => {
  ...
  try {
    ...
    if (props.equipe.manager && props.equipe.manager.includes(membre.id)) {
      const newmanager = props.equipe.manager.filter(id => id !== membre.id)
      await equipesApi.update(props.equipe.id, { manager: newmanager })
    }
...
const setAsManager = async (membre: EquipeMembre) => {
  ...
  try {
    // Récupérer la liste actuelle des manager ou créer un tableau vide
    const currentmanager = props.equipe.manager || []
    
    // Ajouter le nouveau manager s'il n'est pas déjà dans la liste
    if (!currentmanager.includes(membre.id)) {
      const newmanager = [...currentmanager, membre.id]
      await equipesApi.update(props.equipe.id, { manager: newmanager })
    }
...
const removeAsManager = async (membre: EquipeMembre) => {
  ...
  try {
    ...
    if (membre.id === props.equipe.manager) {
      // S'il y a des co-manager, promouvoir le premier comme principal
      const comanager = (props.equipe.manager || []).filter(id => id !== membre.id)
      if (comanager.length > 0) {
...
const confirmTransfer = async () => {
  ...
  try {
    ...
    if (isManager(membreToTransfer.value)) {
      const newmanager = (props.equipe.manager || []).filter(id => id !== membreToTransfer.value!.id)
      await equipesApi.update(props.equipe.id, { 
