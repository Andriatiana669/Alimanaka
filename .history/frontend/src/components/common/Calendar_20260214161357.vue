              ...
              <div class="events-container-horizontal">
                <div 
                  ...
                  :class="{ 
                    ...
                    'conge-matin': !event.allDay && event.start.getHours() === 8,
                    'conge-midi': !event.allDay && event.start.getHours() === 12,
                    'conge-journee': !event.allDay && event.start.getHours() === 8 && event.end?.getHours() === 17
                  }"
                ...
                >
                  ...
                  <div v-if="event.motif" class="event-motif" :title="event.motif">
...
const getEventsForDay = (date: Date) => {
  ...
  return props.events.filter(e => {
    // Ignorer les événements allDay dans la timeline sauf weekends/jours fériés
    if (e.allDay && e.type !== 'weekend' && e.type !== 'ferie' && e.type !== 'exceptionnel' && e.type !== 'conge_annuel') {
      return false
    }
    ...
    // Pour les événements multi-jours, on inclut le dernier jour
...
const formatEventTime = (event: CalendarEvent) => {
  if (event.allDay) {
    ...
    if (event.type === 'exceptionnel') return event.title.includes('⛔') ? 'Exceptionnel' : 'Ouvert'
    if (event.type === 'conge_annuel') return 'Congé annuel'
    return 'Toute la journée'
  }
  
  // Format heure de début uniquement pour les événements courts
  const startStr = format(event.start, 'HH:mm')
  
  // Si l'événement dure moins d'une journée, afficher l'heure
  if (event.end) {
...
const getEventStyle = (event: CalendarEvent) => {
  ...
  if (event.end) {
    ...
    duration = Math.max(endHour - startHour, 0.0)
  }
...
const getEventStyleHorizontal = (event: CalendarEvent) => {
  // Événements allDay (weekends, fériés, etc.)
  if (event.allDay) {
    return {
      left: '0px',
      width: '100%',
  ...
  const startHour = event.start.getHours() + event.start.getMinutes() / 60
  const left = startHour * 80 // 80px par heure
  ...
  const verticalOffset = (eventId % 5) * 12 // Décalage de 0, 15, 30, 45, 60px

  return {
    left: `${left}px`,
    width: `${width}px`,
    top: `${2 + verticalOffset}px`,
    height: `35px`, // Hauteur fixe pour éviter la superposition totale
    backgroundColor: event.color || '#bbdefb',
    position: 'absolute',
    zIndex: 10,
    overflow: 'hidden',
...
const shouldShowInTimeline = (event: CalendarEvent) => {
  // Ne pas montrer les congés allDay dans la timeline
  if (event.type === 'conge' && event.allDay) return false
  
  // Montrer les weekends, fériés, exceptionnels et congés annuels
  return true
}
