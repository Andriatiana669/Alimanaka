  ...
  <div class="equipe-node" :style="{ marginLeft: `${niveau * 2}rem` }">
    ...
    >
      ...
      <div class="equipe-info">
        ...
        <span v-if="equipe.pole_details" class="pole-info">
          ...
          <span v-if="hasManager" class="separator">|</span>
        </span>
      </div>
      
      <!-- Manager info - Affiche plusieurs managers -->
      <div v-if="hasManager" class="manager-info">
        <span class="manager-label">Manager{{ equipe.managers && equipe.managers.length > 1 ? 's' : '' }}:</span>
        <div class="managers-list">
          <span 
            v-for="manager in currentManagers" 
            :key="manager.id" 
            class="manager-tag"
            :title="manager.display_name || manager.username.toUpperCase()"
          >
            {{ manager.display_name || manager.username.toUpperCase() }}
          </span>
        </div>
        <span class="manager-count" v-if="currentManagers.length > 1">
    ...
    <div v-if="showMembres" class="membres-section">
      <div class="membres-header">
        <h4>Membres ({{ allMembres.length }})</h4>
      ...
      <div v-else-if="allMembres.length > 0" class="membres-grid">
        <div 
          v-for="membre in allMembres" 
          :key="membre.id" 
          class="membre-item"
        ...
        >
          ...
          <div class="membre-actions">
            ...
            <button 
              ...
              @click.stop="removeAsManager(membre)"
              title="Retirer comme manager"
            >
    ...
    <div v-if="showEditModal" class="modal-overlay" @click.self="closeEditModal">
      <div class="modal modal-large">
        ...
        <div class="form-group">
          <label>Managers (multiples)</label>
            ...
            >
              <label class="checkbox-label">
                <input 
                  type="checkbox" 
                  :value="membre.id" 
                  v-model="editForm.managers"
                  :checked="isManager(membre)"
                />
          ...
          <p class="form-help">Cochez les membres qui seront managers de cette équipe</p>
        </div>

        <div class="modal-actions">
          <button class="btn-secondary" @click="closeEditModal">Annuler</button>
          <button class="btn-danger" @click="deleteEquipe" v-if="niveau === 0 && canDelete">
        ...
        <div class="form-group">
          ...
          <select v-model="newMembreId" class="user-select">
            ...
            <option 
              ...
              :value="user.id"
            >
              {{ getMembreDisplayName(user) }} ({{ user.username.toUpperCase() }}) - {{ user.pole_details?.nom || 'Sans pôle' }}
            </option>
          </select>
        </div>
...
const canDelete = computed(() => {
  return props.equipe.membres_count === 0 && props.equipe.sous_equipes_count === 0
})

// Gestion des managers multiples
const currentManagers = computed(() => {
  const managers: any[] = []
  
  // Ajoute le manager principal si défini
  if (props.equipe.manager_details) {
    managers.push(props.equipe.manager_details)
  }
  
  // Ajoute les managers supplémentaires si l'API les fournit
  if (props.equipe.managers_details && Array.isArray(props.equipe.managers_details)) {
    props.equipe.managers_details.forEach(manager => {
...
const hasManager = computed(() => currentManagers.value.length > 0)

// Tous les membres (incluant les managers)
const allMembres = computed(() => {
...
// Vérifie si un membre est manager
const isManager = (membre: EquipeMembre) => {
  return membre.id === props.equipe.manager || 
         (props.equipe.managers && props.equipe.managers.includes(membre.id))
}
...
const equipesAvailableForTransfer = computed(() => {
  return allEquipes.value.filter(eq => {
    ...
    // Pas l'équipe actuelle
    if (eq.id === props.equipe.id) return false
...
const handleToggleStatus = () => {
  if (confirm(`${props.equipe.est_actif ? 'Désactiver' : 'Activer'} l'équipe "${props.equipe.nom}" ?`)) {
...
const openEditModal = async () => {
  ...
  // Initialiser les managers sélectionnés
  if (props.equipe.manager) {
    editForm.value.managers.push(props.equipe.manager)
  }
  if (props.equipe.managers && Array.isArray(props.equipe.managers)) {
    props.equipe.managers.forEach(managerId => {
      if (!editForm.value.managers.includes(managerId)) {
        editForm.value.managers.push(managerId)
      }
    })
  }
...
const saveEquipe = async () => {
  ...
  saving.value = true
  try {
    // Prend le premier manager comme manager principal
    const mainManager = editForm.value.managers.length > 0 ? editForm.value.managers[0] : null
    
    await equipesApi.update(props.equipe.id, {
      nom: editForm.value.nom,
      description: editForm.value.description,
      manager: mainManager,
      // Si ton API supporte les managers multiples
      // managers: editForm.value.managers
    })
...
const removeMembre = async (membre: EquipeMembre) => {
  ...
  try {
    // Si c'est un manager, retirer aussi le rôle de manager
    if (isManager(membre)) {
      // Si c'est le manager principal
      if (membre.id === props.equipe.manager) {
        await equipesApi.update(props.equipe.id, { manager: null })
      }
      // Pour les managers multiples, il faudrait mettre à jour la liste des managers
    }
...
const setAsManager = async (membre: EquipeMembre) => {
  if (!confirm(`Définir ${getMembreDisplayName(membre)} comme manager de l'équipe ?`)) return
  
  try {
    // Pour l'instant, on définit comme manager principal
    // Plus tard, tu pourras ajouter à une liste de managers
...
const removeAsManager = async (membre: EquipeMembre) => {
  if (!confirm(`Retirer ${getMembreDisplayName(membre)} comme manager de l'équipe ?`)) return
  
  try {
    // Si c'est le manager principal, le retirer
    if (membre.id === props.equipe.manager) {
      await equipesApi.update(props.equipe.id, { manager: null })
    }
    // Pour les managers multiples, il faudrait retirer de la liste
    ...
    alert('Erreur lors du retrait du manager')
  }
}

// TRANSFERT
const transferMembre = (membre: EquipeMembre) => {
...
const confirmTransfer = async () => {
  ...
  try {
    ...
    // Si c'était un manager, retirer le rôle
    if (isManager(membreToTransfer.value)) {
      // Si c'était le manager principal
