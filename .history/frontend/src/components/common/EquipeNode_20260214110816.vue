  ...
  <div class="equipe-node" :style="{ marginLeft: `${niveau * 2}rem` }">
    ...
    >
      ...
      <div class="equipe-info">
        ...
        <span
          v-if="!isNormalUser && equipe.pole_details"
          class="pole-info"
        >
      ...
      <!-- Managers (PAS pour user normal) -->
      <div
        v-if="!isNormalUser && equipeManagers.length > 0"
        class="managers-container"
      >
        ...
        <div class="managers-tags">
          ...
          >
            <i
              class="bi"
              :class="manager.id === equipe.manager
                ? 'bi-person-badge-fill'
                : 'bi-person-badge'"
            ></i>
            {{ generatePseudo(manager.last_name, manager.pseudo) }}
            <small class="manager-matricule">
      ...
      <!-- Bouton membres (uniquement si autorisé) -->
      <button
        v-if="!isNormalUser"
        class="membres-btn"
      ...
      <!-- ACTIONS -->
      <div class="actions">
        <!-- Toggle statut : UNIQUEMENT si autorisé -->
        <button
          v-if="canToggle"
          class="btn-status"
        ...
        >
          <i
            class="bi"
            :class="equipe.est_actif ? 'bi-toggle-on' : 'bi-toggle-off'"
          ></i>
          <span class="status-text">
            {{ equipe.est_actif ? 'Active' : 'Inactive' }}
          </span>
        </button>
      </div>
    </div>
    ...
    <!-- Membres -->
    <div v-if="showMembres && !isNormalUser" class="membres-section">
      <div v-if="loadingMembres">Chargement...</div>

      <div v-else-if="membres.length">
        <div
          v-for="membre in membres"
          :key="membre.id"
          class="membre-item"
        >
          {{ generatePseudo(membre.last_name, membre.pseudo) }}
        </div>
      </div>
    </div>
...
const getMembreFullName = (membre: any) => {
...
const hasChildren = computed(() => {
  return (props.equipe.sous_equipes?.length ?? 0) > 0
})


const isSelected = computed(
  () => props.selectedId === props.equipe.id
)

const equipeManagers = computed(() => {
  const list: EquipeMembre[] = []

  if (props.equipe.manager_details) {
    list.push(props.equipe.manager_details as any)
  }

  if (props.equipe.co_managers_details) {
    list.push(...(props.equipe.co_managers_details as any))
  }
...
const loadMembres = async () => {
  ...
  membres.value = await equipesApi.getMembres(props.equipe.id)
  loadingMembres.value = false
}

const toggleMembres = async () => {
  showMembres.value = !showMembres.value
  if (showMembres.value && !membres.value.length) {
    await loadMembres()
  }
}

const handleSelect = () => emit('select', props.equipe)
const handleToggleStatus = () => emit('toggle', props.equipe)
const handleMembreClick = (e: MouseEvent) => e.stopPropagation()
