...
const membres = ref<EquipeMembre[]>([]) // CHANGE: Utilise EquipeMembre au lieu de User
...
const membreToTransfer = ref<EquipeMembre | null>(null) // CHANGE
...
const loadInitialData = async () => {
  try {
    // VÉRIFIE LE NOM EXACT DE TA MÉTHODE DANS usersApi
    // Si c'est getAllUsers() et pas getAll()
    const usersResponse = await usersApi.getAllUsers() // CHANGE ICI
...
const addMembre = async () => {
  ...
  try {
    // VÉRIFIE LE NOM EXACT DE TA MÉTHODE DANS usersApi
    // Si c'est updateUser() et pas update()
    await usersApi.updateUser(newMembreId.value, { equipe: props.equipe.id }) // CHANGE ICI
...
const removeMembre = async (membre: EquipeMembre) => {
  ...
  try {
    ...
    // Retirer de l'équipe - VÉRIFIE LE NOM DE LA MÉTHODE
    await usersApi.updateUser(membre.id, { equipe: null }) // CHANGE ICI
...
const confirmTransfer = async () => {
  ...
  try {
    // Transférer vers la nouvelle équipe - VÉRIFIE LE NOM DE LA MÉTHODE
    await usersApi.updateUser(membreToTransfer.value.id, { equipe: transferToEquipeId.value }) // CHANGE ICI
