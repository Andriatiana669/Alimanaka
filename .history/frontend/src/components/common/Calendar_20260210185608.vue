        ...
        <div class="days-grid">
          <div v-for="day in weekDays" :key="day.date.toISOString()" class="day-column">
            ...
            <div class="events-container">
              <!-- Les événements seront fournis par la page parente -->
            </div>
          </div>
        </div>
      </div>
...
// ────────────────────────────────────────────────
// État
const STORAGE_KEY = 'calendar-preferred-view'
...
// ────────────────────────────────────────────────
// Restaurer la vue préférée au montage
onMounted(() => {
  const savedView = localStorage.getItem(STORAGE_KEY) as CalendarView | null
  if (savedView && (savedView === 'timeline' || savedView === 'month')) {
    currentView.value = savedView
  }
  syncSelectors()
})

// Sauvegarder à chaque changement de vue
...
// Helpers (utilisés par la page parente)
