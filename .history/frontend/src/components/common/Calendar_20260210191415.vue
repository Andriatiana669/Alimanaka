...
// ────────────────────────────────────────────────
// Props (pour passer les événements depuis la page parente)
...
const VIEW_STORAGE_KEY = 'calendar-preferred-view'
const currentView = ref<CalendarView>(
  (localStorage.getItem(VIEW_STORAGE_KEY) as CalendarView) || 'timeline'
)

// Sauvegarde automatique quand la vue change
...
const monthDays = computed(() => {
  ...
  // Chercher la fin de la semaine qui contient la fin du mois
  // On veut le premier jour de la semaine suivante (exclusif)
  const endOfWeekAfterMonthEnd = addDays(startOfWeek(addDays(monthEnd, 1), { weekStartsOn: 1 }), -1)
  
  const days = eachDayOfInterval({ 
    start, 
    end: endOfWeekAfterMonthEnd 
  })
...
// ────────────────────────────────────────────────
// Helpers pour les événements (reçoivent les props.events)
const getEventsForDay = (date: Date) => {
  return props.events.filter(e => isSameDay(e.start, date))
}

const getAllDayEventsForDay = (date: Date) => {
  return props.events.filter(e => isSameDay(e.start, date) && e.allDay)
}
