  ...
  <div class="equipe-node" :style="{ marginLeft: `${niveau * 2}rem` }">
    ...
    >
      ...
      <div v-if="equipe.manager_details" class="manager-info">
        ...
        <span class="manager-name">{{ getSimpleDisplayName(equipe.manager_details) }}</span>
        <span class="manager-matricule">{{ equipe.manager_details.username.toUpperCase() }}</span>
      </div>
      <div v-if="equipe.manager_details" style="font-size: 0.7rem; color: #666;">
        Debug: {{ JSON.stringify(equipe.manager_details) }}
      </div>
...
const getManagerDisplayName = (manager: any) => {
  ...
  console.log('Manager debug:', {
    manager,
    hasLastName: !!manager.last_name,
    hasPseudo: !!manager.pseudo,
    hasDisplayName: !!manager.display_name,
    firstName: manager.first_name
  })
  
  // Option 1: Utiliser display_name s'il existe (format complet)
  if (manager.display_name) return manager.display_name
  
  // Option 2: Créer le nom complet manuellement
  if (manager.first_name && manager.last_name) {
    return `${manager.first_name} ${manager.last_name}`
  }
  
  // Option 3: Utiliser generatePseudo avec ce qu'on a
  return generatePseudo(manager.last_name, manager.pseudo)
}

// OU plus simple - utiliser directement le display_name si disponible :
const getSimpleDisplayName = (manager: any) => {
  return manager?.display_name || generatePseudo(manager?.last_name, manager?.pseudo) || 'Non assigné'
}
</script>
