...
const isCurrentUserCoManager = computed(() => props.equipe.co_managers?.includes(currentUser.value?.id || 0) || false)

// Conditions d'affichage
const canSeeMembers = computed(() => {
  // Utilisateur normal ne voit que les membres de ses propres équipes
  if (!isAdmin.value) {
    return isManager.value || isCurrentUserCoManager.value
  }
  // Admin voit tout
  return true
})
...
const canToggleStatus = computed(() => {
  ...
  if (isAdmin.value && isManager.value && !isCurrentUserCoManager.value) return true
