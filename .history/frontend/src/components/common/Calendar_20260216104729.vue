<template>
  <div class="calendar-container">
    ...
    <div class="calendar-body">
      <!-- Vue Timeline (UNE SEULE JOURNÉE) -->
      <div v-if="currentView === 'timeline'" class="timeline-view">
        ...
        <div class="timeline-body-horizontal">
          <!-- UNE SEULE LIGNE pour le jour courant -->
          <div class="days-column">
            <div 
              class="day-row-header"
              :class="{ today: isToday(currentDate) }"
            >
              <div class="day-name">{{ format(currentDate, 'EEE', { locale: fr }).toUpperCase() }}</div>
          ...
          <div class="hours-grid-horizontal">
            ...
            >
              ...
              <!-- Container pour TOUS les événements du jour -->
              <div class="events-container-horizontal">
                <div 
                  v-for="(event, index) in getAllEventsForDay(currentDate).slice(0, 4)" 
                  :key="event.id"
                ...
                <div 
                  v-if="getAllEventsForDay(currentDate).length > 4"
                  class="more-events-indicator"
                  :style="{ top: `${4 + (4 * 34)}px` }"
                  @click="$emit('event-click', { type: 'more', date: currentDate, count: getAllEventsForDay(currentDate).length - 4 })"
                >
                  +{{ getAllEventsForDay(currentDate).length - 4 }} plus
                </div>
              </div>
            </div>
          </div>
...
const emit = defineEmits<{
  (e: 'event-click', event: CalendarEvent | { type: 'more', date: Date, count: number }): void
}>()
...
// Style pour la vue horizontale (UNE SEULE JOURNÉE)
const getEventStyleHorizontal = (event: CalendarEvent, index: number = 0) => {
  const eventHeight = 30
  const eventGap = 4
  const baseTop = 4 + (index * (eventHeight + eventGap))
  ...
  // Calcul précis basé sur l'heure exacte
