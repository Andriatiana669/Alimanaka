        ...
        <div class="timeline-body-horizontal">
          ...
          <div class="hours-grid-horizontal">
            ...
            >
              ...
              <!-- CORRECTION : Container pour TOUS les événements du jour -->
              <div class="events-container-horizontal">
                <div 
                  v-for="(event, index) in getAllEventsForDay(day.date).slice(0, 3)" 
                  :key="event.id"
                  ...
                  :style="getEventStyleHorizontal(event, index)"
                  @click="$emit('event-click', event)"
                >
                  <div class="event-time">{{ formatEventTime(event) }}</div>
                  <div class="event-title">{{ event.title }}</div>
                  <div v-if="event.type === 'conge'" class="status-indicator">
                    <span v-if="getCongeStatus(event.id) === 'approuve'" class="badge-validated">✓</span>
                    <span v-if="getCongeStatus(event.id) === 'refuse'" class="badge-refused">✗</span>
                  </div>
                </div>
                <!-- Indicateur "+X more" si trop d'événements -->
              <div 
                v-if="getAllEventsForDay(day.date).length > 3"
                class="more-events-indicator"
                :style="{ top: `${4 + (3 * 39)}px` }"
              >
                +{{ getAllEventsForDay(day.date).length - 3 }} plus
              </div>
...
const props = defineProps<{
  ...
  blockedDates?: Date[]
  conges?: any[] // Ajout pour accéder aux statuts
}>()
...
const getAllEventsForDay = (date: Date) => {
  ...
  }).sort((a, b) => a.start.getTime() - b.start.getTime()) // Trier par heure
}
...
const getEventStyleHorizontal = (event: CalendarEvent, index: number = 0) => {
  const eventHeight = 35; // px par événement
  const eventGap = 4; // px d'espacement
  const baseTop = 4 + (index * (eventHeight + eventGap)); // Position verticale basée sur l'index
  ...
  // CORRECTION : Calcul précis basé sur l'heure exacte
  const startHour = event.start.getHours() + (event.start.getMinutes() / 60)
  const pixelsPerHour = 60; // ← Réduit pour que ça rentre mieux
  const left = startHour * pixelsPerHour
  
  let duration = 1 // Durée par défaut 1h
  if (event.end) {
    const endHour = event.end.getHours() + (event.end.getMinutes() / 60)
    duration = Math.max(endHour - startHour, 0.5)
  } else {
    // Si pas d'heure de fin, durée selon le type de congé
    duration = 4 // 4h par défaut pour un congé
  }
  ...
  return {
    left: `${left}px`,
    width: `${Math.max(width, 60)}px`, // Minimum 60px de large
    top: `${baseTop}px`,
    height: `${eventHeight}px`,
    backgroundColor: event.color || '#bbdefb',
    position: 'absolute',
...
.hour-header-cell {
  flex: 0 0 60px; /* ← AVANT: flex: 1, width: 80px, MAINTENANT: flex fixe */
...
.day-row-header {
  height: 120px; /* Hauteur d'une ligne de jour */
  display: flex;
  flex-direction: column;
  align-items: center;
...
.day-row {
  height: 120px; /* Même hauteur que day-row-header */
  position: relative;
  border-bottom: 1px solid #e9ecef;
  display: flex;
}
...
.hour-cell-horizontal {
  flex: 0 0 60px; /* ← Correspond à hour-header-cell */
  width: 60px;
  border-right: 1px solid #e9ecef;
  height: 100%;
}
...
.calendar-event.validated {
  ...
  background: linear-gradient(90deg, #e8f5e9 0%, var(--bg-color, #bbdefb) 100%) !important;
}

.calendar-event.refused {
  border-left: 4px solid #f44336 !important;
  background: linear-gradient(90deg, #ffebee 0%, var(--bg-color, #bbdefb) 100%) !important;
  opacity: 0.8;
}
