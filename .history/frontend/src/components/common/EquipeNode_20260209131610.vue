...
const emit = defineEmits<{
  ...
  refresh: [equipe: Equipe]  // NOUVEAU: pour rafraîchir sans toggle
}>()
...
const isManager = (membre: EquipeMembre) => {
  // Vérifie manager unique (ancien format)
  if (props.equipe.manager === membre.id) return true
  // Vérifie dans le tableau des managers (nouveau format)
...
const openEditModal = async () => {
  // S'assurer que les membres sont chargés pour le select du manager
...
const saveEquipe = async () => {
  ...
  try {
    ...
    // Émettre un événement refresh pour rafraîchir l'arbre (pas toggle!)
...
const setAsManager = async (membre: EquipeMembre) => {
  ...
  try {
    ...
    // Rafraîchir l'arbre avec refresh (pas toggle)
    emit('refresh', props.equipe)
...
const transferMembre = async (membre: EquipeMembre) => {
  // Recharger les équipes pour avoir la liste à jour
  await loadInitialData()
...
const handleChildRefresh = (equipe: Equipe) => {
  // Propager l'événement refresh vers le parent
  emit('refresh', equipe)
}
