...
const hasLoadedMembres = ref(false) // NOUVEAU: pour éviter de recharger
  ...
  if (savedState) {
    try {
      ...
      if (state[props.equipe.id]) {
        ...
        if (showMembres.value && !hasLoadedMembres.value) {
...
const loadMembres = async () => {
  if (loadingMembres.value || hasLoadedMembres.value) return
  
  console.log(`Chargement membres équipe ${props.equipe.id}`)
  loadingMembres.value = true
  
  try {
    // Utilise l'action fetchMembres de ton store
    await equipesStore.fetchMembres(props.equipe.id)
    hasLoadedMembres.value = true
  } catch (error) {
    console.error('Erreur chargement membres:', error)
    hasLoadedMembres.value = false
  } finally {
...
watch(() => props.equipe.id, (newId, oldId) => {
  if (newId !== oldId) {
    // Réinitialiser pour la nouvelle équipe
    showMembres.value = false
    hasLoadedMembres.value = false
  }
})

// Utilise les membres depuis le store - VERSION SIMPLIFIÉE
const membres = computed(() => {
  // Version simple: retourne toujours les membres du store
  // Le store contient déjà les membres de la dernière équipe chargée
