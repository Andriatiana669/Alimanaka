...
const emit = defineEmits<{
  ...
  refresh: [] // NOUVEAU: Événement pour rafraîchir l'arbre
}>()
...
const equipeManagers = computed(() => {
  ...
  if (props.equipe.manager_details) {
    managers.push({
      id: props.equipe.manager_details.id,
      username: props.equipe.manager_details.username,
      first_name: props.equipe.manager_details.first_name || '',
      last_name: props.equipe.manager_details.last_name || '',
      pseudo: props.equipe.manager_details.pseudo || null,
      display_name: props.equipe.manager_details.display_name || ''
    } as EquipeMembre)
  }
  
  // Ajoute aussi le manager s'il est dans les membres
  if (props.equipe.manager && membres.value.length > 0) {
    const managerInMembres = membres.value.find(m => m.id === props.equipe.manager)
    if (managerInMembres && !managers.some(m => m.id === managerInMembres.id)) {
      managers.push(managerInMembres)
    }
  }
...
const debugLog = (action: string, data?: any) => {
  console.log(`EquipeNode [${props.equipe.id}]: ${action}`, data || '')
}
</script>
