        ...
        >
          ...
          <div class="membre-info">
            <div class="membre-name-row">
              ...
              <span v-else-if="isCoManager(membre.id)" class="co-manager-badge">🛡️ Co-Manager</span>
            </div>
      ...
      <div class="modal modal-large">
        ...
        <div class="form-group">
          ...
          <select v-model="transferToEquipeId" class="equipe-select">
            ...
            <optgroup v-for="poleGroup in equipesParPoleForTransfer" :key="poleGroup.pole || 'sans-pole'" :label="poleGroup.pole || 'Sans pôle'">
              <option 
                v-for="eq in poleGroup.equipes" 
                :key="eq.id" 
                :value="eq.id"
                :disabled="eq.id === equipe.id"
              >
                {{ eq.nom }} {{ eq.id === equipe.id ? '(actuelle)' : '' }}
              </option>
            </optgroup>
          </select>
        </div>
        ...
        <div class="modal-actions">
          ...
          <button 
            ...
            :disabled="!transferToEquipeId || transferring || transferToEquipeId === equipe.id"
          >
...
// Vérifier si un membre est co-manager
...
// Utilisateurs disponibles (même pôle ou sans équipe, pas dans l'équipe actuelle)
const usersAvailable = computed(() => {
  ...
  return allUsers.value.filter(user => {
    ...
    // Soit même pôle, soit sans pôle, soit sans équipe
    const memePole = user.pole === props.equipe.pole
    const sansEquipe = !user.equipe
    const sansPole = !user.pole
    
    return memePole || sansEquipe || sansPole
  })
})

// Équipes disponibles pour transfert (toutes sauf celle-ci, groupées par pôle)
const equipesParPoleForTransfer = computed(() => {
  ...
  allEquipes.value.forEach(eq => {
    ...
    const poleNom = eq.pole_details?.nom || 'Sans pôle'
  ...
  // Trier : Sans pôle d'abord, puis par ordre alphabétique
...
const loadInitialData = async () => {
  try {
    // Charger tous les utilisateurs (sans équipe ou tous selon permissions)
    const usersResponse = await usersApi.getAllUsers()
    allUsers.value = usersResponse
    
    // Charger toutes les équipes pour les transferts
...
// Gestion du double-clic sur membres
...
const openAddMembreModal = async () => {
  // Recharger les données à chaque ouverture pour avoir les utilisateurs à jour
  await loadInitialData()
...
const addMembre = async () => {
  ...
  try {
    ...
    await usersApi.updatePoleEquipe({
      user_id: newMembreId.value,
      pole_id: props.equipe.pole || 0,
      equipe_id: props.equipe.id
    })
...
const removeMembre = async (membre: EquipeMembre) => {
  ...
  try {
    ...
    // Retirer de l'équipe (mettre equipe à null)
    await usersApi.updatePoleEquipe({
      user_id: membre.id,
      pole_id: 0,  // Garder le pôle actuel ou 0 pour ne pas changer
      equipe_id: 0  // 0 = retirer de l'équipe
    })
...
const setAsManager = async (membre: EquipeMembre) => {
  if (!confirm(`Définir ${getMembreDisplayName(membre)} comme manager principal de l'équipe ?`)) return
...
const transferMembre = (membre: EquipeMembre) => {
  ...
  // Recharger les équipes pour avoir les données à jour
  loadInitialData()
}
...
const confirmTransfer = async () => {
  ...
  try {
    ...
    // Transférer vers la nouvelle équipe (et son pôle si différent)
    await usersApi.updatePoleEquipe({
      user_id: membreToTransfer.value.id,
      pole_id: targetEquipe?.pole || 0,
      equipe_id: transferToEquipeId.value
    })
    ...
    // Si c'était un co-manager, le retirer
