<template>
  <header class="navbar">
    <div class="navbar-left">
      <h1 class="breadcrumb">Lalamby : Alimanaka</h1>
      <p class="breadcrumb">Gestions des absences</p>
    </div>
    ...
    <div class="navbar-right">
      ...
      <div class="user-menu" @click="toggleDropdown" ref="userMenuContainer">
        <div class="user-info">
          <span class="user-name">{{ userFullName }} ({{ currentUser?.username.toUpperCase() }})</span>
...
// Récupération de l'utilisateur courant (avec données complètes)
const fetchCurrentUser = async () => {
  loadingUser.value = true
  try {
    const userData = await usersApi.getCurrentUser()
    currentUser.value = userData
    // Synchroniser avec le store pour les autres composants
    if (authStore.user) {
      authStore.user = { ...authStore.user, ...userData }
    }
  } catch (err: any) {
    console.error('Erreur récupération user:', err)
    currentUser.value = authUser.value as User
  } finally {
    loadingUser.value = false
  }
}

// Watch pour détecter les changements depuis le store (ex: Profile.vue)
watch(authUser, (newUser) => {
  if (newUser) {
    currentUser.value = { ...currentUser.value, ...newUser } as User
  }
}, { deep: true, immediate: true })

// Computed utilisent currentUser (qui a les données complètes)
const userFullName = computed(() => {
  if (!currentUser.value) return 'Utilisateur'
  if (currentUser.value.display_name) return currentUser.value.display_name
...
const userInitials = computed(() => {
  ...
  const nameToUse = currentUser.value.display_name || currentUser.value.pseudo
...
.navbar-left h1.breadcrumb {
  margin: 0;               /* empêche tout décalage vertical */
  line-height: normal;     /* conserve le rendu initial */
  font-size: inherit;      /* garde la taille actuelle */
  font-weight: inherit;    /* garde l’épaisseur actuelle */
}


.navbar-left p.breadcrumb {
  margin-top: 1px;
