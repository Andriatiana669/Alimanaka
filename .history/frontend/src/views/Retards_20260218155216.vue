...
const blockedDates = computed(() => {
  ...
  // 2. Jours fériés (depuis le store des congés)
  ...
  // 3. Jours exceptionnels (rayés)
  congesStore.joursExceptionnels
    .filter(je => je.type_jour === 'exceptionnel')
    .forEach(je => {
      dates.push(new Date(je.date))
    })
