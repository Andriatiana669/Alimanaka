...
const filteredEventsForCalendar = computed<CalendarEvent[]>(() => {
  ...
  return allEvents.filter(event => {
    // Pour les événements de type 'conge_partiel', on filtre par leur statut individuel
    if (event.type === 'conge_partiel') {
    ...
    // Garder les autres types d'événements (fériés, etc.)
    return true
  })
})
