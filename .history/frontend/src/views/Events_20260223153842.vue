<template>
    ...
    <div v-if="viewMode === 'calendar'" class="calendar-view">
      ...
      <div class="calendar-legend">
        ...
        <div class="legend-item">
          ...
          <span class="legend-label">Jour férié</span>
        </div>
        <div class="legend-item">
                ...
                <div class="event-content">
                  <div class="event-header">
                    <div class="event-title">
                      <span v-if="event.isSystem" class="event-system">
                        <i class="bi bi-calendar2-event"></i>
                        Système
                      </span>
                      <span v-else class="event-user">
                        <i class="bi bi-person-circle"></i>
                      ...
                      <span v-if="!event.isSystem" class="event-statut-badge" :class="event.statut">
                  ...
                  <div class="event-details">
                    <!-- Événements système -->
                    <template v-if="event.isSystem">
                      <span class="detail-chip">
                        <i class="bi bi-info-circle"></i>
                        {{ event.type === 'weekend' ? 'Weekend' : event.type === 'ferie' ? 'Jour férié' : 'Jour exceptionnel' }}
                      </span>
                      <span v-if="event.isBlocked" class="detail-chip blocked">
                        <i class="bi bi-lock"></i>
                        Jour bloqué
                      </span>
                    </template>

                    <!-- Détails spécifiques selon le type pour les événements utilisateur -->
                    <template v-else>
                      ...
                      <template v-else-if="event.type === 'retard'">
                        <span class="detail-chip">
                          ...
                          {{ event.minutes_retard || 0 }}min de retard
                        </span>
                        <span class="detail-chip">
                          <i class="bi bi-arrow-repeat"></i>
                          {{ event.heures_restantes || 0 }}h restantes
                        </span>
                      </template>
                    ...
                    <!-- Motif si présent -->
                    <span v-if="event.motif && !event.isSystem" class="detail-chip motif">
                  ...
                  <div v-if="canManageThisEvent(event) && !event.isSystem" class="event-actions" @click.stop>
                    <button 
    ...
    <!-- Modals (à importer selon vos besoins) -->
    <!-- 
    <Teleport to="body">
      <CongeDetailModal v-if="showCongeDetail" :conge="selectedEvent" @close="closeDetailModal" @refresh="refreshAll" />
      <RetardDetailModal v-if="showRetardDetail" :retard="selectedEvent" @close="closeDetailModal" @refresh="refreshAll" />
      <PermissionDetailModal v-if="showPermissionDetail" :permission="selectedEvent" @close="closeDetailModal" @refresh="refreshAll" />
      <ReposMedicalDetailModal v-if="showReposDetail" :repos="selectedEvent" @close="closeDetailModal" @refresh="refreshAll" />
      <OstieDetailModal v-if="showOstieDetail" :ostie="selectedEvent" @close="closeDetailModal" @refresh="refreshAll" />
      <RetourModal v-if="showRetourModal" :permission="selectedEvent" @close="closeRetourModal" @validated="handleActionValidated" />
      <RattrapageModal v-if="showRattrapageModal" :event="selectedEvent" @close="closeRattrapageModal" @validated="handleActionValidated" />
      <ValidationOstieModal v-if="showValidationOstieModal" :ostie="selectedEvent" @close="closeValidationOstieModal" @validated="handleActionValidated" />
      <TransformationModal v-if="showTransformationModal" :event="selectedEvent" :types-conge="typesConge" @close="closeTransformationModal" @validated="handleActionValidated" />
      <TransformationOstieModal v-if="showTransformationOstieModal" :ostie="selectedEvent" @close="closeTransformationOstieModal" @validated="handleActionValidated" />
    </Teleport>
    -->
</template>
...
const filteredEventsForCalendar = computed<CalendarEventType[]>(() => {
  return filteredEvents.value.map(event => {
    let color = event.color || getEventColor(event)
...
const getEventIcon = (type: string): string => {
  const icons: Record<string, string> = {
    ...
    ferie: 'bi bi-calendar-heart',
    exceptionnel: 'bi bi-exclamation-triangle',
    system: 'bi bi-gear'
  }
...
const getEventTitle = (event: any): string => {
  if (event.isSystem) {
    return event.title || 'Événement système'
  }
  
  const user = event.user_display_name || `Utilisateur #${event.user_id}`
  
  switch (event.type) {
    case 'conge':
      return `${user} - Congé`
    case 'retard':
      return `${user} - Retard ${event.minutes_retard || 0}min`
    case 'permission':
      return `${user} - Permission ${formatTime(event.heure_depart)}`
    case 'repos_medical':
      return `${user} - Repos médical ${event.duree_heures || 0}h`
    case 'ostie':
...
const getEventColor = (event: any): string => {
  ...
  if (event.isSystem) {
    if (event.type === 'weekend') return '#e0e0e0'
    if (event.type === 'ferie') return '#ffeb3b'
    if (event.type === 'exceptionnel') return '#ff5722'
    return '#95a5a6'
  }
...
const getTypeCount = (type: string): number => {
  return allEvents.value.filter(e => e.type === type && !e.isSystem).length
}
...
const loadAllEvents = async (reset = true) => {
  ...
  try {
    ...
    const userEvents = [
      ...congesStore.calendrierEvents
        .filter(e => e.user_id && e.type !== 'weekend' && e.type !== 'ferie' && e.type !== 'exceptionnel')
        .map(e => ({ ...e, type: 'conge' })),
      ...retardsStore.calendrierEvents
        .filter(e => e.user_id)
        .map(e => ({ ...e, type: 'retard' })),
      ...permissionsStore.calendrierEvents
        .filter(e => e.user_id)
        .map(e => ({ ...e, type: 'permission' })),
      ...reposStore.calendrierEvents
        .filter(e => e.user_id)
        .map(e => ({ ...e, type: 'repos_medical' })),
      ...ostieStore.calendrierEvents
        .filter(e => e.user_id)
        .map(e => ({ ...e, type: 'ostie' }))
    ]

    // Événements système (TOUS, sans filter)
    const systemEvents = congesStore.calendrierEvents
      .filter(e => !e.user_id) // Garder tous les événements sans user_id
      .map(e => ({ 
        ...e, 
        isSystem: true,
        // S'assurer que le type est correctement défini
        type: e.type || (
          e.title?.includes('🎉') ? 'ferie' :
          e.title?.includes('⛔') ? 'exceptionnel' :
          e.title?.includes('📅') ? 'weekend' : 'system'
        ),
        // Garder la couleur d'origine
        color: e.color || (
          e.type === 'ferie' ? '#ffeb3b' :
          e.type === 'exceptionnel' ? '#ff5722' :
          e.type === 'weekend' ? '#e0e0e0' : '#95a5a6'
        ),
        title: e.title || 'Jour spécial',
        isBlocked: e.isBlocked || false
      }))

    console.log('Événements système:', systemEvents) // Pour déboguer

    // Enrichir les événements utilisateur
    const enrichedUserEvents = userEvents.map(event => ({
      ...event,
      user_display_name: getUserDisplayName(event.user_id),
      isUserEvent: true
    }))

    allEvents.value = reset 
      ? [...enrichedUserEvents, ...systemEvents]
      : [...allEvents.value, ...enrichedUserEvents, ...systemEvents]
    
    console.log('Tous les événements:', allEvents.value) // Pour déboguer
    
    hasMoreEvents.value = userEvents.length >= pageSize
...
const refreshAll = async () => {
  await Promise.all([
    ...
    congesStore.fetchUtilisateursGerables ? congesStore.fetchUtilisateursGerables() : Promise.resolve(),
    retardsStore.fetchUtilisateursGerables ? retardsStore.fetchUtilisateursGerables() : Promise.resolve(),
    permissionsStore.fetchUtilisateursGerables ? permissionsStore.fetchUtilisateursGerables() : Promise.resolve(),
    reposStore.fetchUtilisateursGerables ? reposStore.fetchUtilisateursGerables() : Promise.resolve(),
    ostieStore.fetchUtilisateursGerables ? ostieStore.fetchUtilisateursGerables() : Promise.resolve()
  ])
...
const onEventClick = (calendarEvent: CalendarEventType) => {
  ...
  if (originalEvent.isSystem) return
...
onMounted(async () => {
  ...
  if (congesStore.canManageOthers) {
    if (congesStore.fetchUtilisateursGerables) await congesStore.fetchUtilisateursGerables()
    if (retardsStore.fetchUtilisateursGerables) await retardsStore.fetchUtilisateursGerables()
    if (permissionsStore.fetchUtilisateursGerables) await permissionsStore.fetchUtilisateursGerables()
    if (reposStore.fetchUtilisateursGerables) await reposStore.fetchUtilisateursGerables()
    if (ostieStore.fetchUtilisateursGerables) await ostieStore.fetchUtilisateursGerables()
  }
