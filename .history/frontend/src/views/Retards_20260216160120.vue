...
// Permissions
const canCreate = computed(() => true); // Tout le monde peut déclarer son retard
...
const selectedPole = ref<number | ''>('');
const selectedEquipe = ref<number | ''>('');
...
const onCalendarDateClick = (date: string) => {
  // Optionnel: pré-remplir la date dans le formulaire de création
...
const exportRetards = async () => {
  try {
    const response = await store.retardsApi?.exportRetards();
    if (response) {
...
// Watch pour recharger si l'année change
watch(selectedAnnee, () => {
  loadRetards();
});
</script>
