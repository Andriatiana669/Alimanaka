...
// Événements filtrés selon l'onglet actif
const filteredEventsForCalendar = computed<CalendarEvent[]>(() => {
  ...
  return events.filter(event => {
    const eventData = congesStore.calendrierEvents.find(e => String(e.id) === String(event.id))
    if (!eventData) return true
    
    if (statusFilter.value === 'approuve') {
      return eventData.type === 'conge' && !eventData.id?.toString().includes('en_attente')
    }
    if (statusFilter.value === 'en_attente') {
      return eventData.type === 'conge' && eventData.id?.toString().includes('en_attente')
    }
    return true
  })
...
// Watch pour sauvegarder l'année quand elle change
watch(yearFilter, (newYear) => {
  localStorage.setItem(STORAGE_KEY_YEAR, String(newYear))
})

// Watch pour recharger les données quand le filtre change
watch(statusFilter, () => {
  // Les événements sont filtrés via computed, pas besoin de recharger
...
// Gestion du changement d'année depuis le calendrier
