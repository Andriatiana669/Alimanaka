...
const minutesRetard = computed(() => {
  ...
  const [heures, minutes] = form.value.heure_arrivee_reelle.split(':').map(Number)
  const [heuresP, minutesP] = heureDebutPrevue.split(':').map(Number)
  
  const arrivee = new Date(dateStr)
  arrivee.setHours(heures, minutes, 0)
  
  const prevue = new Date(dateStr)
  prevue.setHours(heuresP, minutesP, 0)
