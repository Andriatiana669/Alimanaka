    ...
    <div v-if="showDetailModal" class="modal-overlay" @click.self="closeDetailModal">
      <div class="modal modal-large">
        ...
        <div class="modal-body" v-if="selectedPermission">
          ...
          <!-- Section Approuvé par (si statut approuvé) -->
          <div v-if="selectedPermission?.statut === 'approuve'" class="validation-box approved-box">

            <!-- DEBUG TEMPORAIRE -->
            <pre style="background: #f0f0f0; padding: 10px; font-size: 10px;">
              {{ JSON.stringify({
                statut: selectedPermission?.statut,
                valide_par: selectedPermission?.valide_par,
                valide_par_details: selectedPermission?.valide_par_details,
                date_validation: selectedPermission?.date_validation
              }, null, 2) }}
            </pre>
            <h5>✓ Permission approuvée</h5>
            
            <div v-if="selectedPermission.valide_par_details">
              <p><strong>Approuvé par:</strong> {{ selectedPermission.valide_par_details.display_name }}</p>
              <p class="detail-meta">Le {{ formatDateTime(selectedPermission.date_validation || '') }}</p>
            </div>
            
            <!-- Si approuvé via rattrapage complet -->
            <div v-if="selectedPermission.rattrapages?.length">
              <p class="detail-meta">
                Rattrapage terminé ({{ selectedPermission.rattrapages.length }} session(s))
              </p>
            </div>
          </div>
  ...
  <Teleport to="body">
    <div v-if="showRetourModal" class="modal-overlay" @click.self="closeRetourModal">
        ...
        <div class="modal-body">
            ...
            <p class="info-text">Vous pourrez choisir entre rattrapage ou transformation en congé.</p>
            </div>
            <div v-else-if="retourForm.heure_arrivee_reelle && minutesDepassement <= 0" class="preview-box info-box">
            <p>✅ Retour dans les temps !</p>
            <p><strong>⚠️ Rattrapage obligatoire de 30 minutes</strong></p>
            <p>Vous devrez effectuer un rattrapage de 30 minutes.</p>
            </div>
