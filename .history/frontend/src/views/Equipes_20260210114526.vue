...
// ========== UTILITAIRES ==========
const { getRoleLabel, getRoleClass, canEditEquipeDetails, canToggleEquipeStatus } = useRoles()
...
const toggleStatus = async (equipe: Equipe) => {
  if (!currentUser.value || !canToggleEquipeStatus(currentUser.value)) {
    alert('Permission refusée : Seuls les Super Admins peuvent modifier le statut des équipes')
    return
  }
