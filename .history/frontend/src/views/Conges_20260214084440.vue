<template>
  ...
  <!-- Filtres - Affiché seulement pour managers/admins -->
  <div v-if="congesStore.isManagerOrAdmin || congesStore.isSuperAdmin" class="filters-bar">
    <div class="filter-group">
      <label>Pôle</label>
      <select v-model="filters.pole" @change="onFilterChange">
        <option :value="null">Tous les pôles</option>
        <option v-for="pole in availablePoles" :key="pole.id" :value="pole.id">
          {{ pole.nom }}
        </option>
      </select>
    </div>
    ...
    <div class="filter-group">
      <label>Équipe</label>
      <select v-model="filters.equipe" @change="onFilterChange" :disabled="!filters.pole">
        <option :value="null">Toutes les équipes</option>
        <option v-for="equipe in availableEquipes" :key="equipe.id" :value="equipe.id">
          {{ equipe.nom }}
        </option>
      </select>
    </div>
  ...
  <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
    <div class="modal modal-large">
      <div class="modal-header">
        <h3>{{ isManagingOthers ? 'Ajouter un congé (Manager)' : 'Nouvelle demande de congé' }}</h3>
...
// Admin check
const isManagingOthers = computed(() => congesStore.canManageOthers && selectedUser.value !== null)
...
// Pôles et équipes disponibles pour les filtres
const availablePoles = computed(() => {
  // À récupérer depuis un store/API si nécessaire
  return []
})

const availableEquipes = computed(() => {
  if (!filters.value.pole) return []
  // Filtrer par pôle sélectionné
  return []
})
...
// Événements filtrés selon le statut
const filteredEventsForCalendar = computed<CalendarEvent[]>(() => {
  ...
  return allEvents.filter(event => {
    ...
    return conge.statut === statusFilter.value
  })
})
