...
const filtrerEquipesParRecherche = (equipes: Equipe[], query: string): Equipe[] => {
  ...
  return equipes
    .map(eq => {
      ...
      const membresFiltrees: EquipeMembre[] | undefined = eq.membres?.filter(m =>
      ...
      if (matchEquipe || (membresFiltrees && membresFiltrees.length > 0) || sousEquipesFiltrees.length > 0) {
        return {
          ...eq,
          membres: membresFiltrees,       // on peut laisser undefined si aucun membre filtré
