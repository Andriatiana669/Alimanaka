<template>
  ...
  <!-- Modal Création de retard (UNIQUE ET CORRIGÉ) -->
  <Teleport to="body">
    <div v-if="showCreateModal" class="modal-overlay" @click.self="closeCreateModal">
      <div class="modal modal-large">
        ...
        <div class="modal-body">
          ...
          <!-- Sélection du type de retard (avec heure configurable) -->
...
const allCalendarEvents = computed<CalendarEvent[]>(() => {
  ...
  // 1. Ajouter les retards
  events.push(...retardsStore.eventsForCalendar)
  
  // 2. Ajouter les jours fériés (priorité haute)
  congesStore.joursFeries.forEach(jf => {
    try {
      const date = new Date(currentYear, jf.mois - 1, jf.jour)
      events.push({
        id: `ferie_${jf.id}`,
        title: `🎉 ${jf.nom}`,
        start: date,
  ...
  congesStore.joursExceptionnels.forEach(je => {
    events.push({
      id: `exc_${je.id}`,
      title: `⛔ ${je.description || 'Exceptionnel'}`,
      start: new Date(je.date),
      allDay: true,
      color: je.type_jour === 'exceptionnel' ? '#ff5722' : '#4caf50',
      type: 'exceptionnel',
      isBlocked: je.type_jour === 'exceptionnel'
    })
  })
  
  // 4. Ajouter les weekends (avec isBlocked true pour le style)
  ...
  while (current <= end) {
    ...
    if (day === 0 || day === 6) { // Dimanche ou Samedi
      events.push({
        id: `weekend_${format(current, 'yyyy-MM-dd')}`,
        title: '📅 Weekend',
...
// Dates bloquées (weekends et jours fériés - AJOUTÉ)
const blockedDates = computed(() => {
  const dates: Date[] = []
  const currentYear = getYear(new Date())
  
  // Générer tous les weekends de l'année
  const start = new Date(currentYear, 0, 1)
  const end = new Date(currentYear, 11, 31)
  let current = new Date(start)
...
const openCreateModal = async () => {
  ...
  try {
    ...
    // Ouvrir le modal SEULEMENT après le chargement
...
const refreshData = async () => {
  ...
  await Promise.all([
    ...
    // Charger aussi les jours fériés et exceptionnels depuis le store des congés
    congesStore.fetchJoursFeries(),
    congesStore.fetchJoursExceptionnels(currentYear)
  ])
