<template>
  ...
  <div class="page-header">
    ...
    <div class="header-actions">
      ...
      >
        <i class="bi" :class="exportingMine ? 'bi-arrow-repeat spin' : 'bi-download'"></i>
        {{ exportingMine ? 'Export en cours...' : 'Exporter Seul' }}
      </button>

      <button 
        v-if="retardsStore.isSuperAdmin" 
        class="btn-export-all" 
        @click="exportAll"
        :disabled="exportingAll"
      >
        <i class="bi" :class="exportingAll ? 'bi-arrow-repeat spin' : 'bi-download'"></i>
        {{ exportingAll ? 'Export en cours...' : 'Exporter Tous' }}
      </button>
  ...
  <div v-if="retardsStore.isManagerOrAdmin || retardsStore.isSuperAdmin" class="filters-bar">
    ...
    <div v-if="showCreateModal" class="modal-overlay" @click.self="closeCreateModal">
      <div class="modal modal-large">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <div class="type-options">
              ...
              >
                ...
                />
                <span class="type-label">{{ type.nom }}</span>
          ...
          <!-- Aperçu du retard -->
          <div v-if="form.date && form.heure_arrivee_reelle && selectedType && minutesRetard > 0" class="preview-box" :style="{ borderLeftColor: selectedType.couleur }">
            <p><strong>⏰ {{ minutesRetard }} minutes</strong> de retard</p>
            <p>Type: <strong>{{ selectedType.nom }}</strong></p>
    ...
    <div v-if="showDetailModal" class="modal-overlay" @click.self="closeDetailModal">
      <div class="modal modal-large">
        ...
        <div class="modal-body" v-if="selectedRetard">
          ...
          <div class="detail-grid">
            ...
            <div class="detail-item">
              ...
              <span class="detail-minutes" :style="{ color: selectedRetard.type_retard_details?.couleur }">
        ...
        <div class="modal-actions">
          ...
          <!-- Bouton ajouter rattrapage - visible seulement si l'utilisateur peut gérer ce retard -->
          <button 
            v-if="canAddRattrapage(selectedRetard) && canManageThisRetard(selectedRetard)"
            class="btn-success"
          ...
          <!-- Bouton annuler - visible pour le propriétaire OU le manager -->
          <button 
            v-if="canCancelRetard(selectedRetard) && (selectedRetard?.utilisateur === authStore.user?.id || canManageThisRetard(selectedRetard))"
            class="btn-warning"
...
import type { GerableUserRetard, TypeRetardConfig } from '@/types/retards'
...
// ============================================
// Rattrapage retard - Admin
// ============================================

const canManageThisRetard = (retard: any): boolean => {
  if (!retard) return false
  
  // Si l'utilisateur n'est pas manager/admin, il ne peut pas gérer les retards des autres
  if (!retardsStore.isManagerOrAdmin && !retardsStore.isSuperAdmin) return false
  
  // Super admin peut tout faire
  if (retardsStore.isSuperAdmin) return true
  
  // Vérifier si l'utilisateur du retard est dans les équipes gérables
  const gerableUser = retardsStore.utilisateursGerables.find(u => u.id === retard.utilisateur)
  return !!gerableUser
}
...
const filteredEventsForCalendar = computed<CalendarEvent[]>(() => {
  return retardsStore.eventsForCalendar
})
...
const totalHeuresARattraper = computed(() => {
  ...
  return (typeof total === 'number' ? total : parseFloat(total) || 0).toFixed(2)
})
...
const submitForm = async () => {
  ...
  try {
    const payload: any = {
      ...
      type_retard_id: selectedTypeId.value
    }
...
const canAddRattrapage = (retard: any) => {
  if (!retard) return false
  return (retard.statut === 'en_attente' || retard.statut === 'en_cours') && 
         parseFloat(retard.heures_restantes) > 0
}
...
const exportingMine = ref(false)
const exportingAll = ref(false)

const exportMine = async () => {
  if (exportingMine.value) return
  
  exportingMine.value = true
  try {
    await retardsStore.exportMine()
  } catch (err: any) {
    console.error('Erreur export:', err)
    alert('Erreur lors de l\'export : ' + (err.message || 'Erreur inconnue'))
  } finally {
    exportingMine.value = false
  }
}
