...
const topRetardataires = computed(() => {
  ...
  // retardsFiltres contient DÉJÀ les retards filtrés par équipe/membre sélectionné
  retardsFiltres.value.forEach((r: any) => {
    if (r.statut === 'approuve' || r.statut === 'en_cours') {
      ...
      if (!stats[userId]) {
        // Chercher d'abord dans les détails du retard
        let userName = r.utilisateur_details?.display_name
        
        // Si pas trouvé, chercher dans allGerablesMembres
        if (!userName) {
  ...
  // Convertir en tableau, filtrer ceux avec > 0 heures, trier et prendre top 5
