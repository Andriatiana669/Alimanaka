  ...
  <div v-if="showCreateModal" class="modal-overlay" @click.self="closeCreateModal">
    <div class="modal modal-large">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <div class="type-options">
            ...
            >
              ...
              <span class="type-time">Début prévu: {{ selectedType ? formatTime(selectedType.heure_debut_prevue) : '--:--' }}</span>
        ...
        <!-- Aperçu du retard -->
        <div v-if="form.date && form.heure_arrivee_reelle && selectedType" class="preview-box" :class="retardTypeClass">
    ...
    <!-- Modal test - simplifié -->
  <div v-if="showCreateModal" class="modal-overlay" @click.self="closeCreateModal">
    <div class="modal" style="background: white; padding: 20px; max-width: 400px;">
      <h3>TEST MODAL</h3>
      <p>Le modal fonctionne !</p>
      <p>Types chargés: {{ typesRetard.length }}</p>
      <button @click="closeCreateModal">Fermer</button>
    </div>
  </div>
...
const parseTime = (timeStr: string | undefined): { hours: number; minutes: number } => {
  ...
  // Récupérer les valeurs avec des vérifications
  const hoursStr = parts[0]
  const minutesStr = parts[1]
  
  return {
    hours: hoursStr ? parseInt(hoursStr) || 0 : 0,
    minutes: minutesStr ? parseInt(minutesStr) || 0 : 0
  }
}

const formatTime = (timeStr: string | undefined): string => {
  if (!timeStr) return '--:--'
  // S'assurer que c'est une chaîne et prendre les 5 premiers caractères
...
const minutesRetard = computed(() => {
  ...
  const prevue = parseTime(selectedType.value.heure_debut_prevue)  // ← Maintenant ça existe
  
  const arriveeDate = new Date(dateStr)
  arriveeDate.setHours(arrivee.hours, arrivee.minutes, 0)
  
  const prevueDate = new Date(dateStr)
...
const heuresARattraper = computed(() => {
  ...
  return ((minutesRetard.value / 60) * (selectedType.value.multiplicateur || 1)).toFixed(2)
})
...
const openCreateModal = async () => {
  ...
  try {
    if (retardsStore.typesRetard.length === 0) {
      ...
      console.log('Types retard chargés:', retardsStore.typesRetard) // ← AJOUTEZ CE LOG
    }
    
    if (retardsStore.canManageOthers) {
      await retardsStore.fetchUtilisateursGerables()
      console.log('Utilisateurs gérables:', retardsStore.utilisateursGerables) // ← AJOUTEZ CE LOG
    }
    
    showCreateModal.value = true
    console.log('Modal ouvert') // ← AJOUTEZ CE LOG
  } catch (error) {
...
.modal {
  ...
  z-index: 10000;  /* ← ENCORE PLUS ÉLEVÉ */
  position: relative;
}
