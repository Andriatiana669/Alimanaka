...
const filteredEventsForCalendar = computed<CalendarEventType[]>(() => {
  return filteredEvents.value.map(event => {
    ...
    // Pour les événements système, utiliser leur couleur définie
    if (event.isSystem) {
...
const getEventColor = (event: any): string => {
  // Si l'événement a déjà une couleur définie
  if (event.color) return event.color
  
  // Pour les événements système
  if (event.isSystem) {
  ...
  // Priorité au statut pour la couleur des événements utilisateur
...
const loadAllEvents = async (reset = true) => {
  ...
  try {
    ...
    // Fusionner tous les événements utilisateur
    const userEvents = [
    ...
    // Récupérer les événements système (jours fériés, exceptionnels, weekends)
    const systemEvents = [
      ...congesStore.calendrierEvents
        .filter(e => !e.user_id && e.type !== 'weekend') // Jours fériés et exceptionnels
        .map(e => ({ 
          ...e, 
          type: e.type || 'system',
          isSystem: true,
      ...
      ...congesStore.calendrierEvents
        .filter(e => e.type === 'weekend') // Weekends
        .map(e => ({ 
          ...e, 
          type: 'weekend',
          isSystem: true,
    ...
    // Enrichir les événements utilisateur avec les détails
    ...
    allEvents.value = reset ? allEvents_ : [...allEvents.value, ...allEvents_]
...
const getUserDisplayName = (userId: number | undefined): string => {
  ...
  // Si pas trouvé, essayer de chercher dans authStore.user (l'utilisateur connecté)
  if (authStore.user && authStore.user.id === userId) {
    return authStore.user.display_name || authStore.user.username || `Utilisateur #${userId}`
  }
  
  console.warn(`Utilisateur #${userId} non trouvé dans allUsers`) // 👈 LOG POUR DÉBOGUER
...
const refreshAll = async () => {
  await Promise.all([
    ...
    // AJOUTEZ CES LIGNES pour charger les utilisateurs gérables
    congesStore.fetchUtilisateursGerables(),
    retardsStore.fetchUtilisateursGerables(),
