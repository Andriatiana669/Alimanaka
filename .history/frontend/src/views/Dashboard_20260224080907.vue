  ...
  <div class="dashboard">
      ...
      <div v-if="topRetardataires.length > 0" class="top-list">
        ...
        <div class="top-items">
          <div v-for="(item, index) in topRetardataires" :key="item.userId" class="top-item">
            ...
            <span class="top-value">{{ item.totalHeures }}h</span>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="stats-grid">
          <div class="stats-card">
            ...
            <div class="pie-chart-placeholder">
              <!-- Ici tu pourras mettre un vrai graphique Chart.js -->
    ...
    <div v-if="isStaff" class="dashboard-section">
      ...
      <div class="alerts-list">
        ...
        <div v-if="Object.keys(alertes).length === 0" class="alert-item success">
...
import { format, parseISO, formatDistanceToNow, isToday, startOfDay, getMonth } from 'date-fns'
...
const isManagerOrHigher = computed(() => {
  return congesStore.isManagerOrAdmin || congesStore.isSuperAdmin || authStore.user?.is_staff
})
...
const congesApprouves = computed(() => {
  ...
  return congesStore.conges.filter(c => c.statut === 'approuve').length
})

const congesEnAttente = computed(() => {
  if (!congesStore.conges) return 0
  return congesStore.conges.filter(c => c.statut === 'en_attente').length
})

const congesEnAttenteList = computed(() => {
  if (!congesStore.conges || !isManagerOrHigher.value) return []
  return congesStore.conges.filter(c => c.statut === 'en_attente')
})
...
const ostieTransformes = computed(() => {
  ...
  return ostieStore.osties.filter(o => o.statut === 'transforme').length
})

const ostieEnAttenteList = computed(() => {
  if (!ostieStore.osties || !isManagerOrHigher.value) return []
  return ostieStore.osties.filter(o => o.statut === 'en_attente')
})

// ========== COMPUTED - STATS PERMISSIONS ==========
const permissionsHeures = computed(() => {
  return permissionsStore.totalHeuresARattraper?.toFixed(2) || '0.00'
})
...
const permissionsEnAttenteList = computed(() => {
  ...
  return permissionsStore.permissions.filter(p => p.statut === 'en_attente')
})

// ========== COMPUTED - STATS REPOS ==========
const reposHeures = computed(() => {
  return reposStore.totalHeuresRepos?.toFixed(2) || '0.00'
})

const reposEnAttente = computed(() => reposStore.reposEnAttente || [])
const reposApprouves = computed(() => {
  if (!reposStore.repos) return 0
  return reposStore.repos.filter(r => r.statut === 'approuve').length
})

const reposEnAttenteList = computed(() => {
  if (!reposStore.repos || !isManagerOrHigher.value) return []
  return reposStore.repos.filter(r => r.statut === 'en_attente')
})
...
const retardsRattrapes = computed(() => {
  ...
  return retardsStore.retards.filter(r => r.statut === 'approuve').length
})
...
const mesDemandes = computed(() => {
  ...
  if (retardsStore.retards) {
    retardsStore.retards.forEach(r => {
      if (r.utilisateur === userId) {
        demandes.push({
          ...r,
          type: 'retards',
...
const teamMembers = computed(() => {
  ...
  return filtersStore.utilisateurs?.length || 0
})
...
const absentsAujourdhui = computed(() => {
  ...
  // Congés aujourd'hui
  if (congesStore.conges) {
    count += congesStore.conges.filter(c => 
      c.statut === 'approuve' && 
      c.date_debut <= today && 
      c.date_fin >= today
    ).length
  ...
  // OSTIE aujourd'hui
  if (ostieStore.osties) {
    count += ostieStore.osties.filter(o => 
  ...
  if (reposStore.repos) {
    count += reposStore.repos.filter(r => 
      r.statut === 'approuve' && 
      r.date === today
    ).length
  }
...
const retardsAujourdhui = computed(() => {
  ...
  if (!retardsStore.retards) return 0
  return retardsStore.retards.filter(r => 
...
const globalStats = computed(() => {
  return {
    ...
    repos: reposStore.repos?.length || 0,
...
const alertes = computed(() => {
  ...
  if (!hasTypesRetard.value) alerts.typesRetard = 'Aucun type de retard'
  if (!hasTypesConge.value) alerts.typesConge = 'Aucun type de congé'
  return alerts
})
...
const goToModule = (module: string, id?: number) => {
  ...
  if (path) {
    if (id) {
      // Ouvrir le modal avec l'ID (à implémenter dans chaque module)
