  ...
  <div v-if="showDetailModal" class="modal-overlay" @click.self="closeDetailModal">
    <div class="modal modal-large">
      <div class="modal-header" :class="{
        ...
        'header-replaced': selectedConge?.statut === 'remplace'
      }">
        <h3>
          Détails du congé
          ...
          <span v-if="selectedConge?.statut === 'remplace'" class="header-badge replaced">🔄 Fractionné</span>
          <span v-if="selectedConge?.statut === 'en_attente'" class="header-badge pending">⏳ En attente</span>
        </h3>
        <button class="btn-close" @click="closeDetailModal">×</button>
      </div>

      <div class="modal-body" v-if="selectedConge">
        <!-- Message pour congé fractionné -->
        <div v-if="selectedConge.statut === 'remplace'" class="alert-info">
          <i class="bi bi-info-circle-fill"></i>
          <div>
            <strong>Ce congé a été fractionné</strong>
            <p>Il a été remplacé par plusieurs périodes distinctes suite à une validation partielle.</p>
            <button v-if="congesFractionnes.length > 0" class="btn-link" @click="voirCongesFractionnes">
              Voir les {{ congesFractionnes.length }} période(s) créée(s)
            </button>
          </div>
        </div>
        ...
        <div class="detail-grid">
          ...
          <div class="detail-item">
            ...
            <span class="detail-badge" :class="'status-' + selectedConge.statut">
              {{ getStatutDisplay(selectedConge.statut) }}
            </span>
          </div>
        ...
        <!-- Liste des congés fractionnés (si c'est l'original) -->
        <div v-if="congesFractionnes.length > 0" class="fractionnes-list">
          <h4>Périodes fractionnées</h4>
          <div class="fractionnes-grid">
            <div 
              v-for="conge in congesFractionnes" 
              :key="conge.id" 
              class="fractionne-card"
              :class="'status-' + conge.statut"
              @click="voirConge(conge.id)"
            >
              <div class="fractionne-header">
                <span class="fractionne-periode">
                  {{ formatDate(conge.date_debut) }} → {{ formatDate(conge.date_fin) }}
                </span>
                <span class="fractionne-badge" :class="'status-' + conge.statut">
                  {{ getStatutCourt(conge.statut) }}
                </span>
              </div>
              <div class="fractionne-footer">
                <span class="fractionne-deduction">{{ conge.jours_deduits }}j</span>
                <span class="fractionne-icon">👆 Voir</span>
              </div>
            </div>
          </div>
        </div>

        <!-- Lien vers le congé original (si c'est un fractionné) -->
        <div v-if="selectedConge.conge_original" class="original-link">
          <i class="bi bi-arrow-return-left"></i>
          <span>Issu du fractionnement de </span>
          <button class="btn-link" @click="voirConge(selectedConge.conge_original)">
            la demande du {{ formatDate(selectedConge.conge_original_details?.date_debut) }}
          </button>
        </div>
      </div>
      ...
      <div class="modal-actions">
        ...
        <template v-if="canManageThisConge(selectedConge) && selectedConge?.statut === 'en_attente'">
          ...
          <button v-if="isMultiDay(selectedConge)" class="btn-success-outline" @click="openPartialValidation">
            <i class="bi bi-calendar-check"></i> Valider partiel
          </button>
        </template>
        ...
        <template v-if="canCancelConge(selectedConge)">
          ...
          <button v-if="selectedConge?.statut === 'approuve' && isMultiDay(selectedConge)" class="btn-warning-outline" @click="openPartialCancel">
            <i class="bi bi-calendar-x"></i> Annuler partiel
          </button>
        </template>
      </div>
    </div>
...
// Fraction
const congesFractionnes = ref<any[]>([])

const chargerCongesFractionnes = async (congeId: number) => {
  try {
    // À adapter selon ton API
    const response = await congesApi.getCongesFractionnes(congeId)
    congesFractionnes.value = response
  } catch (error) {
    console.error('Erreur chargement congés fractionnés:', error)
    congesFractionnes.value = []
  }
}

const voirConge = async (congeId: number) => {
  try {
    const conge = await congesApi.getCongeDetails(congeId)
    selectedConge.value = conge
    await chargerCongesFractionnes(congeId)
  } catch (error) {
    console.error('Erreur chargement congé:', error)
  }
}

const voirCongesFractionnes = () => {
  // Option 1: Afficher dans une liste à part
  // Option 2: Filtrer le calendrier pour ne montrer que ces congés
  console.log('Afficher les congés fractionnés:', congesFractionnes.value)
}
...
const getStatutCourt = (statut: string) => {
  const map: Record<string, string> = {
    'en_attente': '⏳',
    'approuve': '✓',
    'refuse': '✗',
