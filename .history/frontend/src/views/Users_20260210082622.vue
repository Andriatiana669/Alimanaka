      ...
      <table v-else class="users-table">
        <thead>
          <tr>
            ...
            <!-- PÔLE - ÉDITABLE EN LIGNE -->
            <th>Pôle</th>
            <!-- ÉQUIPE - ÉDITABLE EN LIGNE AVEC INDICATION -->
            <th>Équipe</th>
            <th>Rôle</th>
...
const canEditUser = (user: User): boolean => {
  ...
  if (!currentAuthUser.value) return false
  return user.id !== currentAuthUser.value.id
}
...
const viewProfile = (user: User) => {
  ...
  // Si vous avez une route nommée 'profile'
