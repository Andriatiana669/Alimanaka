...
const openRequestModal = async () => {
  ...
  try {
    console.log('1. typesConge.value.length:', typesConge.value.length)
    console.log('2. congesStore.typesConge.length:', congesStore.typesConge.length)
    
    if (typesConge.value.length === 0) {
      console.log('3. Appel API fetchTypesConge...')
      await congesStore.fetchTypesConge()
      console.log('4. API terminée, types chargés:', congesStore.typesConge)
      
      typesConge.value = congesStore.typesConge
      console.log('5. typesConge.value mis à jour:', typesConge.value)
    } else {
      console.log('3. Types déjà chargés')
    }
    
    console.log('6. Tentative ouverture modal')
    showModal.value = true
    console.log('7. showModal =', showModal.value)
    
  } catch (error) {
    console.error('❌ Erreur dans openRequestModal:', error)
    formError.value = 'Impossible de charger les types de congé'
  }
}
