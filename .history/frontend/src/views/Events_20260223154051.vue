...
const loadAllEvents = async (reset = true) => {
  ...
  try {
    ...
    // Événements utilisateur (avec user_id)
    const userEvents = [
    ...
    // Événements système (TOUS les événements sans user_id)
    // Ça inclut: weekends, jours fériés, jours exceptionnels, etc.
    const systemEvents = congesStore.calendrierEvents
      .filter(e => !e.user_id) // Tous les événements sans utilisateur
      .map(e => ({ 
        ...e, 
        isSystem: true,
        // Conserver le type original (weekend, ferie, exceptionnel)
        type: e.type || 'system',
        // S'assurer que isBlocked est préservé
