<template>
  ...
  <div class="filters-bar">
    ...
    <!-- TOGGLE Inactives - visible seulement pour super admin ou (admin et manager) -->
    <label v-if="canToggleInactives" class="toggle-inactives">
      <input 
        type="checkbox" 
...
import { usersApi } from '@/api/users' // ← IMPORTANT: Ajouter cet import
...
const STORAGE_KEY = 'equipes_filters'
const loadFiltersFromStorage = () => {
  const saved = localStorage.getItem(STORAGE_KEY)
  if (saved) {
    try {
      const filters = JSON.parse(saved)
      afficherInactives.value = filters.afficherInactives ?? true
      poleFilter.value = filters.poleFilter ?? ''
      equipeFilter.value = filters.equipeFilter ?? ''
...
// CORRECTION: isAdmin est une fonction, pas une valeur réactive
const isUserAdmin = computed(() => currentUser.value ? isAdmin(currentUser.value) : false)

const isUserManager = computed(() => currentUser.value?.est_chef_equipe === true)

// Condition pour afficher le toggle inactives
const canToggleInactives = computed(() => {
  return isSuperAdmin.value || (isUserAdmin.value && isUserManager.value)
})
...
// Filtrer l'arbre pour un utilisateur normal (ne voit que ses équipes)
const filtrerArbrePourUtilisateurNormal = (equipes: Equipe[], userId: number): Equipe[] => {
  const filtrerRecursif = (node: Equipe): Equipe | null => {
    // Vérifier si l'utilisateur est dans cette équipe via membres[]
    const estMembre = node.membres?.some(membre => membre.id === userId) || false
    const estManager = node.manager === userId
    const estCoManager = node.co_managers?.includes(userId) || false
    ...
    // Garder le nœud si l'utilisateur est membre/manager/co-manager OU si des enfants sont gardés
    const garderNode = estMembre || estManager || estCoManager || enfantsFiltres.length > 0
...
const arbreEquipesFiltrees = computed(() => {
  ...
  // 0. Si utilisateur normal (non staff et non superuser), filtrer par ses équipes
  if (currentUser.value && !currentUser.value.is_staff && !currentUser.value.is_superuser) {
    result = filtrerArbrePourUtilisateurNormal(result, currentUser.value.id)
  }
...
const onSearchInput = () => {
  ...
  searchTimeout.value = window.setTimeout(() => {
    // Sauvegarder les filtres après le debounce
    saveFiltersToStorage()
  }, 300)
}
...
const toggleStatus = async (equipe: Equipe) => {
  // Vérifier les permissions (super admin ou admin et manager)
  if (!currentUser.value || !canToggleInactives.value) {
    alert(`Permission refusée : Vous n'avez pas les droits pour modifier le statut des équipes`)
    return
  }
...
onMounted(async () => {
  ...
  // Watcher pour sauvegarder les changements de afficherInactives
  watch(afficherInactives, () => {
    saveFiltersToStorage()
  })
})
