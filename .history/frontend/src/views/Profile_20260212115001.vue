...
const loadProfile = async () => {
  ...
  try {
    // Teste les deux endpoints pour voir la différence
    const profileData = await usersApi.getUserProfile()
    console.log('Données de /api/users/profile/:', profileData)
    console.log('pole_details:', profileData.pole_details)
    console.log('equipe_details:', profileData.equipe_details)
...
const savePseudo = async () => {
  ...
  try {
    ...
    // ✅ IMPORTANT: Mettre à jour le store auth pour propagation
    const authStore = useAuthStore()
    if (authStore.user) {
      authStore.user = {
        ...authStore.user,
