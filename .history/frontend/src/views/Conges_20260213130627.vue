<template>
    ...
    <div class="filters-bar">
      ...
      <div class="filter-tabs">
        <button :class="{ active: statusFilter === 'tous' }" @click="setStatusFilter('tous')">
          Tous
        </button>
        
        <button :class="{ active: statusFilter === 'approuve' }" @click="statusFilter = 'approuve'">
          Approuvés
        </button>
        <button :class="{ active: statusFilter === 'en_attente' }" @click="statusFilter = 'en_attente'">
          En attente
        </button>
      </div>
...
import { watch } from 'vue'  // ← Nouveau
import { getYear } from 'date-fns'  // ← Nouveau
import type { CalendarEvent } from '@/types/conges'  // ← Nouveau


// Resaka mitana onglet
const STORAGE_KEY_STATUS = 'conges_status_filter'
const STORAGE_KEY_YEAR = 'conges_year_filter'
...
const statusFilter = ref<'tous' | 'approuve' | 'en_attente'>((localStorage.getItem(STORAGE_KEY_STATUS) as 'tous' | 'approuve' | 'en_attente') || 'tous')
...
const setStatusFilter = (filter: 'tous' | 'approuve' | 'en_attente') => {
  ...
  localStorage.setItem(STORAGE_KEY_STATUS, filter)  // ← Sauvegarde
}


const filteredEventsForCalendar = computed<CalendarEvent[]>(() => {
  let events = congesStore.eventsForCalendar
...
// Sauvegarde l'année quand elle change
watch(yearFilter, (newYear) => {
  localStorage.setItem(STORAGE_KEY_YEAR, String(newYear))
})

// Log quand le filtre change (debug)
watch(statusFilter, () => {
  console.log('Filtre changé:', statusFilter.value)
})


const onCalendarYearChange = (newYear: number) => {
  yearFilter.value = newYear
}

onMounted(async () => {
  // Restaurer les filtres depuis localStorage
  const savedStatus = localStorage.getItem(STORAGE_KEY_STATUS) as 'tous' | 'approuve' | 'en_attente' | null
  const savedYear = localStorage.getItem(STORAGE_KEY_YEAR)
  
  if (savedStatus && ['tous', 'approuve', 'en_attente'].includes(savedStatus)) {
    statusFilter.value = savedStatus
  }
  
  if (savedYear) {
    yearFilter.value = parseInt(savedYear)
  }
  ...
  // Puis charger les congés avec les filtres restaurés
  await refreshData()
  
  // Écouteur pour rafraîchir quand retour sur l'onglet
  document.addEventListener('visibilitychange', onVisibilityChange)
  
  // Écouteur pour les changements d'année depuis le calendrier (si le calendrier émet un événement)
  window.addEventListener('calendar-year-change', ((e: CustomEvent) => {
    onCalendarYearChange(e.detail)
  }) as EventListener)
})

onUnmounted(() => {
  document.removeEventListener('visibilitychange', onVisibilityChange)
  window.removeEventListener('calendar-year-change', ((e: CustomEvent) => {
    onCalendarYearChange(e.detail)
  }) as EventListener)
})
</script>
