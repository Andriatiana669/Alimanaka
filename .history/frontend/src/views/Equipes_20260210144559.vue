...
const toggleStatus = async (equipe: Equipe) => {
  ...
  try {
    ...
    console.log('Tentative de mise à jour équipe ID:', equipe.id)
    console.log('Données envoyées:', equipeData)
    
    const response = await equipesStore.updateEquipe(equipe.id, equipeData)
