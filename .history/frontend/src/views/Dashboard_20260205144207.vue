...
const conicGradientStyle = computed(() => {
  const segments = teamDistribution.value
  let gradient = ''
  let accumulated = 0
  
  segments.forEach((segment, index) => {
    const start = accumulated
    const end = accumulated + segment.percentage
...
const viewAllActivities = () => {
  // router.push('/activities') // À créer plus tard
  console.log('Voir toutes les activités')
}

const viewAllPending = () => {
  // router.push('/approvals') // À créer plus tard
  console.log('Voir toutes les approbations')
}
...
const requestLeave = () => {
  // router.push('/leaves/request') // À créer plus tard
  console.log('Demander un congé')
}
...
const viewTeam = () => {
  // router.push('/team') // À créer plus tard
...
.pie-chart {
  ...
  border-radius: 50%;
  margin: 0 auto;
  background: v-bind(conicGradientStyle);
}
