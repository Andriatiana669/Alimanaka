...
<template>
  <div class="equipes-page">
    ...
    <div class="page-header">
      ...
      <!-- DEBUG: Affiche le rôle -->
      <span class="role-badge-header" :class="getUserRoleClass(authUser)">
        {{ getUserRole(authUser) }}
      </span>
      <button v-if="canAdminEquipes" class="btn-primary" @click="openCreateModal">
    ...
    <div v-if="selectedEquipe" class="equipe-details">
      <div class="details-header">
        ...
        <!-- Super Admin peut tout modifier, Admin limité -->
        <button 
          v-if="canAdminEquipes" 
          class="btn-icon edit" 
          @click="openEditModal(selectedEquipe)"
        >
          <i class="bi bi-pencil"></i> 
          {{ isSuperAdmin ? 'Modifier' : 'Gérer membres' }}
        </button>
      </div>
        ...
        <table v-else class="membres-table">
          <thead>
            <tr>
              ...
              <th>Prénom</th>
              <th>Pseudo</th>
              <th v-if="canAdminEquipes">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="membre in membres" :key="membre.id">
              ...
              <td>
                <span v-if="membre.pseudo" class="pseudo-tag">{{ generatePseudo(membre.last_name, membre.pseudo) }}</span>
                <span v-else>-</span>
              </td>
              <td v-if="canAdminEquipes">
                <button 
    ...
    <div v-if="showCreateModal && isSuperAdmin" class="modal-overlay" @click.self="closeCreateModal">
      <div class="modal">
    ...
    <div v-if="showEditModal" class="modal-overlay" @click.self="closeEditModal">
      ...
      <div v-else-if="editingEquipe" class="modal modal-large">
        <h3>
          {{ isSuperAdmin ? 'Modifier' : 'Gérer' }} l'équipe : {{ editingEquipe.nom }}
          <span class="role-indicator" :class="getUserRoleClass(authUser)">
            {{ getUserRole(authUser) }}
          </span>
        </h3>
        ...
        <div v-if="isSuperAdmin" v-show="editTab === 'infos'" class="tab-content">
        ...
        <div v-show="editTab === 'membres' || !isSuperAdmin" class="tab-content">
...
const getUserRole = (user: User | null) => {
  ...
  console.log('User dans getUserRole:', user)  // DEBUG
  console.log('is_superuser:', user.is_superuser)  // DEBUG
  console.log('is_staff:', user.is_staff)  // DEBUG
...
const isSuperAdmin = computed(() => authUser.value?.is_superuser === true)
const isAdmin = computed(() => authUser.value?.is_staff === true || isSuperAdmin.value)
const canAdminEquipes = computed(() => isAdmin.value)
...
const openEditModal = async (equipe: Equipe) => {
  ...
  try {
    ...
    editTab.value = isSuperAdmin.value ? 'infos' : 'membres'
  } catch (err) {
...
const saveEquipeInfos = async () => {
  if (!editingEquipe.value || !isSuperAdmin.value) return
