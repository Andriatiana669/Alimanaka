<template>
  <div class="dashboard">
      ...
      <div class="member-filter">
        ...
        <div v-if="filterType === 'equipe'" class="equipe-selector">
          <select v-model="selectedEquipeId" @change="onEquipeChange" class="form-select">
            ...
            <option v-for="equipe in equipesGerables" :key="equipe.id" :value="equipe.id">
              {{ getEquipePath(equipe) }}
            </option>
          </select>
        </div>

        <!-- Sélecteur de membre (avec recherche) -->
        <div v-else-if="filterType === 'membre'" class="membre-selector">
          <div class="search-wrapper">
            ...
            <input
              ...
              placeholder="Rechercher un membre (nom, prénom, matricule)..."
              class="search-input"
              @input="onMembreSearch"
            />
          </div>
          
          <select v-model="selectedMembreId" @change="onMembreChange" class="form-select" :disabled="!membresFiltres.length">
    ...
    <!-- (Filtrées selon l'équipe/membre sélectionné) -->
    <!-- ============================================ -->
    <div v-if="isManagerOrHigher" class="dashboard-section">
      ...
      <div v-else class="pending-grid">
        ...
        <div v-if="congesEnAttenteFiltres.length > 0" class="pending-card conges">
          <h3>
            <i class="bi bi-calendar-check"></i>
            Congés ({{ congesEnAttenteFiltres.length }})
          </h3>
        ...
        <div v-if="ostieEnAttenteFiltres.length > 0" class="pending-card ostie">
          <h3>
            <i class="bi bi-heart-pulse"></i>
            OSTIE ({{ ostieEnAttenteFiltres.length }})
          </h3>
          <div class="pending-list">
        ...
        <div v-if="permissionsEnAttenteFiltres.length > 0" class="pending-card permissions">
          <h3>
            <i class="bi bi-door-open"></i>
            Permissions ({{ permissionsEnAttenteFiltres.length }})
          </h3>
          <div class="pending-list">
        ...
        <div v-if="reposEnAttenteFiltres.length > 0" class="pending-card repos">
          <h3>
            <i class="bi bi-hospital"></i>
            Repos médicaux ({{ reposEnAttenteFiltres.length }})
          </h3>
          <div class="pending-list">
        ...
        <div v-if="retardsEnAttenteFiltres.length > 0" class="pending-card retards">
          <h3>
            <i class="bi bi-clock-history"></i>
            Retards ({{ retardsEnAttenteFiltres.length }})
          </h3>
          <div class="pending-list">
      ...
      <div class="module-stats">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-bars">
              <div class="stat-bar">
                ...
                <div class="bar-container">
                  <div class="bar" :style="{ width: congesPourcentage.approuves + '%', background: '#2e7d32' }"></div>
                </div>
                <span class="bar-value">{{ congesStats.approuves }}</span>
              </div>
              <div class="stat-bar">
                <span class="bar-label">En attente</span>
                <div class="bar-container">
                  <div class="bar" :style="{ width: congesPourcentage.enAttente + '%', background: '#f57c00' }"></div>
                </div>
                <span class="bar-value">{{ congesStats.enAttente }}</span>
              </div>
              <div class="stat-bar">
                <span class="bar-label">Refusés</span>
                <div class="bar-container">
                  <div class="bar" :style="{ width: congesPourcentage.refuses + '%', background: '#c62828' }"></div>
                </div>
                <span class="bar-value">{{ congesStats.refuses }}</span>
              </div>
              <div class="stat-bar">
                <span class="bar-label">Annulés</span>
                <div class="bar-container">
                  <div class="bar" :style="{ width: congesPourcentage.annules + '%', background: '#757575' }"></div>
                </div>
                <span class="bar-value">{{ congesStats.annules }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="stat-module">
            ...
            <div class="stat-bars">
              <div class="stat-bar">
                <span class="bar-label">À rattraper</span>
                <div class="bar-container">
                  <div class="bar" :style="{ width: permissionsPourcentage.aRattraper + '%', background: '#f57c00' }"></div>
                </div>
                <span class="bar-value">{{ permissionsStats.heuresARattraper }}h</span>
              </div>
              <div class="stat-bar">
                <span class="bar-label">Rattrapées</span>
                <div class="bar-container">
                  <div class="bar" :style="{ width: permissionsPourcentage.rattrapees + '%', background: '#2e7d32' }"></div>
                </div>
                <span class="bar-value">{{ permissionsStats.heuresRattrapees }}h</span>
              </div>
            </div>
          </div>
          ...
          <div class="stat-module">
            ...
            <div class="stat-bars">
              <div class="stat-bar">
                ...
                <div class="bar-container">
                  <div class="bar" :style="{ width: retardsPourcentage.enCours + '%', background: '#f57c00' }"></div>
                </div>
                <span class="bar-value">{{ retardsStats.heuresEnCours }}h</span>
              </div>
              <div class="stat-bar">
                <span class="bar-label">Rattrapés</span>
                <div class="bar-container">
                  <div class="bar" :style="{ width: retardsPourcentage.rattrapes + '%', background: '#2e7d32' }"></div>
                </div>
                <span class="bar-value">{{ retardsStats.heuresRattrapees }}h</span>
              </div>
            </div>
          </div>
        </div>
...
const equipesGerables = computed(() => {
  ...
  const collectGerables = (equipes: any[]): any[] => {
    let result: any[] = []
    ...
    for (const equipe of equipes) {
      ...
      if (equipe.sous_equipes && equipe.sous_equipes.length > 0) {
        result = [...result, ...collectGerables(equipe.sous_equipes)]
      }
    }
...
const getEquipePath = (equipe: any): string => {
  // Fonction pour obtenir le chemin complet de l'équipe
  let path = equipe.nom
  let current = equipe
  
  // Remonter jusqu'à la racine
  while (current.equipe_parente_details) {
    path = `${current.equipe_parente_details.nom} > ${path}`
    current = current.equipe_parente_details
  }
...
const membresFiltres = computed(() => {
  if (!equipesStore.arbreEquipes) return []
  
  // Collecter tous les membres des équipes gérables
  const collectMembres = async () => {
    const allMembres: any[] = []
    
    for (const equipe of equipesGerables.value) {
      try {
        const membres = await equipesApi.getMembres(equipe.id)
        allMembres.push(...membres.map((m: any) => ({
          ...m,
          equipe_nom: equipe.nom,
          equipe_id: equipe.id
        })))
      } catch (error) {
        console.error(`Erreur chargement membres équipe ${equipe.id}:`, error)
      }
    }
  ...
  if (!query) return membresData.value
  
  return membresData.value.filter((m: any) => 
...
// Stockage des membres (à charger)
const membresData = ref<any[]>([])

// ========== COMPUTED - FILTRE LABEL ==========
const filterLabel = computed(() => {
  if (selectedMembreId.value) {
    const membre = membresData.value.find(m => m.id === selectedMembreId.value)
...
// ========== COMPUTED - STATS AVEC FILTRES ==========
// Congés
const congesSolde = computed(() => {
  if (selectedMembreId.value) {
    // Pour un membre spécifique, son solde
    return '0' // À implémenter avec API
  }
  return authStore.soldeConge?.actuelle || '0'
})

const congesApprouves = computed(() => {
  return congesFiltres.value.filter((c: any) => c.statut === 'approuve').length
})

const congesEnAttente = computed(() => {
  return congesFiltres.value.filter((c: any) => c.statut === 'en_attente').length
})

const congesRefuses = computed(() => {
  return congesFiltres.value.filter((c: any) => c.statut === 'refuse').length
})

const congesAnnules = computed(() => {
  return congesFiltres.value.filter((c: any) => c.statut === 'annule').length
})

const congesStats = computed(() => ({
  approuves: congesApprouves.value,
  enAttente: congesEnAttente.value,
  refuses: congesRefuses.value,
  annules: congesAnnules.value
}))

const congesPourcentage = computed(() => {
  const total = congesFiltres.value.length || 1
  return {
    approuves: (congesApprouves.value / total) * 100,
    enAttente: (congesEnAttente.value / total) * 100,
    refuses: (congesRefuses.value / total) * 100,
    annules: (congesAnnules.value / total) * 100
  }
})
...
const permissionsRattrapage = computed(() => 
  permissionsFiltres.value.filter((p: any) => p.statut === 'rattrapage')
)
const permissionsRetournees = computed(() => 
  permissionsFiltres.value.filter((p: any) => p.statut === 'retourne')
)
const permissionsAnnulees = computed(() => 
  permissionsFiltres.value.filter((p: any) => p.statut === 'annule').length
)

const permissionsStats = computed(() => {
  const heuresARattraper = permissionsFiltres.value.reduce((acc: number, p: any) => {
    return acc + (parseFloat(p.heures_a_rattraper) || 0)
  }, 0)
  
  const heuresRattrapees = permissionsFiltres.value.reduce((acc: number, p: any) => {
    return acc + (parseFloat(p.heures_rattrapees) || 0)
  }, 0)
  
  return { heuresARattraper, heuresRattrapees }
})

const permissionsPourcentage = computed(() => {
  const total = permissionsStats.value.heuresARattraper + permissionsStats.value.heuresRattrapees || 1
  return {
    aRattraper: (permissionsStats.value.heuresARattraper / total) * 100,
    rattrapees: (permissionsStats.value.heuresRattrapees / total) * 100
  }
})
...
const retardsEnCours = computed(() => 
  retardsFiltres.value.filter((r: any) => r.statut === 'en_attente' || r.statut === 'en_cours')
)
const retardsRattrapes = computed(() => 
  retardsFiltres.value.filter((r: any) => r.statut === 'approuve').length
)
const retardsAnnules = computed(() => 
  retardsFiltres.value.filter((r: any) => r.statut === 'annule').length
)

const retardsStats = computed(() => {
  const heuresEnCours = retardsFiltres.value
    .filter((r: any) => r.statut === 'en_attente' || r.statut === 'en_cours')
    .reduce((acc: number, r: any) => acc + (parseFloat(r.heures_a_rattraper) || 0), 0)
  
  const heuresRattrapees = retardsFiltres.value
    .filter((r: any) => r.statut === 'approuve')
    .reduce((acc: number, r: any) => acc + (parseFloat(r.heures_a_rattraper) || 0), 0)
  
  return { heuresEnCours, heuresRattrapees }
})

const retardsPourcentage = computed(() => {
  const total = retardsStats.value.heuresEnCours + retardsStats.value.heuresRattrapees || 1
  return {
    enCours: (retardsStats.value.heuresEnCours / total) * 100,
    rattrapes: (retardsStats.value.heuresRattrapees / total) * 100
  }
})

// Demandes en attente filtrées
const congesEnAttenteFiltres = computed(() => 
  congesFiltres.value.filter((c: any) => c.statut === 'en_attente')
)
const ostieEnAttenteFiltres = computed(() => 
  ostieFiltres.value.filter((o: any) => o.statut === 'en_attente')
)
const permissionsEnAttenteFiltres = computed(() => 
  permissionsFiltres.value.filter((p: any) => p.statut === 'en_attente')
)
const reposEnAttenteFiltres = computed(() => 
  reposFiltres.value.filter((r: any) => r.statut === 'en_attente')
)
const retardsEnAttenteFiltres = computed(() => 
  retardsFiltres.value.filter((r: any) => r.statut === 'en_attente')
)
...
const congesFiltres = computed(() => {
  ...
  if (selectedMembreId.value) {
    ...
    // Filtrer par équipe (et ses sous-équipes)
    const membresEquipe = getMembresEquipe(selectedEquipeId.value)
    data = data.filter((c: any) => membresEquipe.includes(c.utilisateur))
  } else if (isManagerOrHigher.value) {
    // Par défaut, toutes les équipes que je gère
    const membresGerables = getAllGerablesMembres()
    data = data.filter((c: any) => membresGerables.includes(c.utilisateur))
  }
...
const ostieFiltres = computed(() => {
  ...
  if (selectedMembreId.value) {
    ...
    const membresEquipe = getMembresEquipe(selectedEquipeId.value)
    data = data.filter((o: any) => membresEquipe.includes(o.utilisateur))
  } else if (isManagerOrHigher.value) {
    const membresGerables = getAllGerablesMembres()
    data = data.filter((o: any) => membresGerables.includes(o.utilisateur))
  }
...
const permissionsFiltres = computed(() => {
  ...
  if (selectedMembreId.value) {
    ...
    const membresEquipe = getMembresEquipe(selectedEquipeId.value)
    data = data.filter((p: any) => membresEquipe.includes(p.utilisateur))
  } else if (isManagerOrHigher.value) {
    const membresGerables = getAllGerablesMembres()
    data = data.filter((p: any) => membresGerables.includes(p.utilisateur))
  }
...
const reposFiltres = computed(() => {
  ...
  if (selectedMembreId.value) {
    ...
    const membresEquipe = getMembresEquipe(selectedEquipeId.value)
    data = data.filter((r: any) => membresEquipe.includes(r.utilisateur))
  } else if (isManagerOrHigher.value) {
    const membresGerables = getAllGerablesMembres()
    data = data.filter((r: any) => membresGerables.includes(r.utilisateur))
  }
...
// ========== FONCTIONS D'AIDE POUR LES FILTRES ==========
const getMembresEquipe = (equipeId: number): number[] => {
  // À implémenter avec les données réelles des membres
  // Pour l'instant, retourner un tableau vide
  return []
}

const getAllGerablesMembres = (): number[] => {
...
const teamMembersCount = computed(() => {
  ...
  if (selectedEquipeId.value) {
    // Compter les membres de l'équipe sélectionnée
    return getMembresEquipe(selectedEquipeId.value).length
  }
  // Compter tous les membres des équipes gérables
  return getAllGerablesMembres().length
})
...
const absentsAujourdhui = computed(() => {
  ...
  const conges = congesFiltres.value.filter((c: any) => 
    c.statut === 'approuve' && 
    c.date_debut <= today && 
    c.date_fin >= today
  )
  
  const ostie = ostieFiltres.value.filter((o: any) => 
    (o.statut === 'approuve' || o.statut === 'transforme') && 
    o.date === today
  )
  
  const repos = reposFiltres.value.filter((r: any) => 
    r.statut === 'approuve' && 
    r.date === today
  )
  ...
  const userIds = new Set([
    ...conges.map((c: any) => c.utilisateur),
    ...ostie.map((o: any) => o.utilisateur),
    ...repos.map((r: any) => r.utilisateur)
  ])
...
const topRetardataires = computed(() => {
  if (!isManagerOrHigher.value || !retardsFiltres.value) return []
...
// Données d'évolution mensuelle (basées sur les vraies données)
