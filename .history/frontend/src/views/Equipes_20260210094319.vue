  ...
  <div class="equipes-page">
    ...
    <div class="filters-bar">
      ...
      <input
        v-model="searchQuery"
        type="text"
        placeholder="Rechercher un membre (pseudo, matricule, nom, prénom)..."
        class="filter-group"
      />
...
const filtrerEquipesParRecherche = (equipes: Equipe[], query: string): Equipe[] => {
  ...
  return equipes
    .map(eq => {
      ...
      const membresFiltrees = eq.membres?.filter((m: EquipeMembre) =>
      ...
      const sousEquipesFiltrees = eq.sous_equipes ? filtrerEquipesParRecherche(eq.sous_equipes, query) : []
    ...
    .filter((eq): eq is Equipe => eq !== null) // filtre les null et garde le type Equipe
}

// Équipe filtrée finale : filtre par pôle, équipe, puis recherche
