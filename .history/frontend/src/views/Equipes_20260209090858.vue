...
// Équipes filtrées pour le select (dépend du pôle sélectionné)
...
const applyFilters = () => {
  // Si on change de pôle, on réinitialise le filtre équipe
  if (poleFilter.value === '') {
    equipeFilter.value = ''
  }
}
...
const selectEquipe = (equipe: Equipe) => {
  ...
  console.log('Équipe sélectionnée:', equipe.nom)
}

const openEditModal = (equipe: Equipe) => {
  console.log('Éditer équipe:', equipe.nom)
  // À implémenter
}
