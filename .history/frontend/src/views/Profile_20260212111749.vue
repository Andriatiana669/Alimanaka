...
const savePseudo = async () => {
  ...
  try {
    ...
    if (pseudoForm.format === 'custom') {
      // Mode custom : envoyer LES DEUX (pseudo + pseudo_format)
      data = { 
        pseudo: pseudoForm.customValue.trim(),
        pseudo_format: 'custom'
      }
    } else {
      // Mode automatique : envoyer seulement le format
      // Le backend recalculera le pseudo automatiquement
      data = { pseudo_format: pseudoForm.format }
    }
    
    console.log('Envoi données:', data) // Debug
