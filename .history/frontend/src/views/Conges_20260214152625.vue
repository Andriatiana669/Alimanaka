<template>
  ...
  <!-- Modal Détails du Congé (VERSION CORRIGÉE) -->
    ...
    <div class="modal modal-large">
      ...
      <div class="modal-body" v-if="selectedEvent">
        <div class="detail-section">
          <div class="detail-avatar">{{ getInitials(selectedEvent.userName || '') }}</div>
          <div class="detail-user-info">
            <h4>{{ selectedEvent.userName }}</h4>
            <p class="detail-meta">Matricule: {{ selectedEvent.userMatricule }}</p>
            <p class="detail-meta">Email: {{ selectedEvent.userEmail || 'Non renseigné' }}</p>
            <p class="detail-meta">Pseudo: {{ selectedEvent.userPseudo || 'Non défini' }}</p>
          </div>
        </div>
        ...
        <div class="detail-grid">
          <div class="detail-item">
            ...
            <span class="detail-badge" :style="{ backgroundColor: selectedEvent.color }">
              {{ selectedEvent.typeConge }}
            </span>
            <span class="detail-heures" v-if="selectedEvent.heureDebut && selectedEvent.heureFin">
          ...
          <div class="detail-item">
            <label>Période</label>
            <span>{{ formatDate(selectedEvent.start) }} → {{ formatDate(selectedEvent.endDate || selectedEvent.start) }}</span>
          </div>

          <div class="detail-item">
            <label>Jours déduits</label>
            <span class="detail-deduction">{{ selectedEvent.joursDeduits }}j</span>
          </div>

          <div class="detail-item">
            <label>Statut</label>
            <span class="detail-badge" :class="'status-' + selectedEvent.statut">
              {{ getStatutLabel(selectedEvent.statut) }}
            </span>
          </div>

          <div class="detail-item full-width">
            <label>Date de création</label>
            <span>{{ formatDateTime(selectedEvent.dateCreation) }}</span>
          </div>

          <div class="detail-item full-width" v-if="selectedEvent.motif">
            <label>Motif</label>
            <p class="detail-motif">{{ selectedEvent.motif }}</p>
          </div>
        </div>
      </div>
...
import { congesApi } from '@/api/conges'  // AJOUT: Import de l'API
...
const loadingDetails = ref(false)  // AJOUT: état de chargement


// CORRECTION : Gestion du clic sur un événement avec récupération API
const onEventClick = async (event: CalendarEvent) => {
  ...
  try {
    const response = await congesApi.getCongeDetails(congeId)
    const conge = response.data
    
    selectedEvent.value = {
      id: conge.id,
      userName: conge.utilisateur_details?.display_name || conge.utilisateur?.get_display_name,
      userMatricule: conge.utilisateur_details?.username?.toUpperCase() || conge.utilisateur?.username?.toUpperCase(),
      userEmail: conge.utilisateur_details?.email,
      userPseudo: conge.utilisateur_details?.pseudo,
      typeConge: conge.type_conge_display || event.title.split(' - ')[1],
      heureDebut: conge.heure_debut || event.heure_debut,  // Priorité aux données API
      heureFin: conge.heure_fin || event.heure_fin,
      start: conge.date_debut || event.start,
      endDate: conge.date_fin || event.end,
      joursDeduits: conge.jours_deduits || event.jours_deduits,
      statut: conge.statut || event.statut,
      motif: conge.motif || event.motif,
      dateCreation: conge.date_creation || event.date_creation,
      canCancel: conge.can_cancel || (conge.utilisateur?.id === authStore.user?.id && ['en_attente', 'approuve'].includes(conge.statut)),
      color: event.color || getCongeColor(conge.type_conge)
    }
    
    showDetailModal.value = true
  } catch (err) {
    console.error('Erreur chargement détails:', err)
    // Fallback sur les données de l'événement si l'API échoue
    selectedEvent.value = {
      id: congeId,
      userName: event.title.split(' - ')[0],
      userMatricule: 'N/A',
      userEmail: null,
      userPseudo: null,
      typeConge: event.title.split(' - ')[1] || 'Congé',
      heureDebut: event.heure_debut,
      heureFin: event.heure_fin,
      start: event.start,
      endDate: event.end,
      joursDeduits: event.jours_deduits,
      statut: event.statut,
      motif: event.motif,
      dateCreation: event.date_creation,
...
const cancelSelectedConge = async () => {
  ...
  if (confirm('Voulez-vous vraiment annuler ce congé ?')) {
    try {
      await congesStore.annulerConge(selectedEvent.value.id)
      closeDetailModal()
      await refreshData()
    } catch (err) {
...
/* AJOUT: Style pour les heures dans le modal détails */
.detail-heures {
  display: block;
  margin-top: 0.5rem;
  font-size: 0.9rem;
