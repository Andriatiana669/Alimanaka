...
const minutesRetard = computed(() => {
  ...
  // Sécuriser le split avec des valeurs par défaut
  const arriveeParts = form.value.heure_arrivee_reelle.split(':')
  const prevueParts = heureDebutPrevue.split(':')
  
  const heuresArrivee = arriveeParts.length > 0 ? parseInt(arriveeParts[0]) || 0 : 0
  const minutesArrivee = arriveeParts.length > 1 ? parseInt(arriveeParts[1]) || 0 : 0
  
  const heuresPrevue = prevueParts.length > 0 ? parseInt(prevueParts[0]) || 0 : 0
  const minutesPrevue = prevueParts.length > 1 ? parseInt(prevueParts[1]) || 0 : 0
  
  const arrivee = new Date(dateStr)
  arrivee.setHours(heuresArrivee, minutesArrivee, 0)
  
  const prevue = new Date(dateStr)
  prevue.setHours(heuresPrevue, minutesPrevue, 0)
  
  return Math.max(0, differenceInMinutes(arrivee, prevue))
})

// Type de retard selon les minutes
const retardType = computed<TypeRetardConfig | undefined>(() => {
  const minutes = minutesRetard.value
  return retardsStore.typesRetard.find(t => 
...
const retardTypeLabel = computed(() => {
  // CORRECTION: Utiliser le type_retard pour déterminer le label
  const type = retardType.value?.type_retard
  if (type === 'leger') return 'Retard léger'
  if (type === 'moyen') return 'Retard moyen'
  if (type === 'important') return 'Retard important'
  return 'Inconnu'
})
...
const calculateRattrapageHeures = computed(() => {
  ...
  const debutParts = rattrapageForm.value.heure_debut.split(':')
  const finParts = rattrapageForm.value.heure_fin.split(':')
  
  const hD = debutParts.length > 0 ? parseInt(debutParts[0]) || 0 : 0
  const mD = debutParts.length > 1 ? parseInt(debutParts[1]) || 0 : 0
  
  const hF = finParts.length > 0 ? parseInt(finParts[0]) || 0 : 0
  const mF = finParts.length > 1 ? parseInt(finParts[1]) || 0 : 0
  
  const debut = new Date()
  debut.setHours(hD, mD, 0)
...
const filteredEventsForCalendar = computed<CalendarEvent[]>(() => {
  return retardsStore.eventsForCalendar as unknown as CalendarEvent[]
})
