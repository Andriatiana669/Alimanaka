<template>
  <div class="conges-page">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <div v-if="authStore.user" class="solde-badge">
          <span class="solde-dispo">🏖️ {{ authStore.user.solde_conge_actuelle || 0 }}j disponibles</span>
    ...
    <div class="filters-bar">
      <select v-model="yearFilter" class="filter-select" @change="refreshData">
...
import type { CalendarEvent, TypeCongeConfig } from '@/types/conges'
...
const eventsForCalendar = computed((): CalendarEvent[] => {
  return congesStore.eventsForCalendar
})

// Dates bloquées pour le calendrier
const blockedDates = computed((): Date[] => {
...
const deductionPreview = computed(() => {
  ...
  // Calcul simplifié (à améliorer avec la logique exacte des vendredis/jeudis)
  const start = form.value.date_debut ? parseISO(form.value.date_debut) : null
  
  if (start) {
    for (let i = 0; i < days; i++) {
      const current = addDays(start, i)
      const dayOfWeek = current.getDay() // 0=Dim, 1=Lun, ..., 5=Sam, 6=Dim
      
      if (dayOfWeek === 5 && type.vendredi_deduction) { // Vendredi
        deduction += type.vendredi_deduction
      } else if (dayOfWeek === 4 && type.jeudi_deduction) { // Jeudi
        // Vérifier si vendredi suivant est bloqué
...
const isFormValid = computed(() => {
  return form.value.type_conge && 
         ...
         deductionPreview.value <= (authStore.user?.solde_conge_actuelle || 0)
})
...
const onDateChange = () => {
  ...
  if (form.value.date_debut) {
    ...
    // Vérifier si c'est un jour bloqué
    if (congesStore.isDateBlocked(selected)) {
      dateError.value = 'Cette date est indisponible (weekend, férié ou exceptionnel)'
      form.value.date_debut = ''
      return
    }
    
    // Auto-set date_fin si vide
...
const submitForm = async () => {
  ...
  try {
    ...
    // Rafraîchir les données utilisateur pour le solde
    ...
    formError.value = err.response?.data?.error || 'Erreur lors de l\'envoi de la demande'
  } finally {
