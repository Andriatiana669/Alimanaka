        ...
        <div v-if="congesEnAttenteFiltres.length > 0" class="pending-card conges">
          ...
          <div class="pending-list">
            <div v-for="conge in congesEnAttenteFiltres.slice(0, 3)" :key="conge.id" class="pending-item">
              <div class="user-avatar">{{ getInitials(conge.utilisateur_details?.display_name) }} - {{ getInitials(conge.utilisateur_details?.username) }}</div>
...
const getCongesFromCalendar = () => {
  ...
  return congesStore.calendrierEvents
    ...
    .map((event: any) => {
      // Chercher les détails de l'utilisateur dans allGerablesMembres
