    ...
    <div class="global-stats-filters">
      <template v-if="isSuperAdmin">
        <div class="filter-group">
          <label>Pôle :</label>
          <select v-model="globalStatsPoleFilter" class="form-select small">
        ...
        <div class="filter-group">
          <label>Équipe :</label>
          <select v-model="globalStatsEquipeFilter" class="form-select small" :disabled="!globalStatsPoleFilter">
            <option value="">Toutes les équipes</option>
            <option v-for="equipe in equipesFiltreesParPole" :key="equipe.id" :value="equipe.id">
              {{ equipe.nom }}
            </option>
          </select>
        </div>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <select v-model="exportPeriod" @change="updateDateRange" class="form-select">
          ...
          <!-- Année -->
          <div class="form-group" v-if="exportPeriod !== 'personnalise'">
            <label>Année</label>
            <select v-model="exportYear" @change="updateDateRange" class="form-select">
              <option :value="currentYear">{{ currentYear }}</option>
              <option :value="currentYear - 1">{{ currentYear - 1 }}</option>
              <option :value="currentYear - 2">{{ currentYear - 2 }}</option>
            </select>
          ...
          <!-- Trimestre (si période trimestrielle) -->
          <div class="form-group" v-if="exportPeriod === 'trimestriel'">
            <label>Trimestre</label>
            <select v-model="exportQuarter" @change="updateDateRange" class="form-select">
          ...
          <!-- Semestre (si période semestrielle) -->
          <div class="form-group" v-if="exportPeriod === 'semestriel'">
          ...
          <template v-if="exportPeriod === 'personnalise'">
            <div class="form-row">
              <div class="form-group">
          ...
          <div class="preview-box info-box">
            <p><strong>📅 Période sélectionnée :</strong></p>
            <p>{{ formatDate(exportStartDate) }} → {{ formatDate(exportEndDate) }}</p>
          </div>
          ...
          <div class="filters-summary">
            ...
            <div class="filter-tags">
              <span class="filter-tag" v-if="globalStatsPoleFilter">
                <i class="bi bi-house"></i> Pôle: {{ getPoleName(globalStatsPoleFilter) }}
              </span>
              <span class="filter-tag" v-if="globalStatsEquipeFilter">
                <i class="bi bi-people"></i> Équipe: {{ getEquipeName(globalStatsEquipeFilter) }}
              </span>
              <span class="filter-tag" v-if="globalStatsTypeFilter !== 'tous'">
                <i class="bi bi-filter"></i> Type: {{ globalStatsTypeFilter }}
              </span>
              <span class="filter-tag" v-if="globalStatsStatusFilter !== 'tous'">
                <i class="bi bi-tag"></i> Statut: {{ globalStatsStatusFilter }}
              </span>
              <span class="filter-tag" v-if="filterLabel">
                <i class="bi bi-person"></i> {{ filterLabel }}
              </span>
            </div>
          </div>
          ...
          <div class="form-group">
            <label class="checkbox-label">
              ...
              <span>Inclure les graphiques (aperçu visuel)</span>
            </label>
          </div>
        </div>

        <div class="modal-actions">
          <button class="btn-secondary" @click="closeExportModal">Annuler</button>
          <button 
            class="btn-primary" 
            @click="exportAllData"
            :disabled="exporting || !exportStartDate || !exportEndDate"
          >
            <i v-if="exporting" class="bi bi-arrow-repeat spin"></i>
...
// Filtres principaux
const filterType = ref<'moi' | 'equipe' | 'membre'>('moi')
...
// Filtres pour les stats globales
const globalStatsPoleFilter = ref<number | ''>('')
const globalStatsEquipeFilter = ref<number | ''>('')
...
const exportPeriod = ref<'trimestriel' | 'semestriel' | 'annuel' | 'personnalise'>('annuel')
const exportYear = ref<number>(getYear(new Date()))
const exportQuarter = ref<number>(1)
const exportStartDate = ref<string>('')
const exportEndDate = ref<string>('')
const exportFormat = ref<'excel' | 'csv'>('excel')
const exportIncludeCharts = ref<boolean>(false)

// ========== COMPUTED - OPTIONS DE TRIMESTRE ==========
...
const loadAllGerablesMembres = async () => {
  ...
  for (const equipe of equipesGerablesHierarchiques.value) {
    ...
    tousMembres.push(...membres.map((m: any) => ({
      ...
      pole: equipe.pole,
      pole_details: equipe.pole_details
    })))
  }
...
// ========== COMPUTED - ÉQUIPES FILTRÉES PAR PÔLE ==========
const equipesFiltreesParPole = computed(() => {
  if (!globalStatsPoleFilter.value) return []
  return equipesStore.equipesActives?.filter(e => e.pole === globalStatsPoleFilter.value) || []
})
...
const filteredGlobalStats = computed(() => {
  ...
  const filterByYearAndStatusAndTeam = (items: any[], type: string) => {
    return items.filter((item: any) => {
      ...
      // Filtre par équipe/pôle - seulement pour super admin
      if (isSuperAdmin.value) {
        const userId = item.utilisateur
        const membre = allGerablesMembres.value.find((m: any) => m.id === userId)
        
        if (globalStatsPoleFilter.value) {
          if (!membre?.pole || membre.pole !== globalStatsPoleFilter.value) return false
        }
        
        if (globalStatsEquipeFilter.value) {
          if (!membre?.equipe || membre.equipe !== globalStatsEquipeFilter.value) return false
        }
      }
  ...
  if (globalStatsTypeFilter.value === 'tous' || globalStatsTypeFilter.value === 'conges') {
    congesCount = filterByYearAndStatusAndTeam(congesStore.conges || [], 'conges').length
  }
  if (globalStatsTypeFilter.value === 'tous' || globalStatsTypeFilter.value === 'ostie') {
    ostieCount = filterByYearAndStatusAndTeam(ostieStore.osties || [], 'ostie').length
  }
  if (globalStatsTypeFilter.value === 'tous' || globalStatsTypeFilter.value === 'permissions') {
    permissionsCount = filterByYearAndStatusAndTeam(permissionsStore.permissions || [], 'permissions').length
  }
  if (globalStatsTypeFilter.value === 'tous' || globalStatsTypeFilter.value === 'repos') {
    reposCount = filterByYearAndStatusAndTeam(reposStore.reposMedicaux || [], 'repos').length
  }
  if (globalStatsTypeFilter.value === 'tous' || globalStatsTypeFilter.value === 'retards') {
    retardsCount = filterByYearAndStatusAndTeam(retardsStore.retards || [], 'retards').length
  }
...
const filteredEvolutionData = computed(() => {
  ...
  allDemandes.forEach((d: any) => {
    try {
      ...
      // Filtre par équipe/pôle - seulement pour super admin
      if (isSuperAdmin.value) {
        const userId = d.utilisateur
        const membre = allGerablesMembres.value.find((m: any) => m.id === userId)
        
        if (globalStatsPoleFilter.value) {
          if (!membre?.pole || membre.pole !== globalStatsPoleFilter.value) return
        }
        
        if (globalStatsEquipeFilter.value) {
          if (!membre?.equipe || membre.equipe !== globalStatsEquipeFilter.value) return
        }
      }
...
const openExportModal = () => {
  ...
  const now = new Date()
  exportYear.value = getYear(now)
  
  // Par défaut : année en cours
  exportStartDate.value = format(new Date(exportYear.value, 0, 1), 'yyyy-MM-dd')
  exportEndDate.value = format(new Date(exportYear.value, 11, 31), 'yyyy-MM-dd')
...
const updateDateRange = () => {
  if (exportPeriod.value === 'trimestriel') {
    // Calculer les dates du trimestre
    const startMonth = (exportQuarter.value - 1) * 3
    const endMonth = startMonth + 2
    exportStartDate.value = format(new Date(exportYear.value, startMonth, 1), 'yyyy-MM-dd')
    exportEndDate.value = format(new Date(exportYear.value, endMonth + 1, 0), 'yyyy-MM-dd')
  } 
  else if (exportPeriod.value === 'semestriel') {
    // Calculer les dates du semestre
    const startMonth = exportQuarter.value === 1 ? 0 : 6
    const endMonth = startMonth + 5
    exportStartDate.value = format(new Date(exportYear.value, startMonth, 1), 'yyyy-MM-dd')
    exportEndDate.value = format(new Date(exportYear.value, endMonth + 1, 0), 'yyyy-MM-dd')
  }
  else if (exportPeriod.value === 'annuel') {
    // Année complète
    exportStartDate.value = format(new Date(exportYear.value, 0, 1), 'yyyy-MM-dd')
    exportEndDate.value = format(new Date(exportYear.value, 11, 31), 'yyyy-MM-dd')
  }
...
const exportAllData = async () => {
  ...
  try {
    // Collecter toutes les données avec les filtres actuels
    const exportData = {
      periode: {
        debut: exportStartDate.value,
        fin: exportEndDate.value,
        type: exportPeriod.value
      },
      filtres: {
        pole: globalStatsPoleFilter.value || 'tous',
        equipe: globalStatsEquipeFilter.value || 'tous',
        type: globalStatsTypeFilter.value,
        statut: globalStatsStatusFilter.value
      },
      statistiques: {
        conges: filteredGlobalStats.value,
        evolution: filteredEvolutionData.value,
        kpi: {
          conges: {
            solde: congesSolde.value,
          ...
          ostie: {
            total: ostieTotal.value,
            enAttente: ostieEnAttente.value,
            transformes: ostieTransformes.value,
            annules: ostieAnnules.value
          },
          permissions: {
            heures: permissionsHeures.value,
            rattrapage: permissionsRattrapage.value.length,
            retournees: permissionsRetournees.value.length,
            annulees: permissionsAnnulees.value
          },
          repos: {
            heures: reposHeures.value,
            enAttente: reposEnAttente.value.length,
            approuves: reposApprouves.value,
            annules: reposAnnules.value
          },
          retards: {
            heures: retardsHeures.value,
            enCours: retardsEnCours.value.length,
            rattrapes: retardsRattrapes.value,
            annules: retardsAnnules.value
          }
        },
        topRetardataires: topRetardataires.value,
        membres: allGerablesMembres.value.map(m => ({
          id: m.id,
          nom: m.display_name,
          matricule: m.username,
          equipe: m.equipe_nom,
          pole: m.pole_details?.nom || 'N/A',
          soldeConge: m.solde_conge_actuelle
        }))
      },
      demandes: {
        enAttente: {
          conges: congesFiltresParStatut.value.filter((d: any) => d.statut === 'en_attente'),
          ostie: ostieFiltresParStatut.value.filter((d: any) => d.statut === 'en_attente'),
          permissions: permissionsFiltresParStatut.value.filter((d: any) => d.statut === 'en_attente'),
          repos: reposFiltresParStatut.value.filter((d: any) => d.statut === 'en_attente'),
          retards: retardsFiltresParStatut.value.filter((d: any) => d.statut === 'en_attente')
        },
        recentes: mesDemandesFiltrees.value
      }
    }
...
const exportToExcel = async (data: any) => {
  ...
  // Dans la pratique, tu utiliserais une bibliothèque comme xlsx ou exceljs
  const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' })
  downloadFile(blob, `export_alimanaka_${exportStartDate.value}_${exportEndDate.value}.json`)
}
...
const exportToCSV = async (data: any) => {
  ...
  // En-têtes
  csv += 'Période,Type,Statistiques\n'
  csv += `${exportStartDate.value} - ${exportEndDate.value},${exportPeriod.value},Export Alimanaka\n\n`

  // KPIs
  csv += 'Catégorie,Indicateur,Valeur\n'
  Object.entries(data.statistiques.kpi).forEach(([categorie, valeurs]: [string, any]) => {
    Object.entries(valeurs).forEach(([indicateur, valeur]) => {
      csv += `${categorie},${indicateur},${valeur}\n`
    })
  })

  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
  downloadFile(blob, `export_alimanaka_${exportStartDate.value}_${exportEndDate.value}.csv`)
}
...
const getPoleName = (poleId: number | ''): string => {
  if (!poleId) return 'Tous'
  const pole = polesStore.polesActifs?.find(p => p.id === poleId)
  return pole?.nom || 'Inconnu'
}

const getEquipeName = (equipeId: number | ''): string => {
  if (!equipeId) return 'Toutes'
  const equipe = equipesStore.equipesActives?.find(e => e.id === equipeId)
  return equipe?.nom || 'Inconnue'
}
...
// ========== WATCH ==========
watch(globalStatsPoleFilter, () => {
  globalStatsEquipeFilter.value = ''
})
...
onMounted(async () => {
  ...
  await Promise.all([
    polesStore.fetchPoles(),           // Charge les pôles
    equipesStore.fetchEquipes(),       // Charge les équipes (liste plate)
    filtersStore.fetchPoles()          // Garde pour les autres composants si nécessaire
  ])
  await equipesStore.fetchArbre(true)  // Charge l'arbre hiérarchique
