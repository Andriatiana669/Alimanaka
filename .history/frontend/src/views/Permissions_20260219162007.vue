      ...
      <div class="modal modal-large">
        ...
        <div class="modal-body" v-if="selectedPermission">
          ...
          <div v-if="selectedPermission.statut === 'approuve' && selectedPermission.valide_par_details" 
            ...
            <h5>✓ Approuvé par</h5>
            <p><strong>{{ selectedPermission.valide_par_details.display_name }}</strong></p>
        ...
        <div class="modal-actions">
          ...
          <!-- Bouton ajouter rattrapage (TOUJOURS possible après retour) -->
          <button 
          ...
          <!-- Bouton transformer en congé (UNIQUEMENT si dépassement > 2h) -->
          <button 
    ...
    <div v-if="showRetourModal" class="modal-overlay" @click.self="closeRetourModal">
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <!-- Aperçu du dépassement - MODIFIÉ -->
          <div v-if="retourForm.heure_arrivee_reelle && minutesDepassement > 120" class="preview-box warning-box">
            <p><strong>⚠️ DÉPASSEMENT CRITIQUE: {{ minutesDepassement }} minutes</strong></p>
            <p>Vous pourrez transformer cette permission en congé.</p>
          </div>
          <div v-else-if="retourForm.heure_arrivee_reelle && minutesDepassement > 0" class="preview-box warning-box">
            <p><strong>⏰ {{ minutesDepassement }} minutes</strong> de dépassement</p>
            <p>Heures à rattraper: <strong>{{ heuresARattraperRetour }}h</strong></p>
            <p class="help-text">(Dépassement de moins de 2h - rattrapage possible)</p>
          </div>
          <div v-else-if="retourForm.heure_arrivee_reelle && minutesDepassement <= 0" class="preview-box success-box">
            <p>✅ Retour dans les temps ! Aucun dépassement.</p>
            <p>Vous pouvez commencer un rattrapage si vous le souhaitez.</p>
          </div>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="info-permission warning-box">
            ...
            <p>Cette permission sera transformée en congé et déduite du solde.</p>
          </div>
...
const minutesDepassement = computed(() => {
  ...
  return Math.max(0, differenceInMinutes(arriveeDate, maxDate))
})

const heuresARattraperRetour = computed(() => {
  return (minutesDepassement.value / 60).toFixed(2)
})
...
const submitRetour = async () => {
  ...
  try {
    ...
    // Message personnalisé selon le dépassement
    const minutes = minutesDepassement.value
    if (minutes > 120) {
      alert(`Retour enregistré avec ${minutes} minutes de dépassement critique. Vous pouvez maintenant transformer cette permission en congé.`)
    } else if (minutes > 0) {
      alert(`Retour enregistré avec ${minutes} minutes de dépassement. Vous pouvez commencer un rattrapage.`)
    } else {
      alert('Retour enregistré dans les temps. Vous pouvez commencer un rattrapage si vous le souhaitez.')
    }
...
// MODIFIÉ : Transformation possible uniquement si dépassement > 120 minutes (2h)
const peutTransformerEnConge = (permission: Permission | null): boolean => {
  if (!permission) return false
  
  // Transformation possible seulement si :
  // 1. Statut est 'retourne'
  // 2. Heure d'arrivée réelle existe
  // 3. DÉPASSEMENT > 120 minutes (2h)
  return permission.statut === 'retourne' && 
         permission.heure_arrivee_reelle !== null &&
         permission.minutes_depassement > 120
}
