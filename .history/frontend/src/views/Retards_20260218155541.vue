...
// Dans Retards.vue - importer le store des congés
...
const blockedDates = computed(() => {
  // Récupérer TOUS les événements bloqués du calendrier des congés
  // SAUF les weekends (car on veut les rayer uniquement s'ils sont fériés/exceptionnels)
  return congesStore.calendrierEvents
    .filter(e => e.isBlocked && e.type !== 'weekend')  // ← MÊME LOGIQUE QUE DANS CONGES
    .flatMap(e => {
      const dates: Date[] = []
