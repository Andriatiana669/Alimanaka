...
import { format, parseISO, isWithinInterval, subDays, subWeeks, subMonths, differenceInDays } from 'date-fns'
import { fr } from 'date-fns/locale/fr'

// Importer les modals (à créer séparément ou réutiliser les existants)
...
import RetourModal from '@/components/permissions/RetourModal.vue'
import RattrapageModal from '@/components/common/RattrapageModal.vue'
import ValidationOstieModal from '@/components/ostie/ValidationOstieModal.vue'
import TransformationModal from '@/components/common/TransformationModal.vue'
import TransformationOstieModal from '@/components/ostie/TransformationOstieModal.vue'
...
// Filtres
const typeFilters = ref<string[]>([])  // Vide = tous
const statusFilters = ref<string[]>([]) // Vide = tous
...
const filteredUsers = computed(() => {
  if (!userSearchQuery.value) return filtersStore.utilisateurs
  
  const query = userSearchQuery.value.toLowerCase()
  return filtersStore.utilisateurs.filter(u => 
...
const filteredEvents = computed(() => {
  ...
  if (statusFilters.value.length > 0) {
    events = events.filter(e => statusFilters.value.includes(e.statut))
  }

  // Filtre par utilisateur
  if (selectedUserIds.value.length > 0) {
    events = events.filter(e => selectedUserIds.value.includes(e.user_id))
  }
...
const getStatusLabel = (statut: string, type?: string): string => {
...
const isUrgent = (event: any): boolean => {
  if (event.type !== 'conge') return false
...
const toggleUserFilter = (user: any) => {
...
const getUserDisplayName = (userId: number): string => {
  // Chercher dans tous les stores
  const allUsers = [
    ...congesStore.utilisateursGerables,
    ...retardsStore.utilisateursGerables,
    ...permissionsStore.utilisateursGerables,
    ...reposStore.utilisateursGerables,
    ...ostieStore.utilisateursGerables
  ]
...
const refreshAll = async () => {
  await Promise.all([
    ...
    filtersStore.fetchUtilisateurs(),
  ...
  switch (action) {
    ...
    case 'rattrapage':
      return (event.type === 'permission' || event.type === 'retard') &&
             ...
             event.heures_restantes > 0
    case 'transformer':
      return (event.type === 'permission' && event.statut === 'retourne' && event.minutes_depassement > 0) ||
...
const quickValidate = (event: any) => {
  ...
  } else if (event.type === 'repos_medical') {
    // Validation directe
    reposStore.validerReposMedical(event.id).then(() => refreshAll())
  }
}
...
const quickCancel = async (event: any) => {
  ...
  try {
    switch (event.type) {
      case 'conge':
        await congesStore.annulerConge(event.id)
        break
      case 'retard':
        await retardsStore.annulerRetard(event.id)
        break
      case 'permission':
        await permissionsStore.annulerPermission(event.id)
        break
      case 'repos_medical':
        await reposStore.annulerReposMedical(event.id)
        break
      case 'ostie':
        await ostieStore.annulerOstie(event.id)
        break
    }
    await refreshAll()
  } catch (error) {
...
onMounted(async () => {
  ...
  await filtersStore.fetchUtilisateurs()
