...
const retardsFiltres = computed(() => {
  // Convertir les événements du calendrier en objets retard
  const retardsFromCalendar = retardsStore.calendrierEvents
...
const refreshAllData = async () => {
  ...
  await Promise.allSettled([
    ...
    // Retards - utiliser le calendrier (avec mois en cours)
    retardsStore.fetchCalendrier({ annee: currentYear, mois: currentMonth }),
  ...
  console.log('Congés (mes):', congesStore.conges?.length || 0)
