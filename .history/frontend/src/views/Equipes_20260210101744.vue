...
const filtrerEquipesParRecherche = (equipes: Equipe[], query: string): Equipe[] => {
  ...
  return equipes
    .map(eq => {
      ...
      // Sous-équipes filtrées récursivement
      const sousEquipesFiltrees: Equipe[] = eq.sous_equipes ? filtrerEquipesParRecherche(eq.sous_equipes, query) : []

      // Vérifie si l'équipe elle-même correspond (nom) ou si elle contient des membres ou sous-équipes correspondants
...
const createForm = ref({ nom: '', description: '', pole: null as number | null, equipe_parente: null as number | null })
