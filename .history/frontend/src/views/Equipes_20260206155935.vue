<template>
  <div class="equipes-page">
    ...
    <div v-if="showEditModal" class="modal-overlay" @click.self="closeEditModal">
      <!-- Cas 1: Données OK -->
      <div v-if="editingEquipe" class="modal modal-large">
      ...
      <div v-else class="modal error-modal">
        ...
        <div class="modal-actions">
          ...
          <button class="btn-primary" @click="reloadPage">Recharger la page</button>
        </div>
      </div>
    </div>
  </div>
...
const openEditModal = async (equipe: Equipe) => {
  console.log('=== openEditModal appelé ===', equipe)
  
  // 1. D'abord charger les données
  loading.value = true
  
  try {
    // Charger l'équipe complète si nécessaire
    if (!equipe.pole_details || !equipe.manager_details) {
      console.log('Chargement détails équipe...')
      const fullEquipe = await equipesStore.fetchEquipeById(equipe.id)
      if (fullEquipe) {
        equipe = fullEquipe
      }
    }
    ...
    // Charger les users si pas déjà fait
    if (usersStore.users.length === 0) {
      console.log('Chargement users...')
      await usersStore.fetchUsers()
    }
    
    // 2. ENSUITE définir les données et ouvrir le modal
    ...
    console.log('Modal ouvert, editingEquipe:', editingEquipe.value)
