<template>
  ...
  <!-- ============================================ -->
  <!-- APERÇU DE L'ÉQUIPE (AVEC VRAIS MEMBRES)     -->
  <!-- ============================================ -->
  <div v-if="isManagerOrHigher" class="dashboard-section">
    <h2>
...
const getEquipePath = (equipe: any): string => {
  // Trouver l'équipe parente
  if (equipe.equipe_parente_details) {
    return `${equipe.equipe_parente_details.nom} > ${equipe.nom}`
  }
  if (equipe.pole_details) {
    return `${equipe.pole_details.nom} - ${equipe.nom}`
  }
  return equipe.nom
}
...
const congesSolde = computed(() => {
  if (selectedMembreId.value) {
    // Pour un membre spécifique, chercher dans les données
    const membre = allGerablesMembres.value.find(m => m.id === selectedMembreId.value)
...
onMounted(async () => {
  ...
  if (congesStore.typesConge.length === 0) {
    await congesStore.fetchTypesConge()
  }
  if (congesStore.droits.length === 0) {
