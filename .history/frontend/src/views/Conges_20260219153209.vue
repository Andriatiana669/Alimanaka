<template>
  ...
  <div class="page-header">
    <div class="header-left">
      ...
      <div v-if="authStore.user" class="solde-badge">
        <span class="solde-dispo">{{ authStore.soldeConge.actuelle }}j disponibles</span>
    ...
    <div class="modal modal-large">
      ...
      <div class="modal-actions">
        ...
        <template v-if="canManageThisConge(selectedConge) && selectedConge?.statut === 'en_attente'">
          ...
          <button 
            v-if="isMultiDay(selectedConge)"
            class="btn-success" 
            @click="openPartialValidation"
          >
            <i class="bi bi-check-lg"></i> Valider Partiel
          </button>

        </template>
        
        <!-- Bouton annuler (pour le propriétaire ou manager) -->
        <template v-if="canCancelConge(selectedConge) && selectedConge?.statut === 'en_attente' || congesStore.isManagerOrAdmin || congesStore.isSuperAdmin">
          <button class="btn-warning" @click="cancelSelectedConge">
            <i class="bi bi-x-circle"></i> Annuler tout
          </button>
          
          <!-- NOUVEAU : Annulation partielle -->
          <button 
            v-if="selectedConge?.statut === 'approuve' && isMultiDay(selectedConge)"
            class="btn-warning" 
            @click="openPartialCancel"
          >
            <i class="bi bi-dash-circle"></i> Annuler jour par jour
          </button>
  ...
  <AnnulationPartielleModal
    v-if="showAnnulationModal && selectedConge"
    :conge="selectedConge"
    :types-conge="typesConge"
    :solde-disponible="Number(authStore.soldeConge.actuelle)"
    @close="showAnnulationModal = false"
    @annuler="handlePartialCancel"
  />
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          <label class="checkbox-label">
            <input 
              type="checkbox" 
              v-model="form.est_droit"
              @change="onDroitToggle"
            />
            <span>Ceci est un droit (congé exceptionnel)</span>
          </label>
          <p class="help-text" v-if="form.est_droit">
            ⚠️ Les droits ne déduisent pas du solde de congés
          </p>
        </div>

        <!-- Liste des droits (visible si checkbox cochée) -->
        <div v-if="form.est_droit" class="form-group">
          <label>Type de droit <span class="required">*</span></label>
          <div class="droits-options">
            <label 
              v-for="droit in droits" 
              :key="droit.id"
              class="droit-option"
              :class="{ selected: form.droit === droit.id }"
            >
              <input 
                type="radio" 
                v-model="form.droit" 
                :value="droit.id"
              />
              <span class="droit-label">{{ droit.nom }}</span>
              <span v-if="droit.description" class="droit-description">
                {{ droit.description }}
              </span>
            </label>
          </div>
...
import AnnulationPartielleModal from '@/components/conges/AnnulationPartielleModal.vue'
import type { Droit } from '@/types/conges'


// Filtrage
const filtersStore = useFiltersStore()

// Validation partiel et Totale
...
const form = ref({
  ...
  motif: '',
  est_droit: false,
  droit: null as number | null
})


const onDroitToggle = () => {
  if (form.value.est_droit) {
    // Charger les droits si nécessaire
    if (droits.value.length === 0) {
      congesStore.fetchDroits().then(() => {
        droits.value = congesStore.droits
      })
    }
    // Optionnel : reset du motif quand on active un droit
    form.value.motif = ''
  } else {
    form.value.droit = null
  }
}
...
const filteredEventsForCalendar = computed<CalendarEvent[]>(() => {
  ...
  return allEvents.filter(event => {
    ...
    // Maintenant event.statut est disponible !
...
const isFormValid = computed(() => {
  ...
  // NOUVEAU : Si c'est un droit, doit avoir un droit sélectionné
  if (form.value.est_droit) {
    return baseValid && form.value.droit !== null
  }
...
const isMultiDay = (conge: any) => {
  if (!conge) return false
  const start = parseISO(conge.date_debut)
  const end = parseISO(conge.date_fin)
...
const openPartialCancel = () => {
  if (!selectedConge.value) return
  if (selectedConge.value.statut !== 'approuve') {
    alert('Seuls les congés approuvés peuvent être annulés partiellement')
    return
  }
  showAnnulationModal.value = true
}

const handlePartialCancel = async (dates: string[]) => {
  try {
    await congesStore.annulerCongePartiel(selectedConge.value.id, dates)
    alert(`${dates.length} jour(s) annulé(s) avec succès - Solde remboursé`)
    showAnnulationModal.value = false
    closeDetailModal()
    await refreshData()
  } catch (err: any) {
    alert(err.message || 'Erreur lors de l\'annulation partielle')
  }
}
...
const openRequestModal = async () => {
  ...
  // CORRECTION : Inclure tous les champs, y compris les nouveaux
  form.value = {
    type_conge: 'journee',
    date_debut: '',
    date_fin: '',
...
const submitForm = async () => {
  ...
  try {
    const payload: any = {
      ...
      // NOUVEAU : Ajouter les champs de droit
      est_droit: form.value.est_droit
    }
    
    // Si c'est un droit, ajouter l'ID du droit
    if (form.value.est_droit && form.value.droit) {
      payload.droit = form.value.droit
      
      // Optionnel : si pas de motif, utiliser le nom du droit comme motif
      const selectedDroit = droits.value.find(d => d.id === form.value.droit)
      if (selectedDroit && !payload.motif) {
        payload.motif = `[DROIT] ${selectedDroit.nom}`
      }
    }
...
onMounted(async () => {
  ...
  await filtersStore.fetchPoles()  // Charge les pôles au démarrage
  await refreshData()
...
.filter-tabs button.active:hover:not(.active) {
  background: #f8f9fa;
  border-color: #3498db;
  color: #065f46;
}
...
/* Conteneur des options de droits */
.droits-options {
  display: flex;
  flex-direction: column;
  gap: 0.75rem;
...
/* Style pour chaque option de droit */
.droit-option {
  display: flex;
...
.droit-option input[type="radio"]:checked {
  accent-color: #3498db;
}
...
/* Animation pour l'apparition des droits */
.droits-options {
...
/* Style pour l'état désactivé (optionnel) */
.droit-option.disabled {
  opacity: 0.6;
  cursor: not-allowed;
  background: #f8f9fa;
}

.droit-option.disabled:hover {
  border-color: #dee2e6;
  transform: none;
  box-shadow: none;
}

/* Indicateur de droit dans le calendrier  */
.calendar-event.droit {
  background: linear-gradient(135deg, #9b59b6 0%, #8e44ad 100%);
  border-left: 4px solid #f1c40f;
}

.calendar-event.droit::before {
  content: "⚖️";
  margin-right: 4px;
  font-size: 0.9em;
}
