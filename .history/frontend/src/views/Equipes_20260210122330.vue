...
// ========== UTILITAIRES ==========
// CORRECTION ICI : Ajout de getRoleClass et correction de la fonction de permission
...
const loading = computed(() => equipesStore.loading || polesStore.loading)
...
// CORRECTION ICI : Utilisation de isAdmin directement (ou tu peux créer canToggleEquipeStatus dans useRoles)
const toggleStatus = async (equipe: Equipe) => {
  // CORRECTION : Utiliser isAdmin qui vérifie is_staff OU is_superuser
  if (!currentUser.value || !isAdmin(currentUser.value)) {
    alert('Permission refusée : Administrateur requis (is_staff ou is_superuser)')
    return
  }
