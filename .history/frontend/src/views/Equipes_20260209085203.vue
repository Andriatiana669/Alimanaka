...
const fetchCurrentUser = async () => {
  ...
  try {
    // Test 1 : Vérifiez l'ID de l'utilisateur
    const user = await usersApi.getUserProfile()
    console.log('ID utilisateur:', user.id)
    console.log('Username:', user.username)
    
    // Test 2 : Demandez explicitement les informations d'admin
    console.log('is_superuser type:', typeof user.is_superuser)
    console.log('is_superuser value:', user.is_superuser)
    
    // Test 3 : Vérifiez si c'est le même utilisateur
    const allUsers = await usersApi.getAllUsers()
    const currentUserInList = allUsers.find(u => u.id === user.id)
    console.log('Dans la liste complète:', currentUserInList)
