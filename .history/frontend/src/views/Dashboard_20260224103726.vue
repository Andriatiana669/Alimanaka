  ...
  <div class="dashboard">
        ...
        <div v-if="filterType === 'equipe'" class="equipe-selector">
          <select v-model="selectedEquipeId" @change="onEquipeChange" class="form-select">
            ...
            <template v-for="equipe in equipesGerablesHierarchiques" :key="equipe.id">
              ...
              <!-- Toutes les sous-équipes récursivement -->
          ...
          <div class="help-text" v-if="equipesGerablesHierarchiques.length === 0">
    ...
    <div v-if="isManagerOrHigher" class="dashboard-section">
      ...
      <div v-if="topRetardataires.length > 0" class="top-list">
        <h3>⏰ Top des retardataires de l'équipe ({{ topRetardataires.length }})</h3>
        <div class="top-items">
      ...
      <div v-else-if="retardsFiltres.length > 0" class="top-list">
        <p class="no-data-top">
          Aucun retard avec heures à rattraper ({{ retardsFiltres.length }} retards trouvés)
        </p>
      </div>
      <div v-else class="top-list">
        <p class="no-data-top">Aucun retard dans l'équipe</p>
      </div>
    </div>
...
const getEquipeDisplay = (equipe: any): string => {
  ...
  if (equipe.pole_details) {
    display = `[${equipe.pole_details.nom}] ${display}`
  }
  
  // Ajouter un indicateur si l'équipe est inactive
  if (!equipe.est_actif) {
    display += ' (inactive)'
  }
...
const loadAllGerablesMembres = async () => {
  ...
  for (const equipe of equipesGerablesHierarchiques.value) {
    ...
    // Construire le chemin d'affichage pour l'équipe
...
console.log('Tous les retards:', retardsStore.retards)
console.log('Membres de l\'équipe:', allGerablesMembres.value)

// ========== TOP RETARDATAIRES ==========
const topRetardataires = computed(() => {
  console.log('=== DEBUG TOP RETARDATAIRES ===')
  console.log('isManagerOrHigher:', isManagerOrHigher.value)
  console.log('retardsFiltres length:', retardsFiltres.value.length)
  console.log('retardsFiltres:', retardsFiltres.value)
  
  if (!isManagerOrHigher.value || retardsFiltres.value.length === 0) {
    return []
  }
  
  // Dictionnaire pour accumuler les heures par utilisateur
  const stats: Record<number, { name: string; total: number }> = {}
  
  // Parcourir tous les retards filtrés
  retardsFiltres.value.forEach((retard: any) => {
    // Ne garder que les retards avec heures à rattraper
    if (retard.statut === 'approuve' || retard.statut === 'en_cours') {
      ...
      console.log(`Retard trouvé - User: ${userId}, Heures: ${heures}, Statut: ${retard.statut}`)
      ...
      if (heures > 0) {
        if (!stats[userId]) {
          ...
          // Chercher dans la liste des membres si pas trouvé
          if (!userName) {
  ...
  console.log('Stats avant tri:', stats)
  
  // Convertir en tableau, trier et prendre top 5
  ...
  console.log('Top 5 résultat:', result)
...
const refreshAllData = async () => {
  ...
  console.log('Membres IDs:', membresIds)
  
  await Promise.allSettled([
    // Congés - pour toute l'équipe
    congesStore.fetchCalendrier({ 
      annee: currentYear,
      utilisateurs: membresIds  // ← IMPORTANT: Ajouter ce paramètre si l'API le supporte
    }),
    congesStore.fetchMesConges(currentYear), // Garder pour les données personnelles
    
    // OSTIE - pour toute l'équipe
    ostieStore.fetchCalendrier({ 
      annee: currentYear,
      utilisateurs: membresIds 
    }),
    ostieStore.fetchMesOsties(currentYear),
    
    // Permissions - pour toute l'équipe
    permissionsStore.fetchCalendrier({ 
      annee: currentYear,
      utilisateurs: membresIds 
    }),
    permissionsStore.fetchMesPermissions(currentYear),
    
    // Repos médicaux - pour toute l'équipe
    reposStore.fetchCalendrier({ 
      annee: currentYear,
      utilisateurs: membresIds 
    }),
    reposStore.fetchMesRepos(currentYear),
    
    // Retards - pour toute l'équipe
    retardsStore.fetchCalendrier({ 
      annee: currentYear,
      mois: currentMonth,
      utilisateurs: membresIds 
    }),
  ...
  console.log('Congés:', congesStore.conges?.length || 0)
  console.log('OSTIE:', ostieStore.osties?.length || 0)
  console.log('Permissions:', permissionsStore.permissions?.length || 0)
  console.log('Repos:', reposStore.reposMedicaux?.length || 0)
  console.log('Retards:', retardsStore.retards?.length || 0)
...
onMounted(async () => {
  ...
  await equipesStore.fetchArbre(true) // Charger l'arbre hiérarchique au démarrage
