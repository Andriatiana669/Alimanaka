<template>
  ...
  <!-- NOUVEAU CALENDRIER AUTONOME (simplifié pour retards) -->
  <Calendar
    :events="filteredEventsForCalendar"
    :jours-feries="joursFeries"
    :annee="selectedYear"
...
import { format, parseISO, getYear, differenceInMinutes } from 'date-fns'
...
const openCreateModal = async () => {
  ...
  try {
    if (typesRetard.value.length === 0) {
      await retardsStore.fetchTypesRetard()
      typesRetard.value = retardsStore.typesRetard
    }
...
// NOUVEAU: Charger les jours fériés (optionnel pour retards)
const loadJoursFeries = async () => {
  try {
    // Optionnel: charger depuis l'API des congés si disponible
    // Ou créer un endpoint dédié dans retards
    const response = await retardsApi.getTypesRetard()
    // Pour l'instant, on laisse vide ou on charge depuis une source partagée
    joursFeries.value = []
  } catch (error) {
