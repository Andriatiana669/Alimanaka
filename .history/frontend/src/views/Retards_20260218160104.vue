...
// Remplacer blockedDates par ceci
const blockedDates = computed(() => {
  // 1. Récupérer les événements du calendrier des congés
  const congesEvents = congesStore.calendrierEvents
  
  // 2. Filtrer pour avoir les jours bloqués (sauf weekends)
  // Exactement la même logique que dans Conges.vue
  return congesEvents
...
const refreshData = async () => {
  ...
  await Promise.all([
    ...
    // Charger les données des congés pour avoir les jours fériés/exceptionnels
    congesStore.fetchCalendrier({ annee: currentYear }),  // ← AJOUTER CECI
    congesStore.fetchJoursFeries(),                       // ← OPTIONNEL
    congesStore.fetchJoursExceptionnels(currentYear)      // ← OPTIONNEL
  ])
