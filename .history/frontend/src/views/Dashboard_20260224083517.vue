<template>
  <div class="dashboard">
    ...
    <div v-if="isManagerOrHigher" class="filters-section">
      ...
      <!-- Badge actif -->
      <div v-if="selectedUser || selectedEquipe" class="active-filter-badge">
...
const teamMembersCount = computed(() => {
  if (!selectedEquipeObj.value) return equipesStore.equipesActives?.length || 0
...
const getMoisPourcentage = (index: number) => {
  const valeur = evolutionMensuelle.value[index]
