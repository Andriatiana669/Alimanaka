...
const filtrerArbrePourUtilisateurNormal = (equipes: Equipe[]): Equipe[] => {
  ...
  const filtrerRecursif = (node: Equipe): Equipe | null => {
    ...
    // Garder le nœud si l'utilisateur est membre ou si des sous-équipes le contiennent
    if (estMembre || enfantsFiltres.length > 0) {
      return {
        ...node,
        sous_equipes: enfantsFiltres
      }
...
const arbreEquipesFiltrees = computed(() => {
  ...
  // 0. Pour utilisateur normal : ne montrer que ses équipes
