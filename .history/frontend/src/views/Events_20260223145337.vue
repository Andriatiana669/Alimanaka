...
const allUsers = ref<FilterUser[]>([]) // ✅ Remplacer filtersStore.utilisateurs
...
const refreshAll = async () => {
  ...
  // Charger les utilisateurs depuis les stores existants
  await loadAllUsers()
...
const canManageThisEvent = (event: CalendarEvent): boolean => {
  if (!canManageOthers.value) return false
  return true // À affiner selon les règles métier
}

const canQuickAction = (event: CalendarEvent, action: string): boolean => {
...
// Actions rapides
const quickValidate = (event: CalendarEvent) => {
  selectedEvent.value = event
  if (event.type === 'conge') {
...
const quickRetour = (event: CalendarEvent) => {
  selectedEvent.value = event
  showRetourModal.value = true
}

const quickRattrapage = (event: CalendarEvent) => {
  selectedEvent.value = event
  showRattrapageModal.value = true
}

const quickTransform = (event: CalendarEvent) => {
  selectedEvent.value = event
  if (event.type === 'ostie') {
...
const quickCancel = async (event: CalendarEvent) => {
  if (!event.id) return
