<template>
  <div class="equipes-page">
    ...
    <div v-if="True" class="modal-overlay" @click.self="closeCreateModal">
      <div class="modal">
...
const openDetailsModal = async () => {
  if (!selectedEquipe.value) return
  showDetailsModal.value = true
  loadingMembres.value = true
  try {
    await equipesStore.fetchMembres(selectedEquipe.value.id)
  } finally {
...
// Membres de l'équipe en cours d'édition
const membresEditing = computed(() => equipesStore.membresCurrentEquipe)

// Utilisateurs du même pôle qui ne sont pas déjà dans cette équipe
...
const openEditModal = async (equipe: Equipe) => {
  ...
  // Charger les membres pour l'onglet membres
