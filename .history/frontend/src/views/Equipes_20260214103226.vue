...
const isAdminAndManager = computed(() => {
  if (!currentUser.value) return false
  return (currentUser.value.is_staff === true && 
          (currentUser.value.est_chef_equipe === true || 
           currentUser.value.est_co_manager === true))
})

const isAdminAndCoManager = computed(() => {
  if (!currentUser.value) return false
  return (currentUser.value.is_staff === true && 
          currentUser.value.est_co_manager === true && 
          currentUser.value.est_chef_equipe !== true)
...
const canToggleInactives = computed(() => {
  return isSuperAdmin.value || (isAdmin.value && currentUser.value?.est_chef_equipe === true)
})
...
const filtrerArbrePourUtilisateurNormal = (equipes: Equipe[], userId: number): Equipe[] => {
  const filtrerRecursif = (node: Equipe): Equipe | null => {
    ...
    const estMembre = node.membres_ids?.includes(userId) || false
