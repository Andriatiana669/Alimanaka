...
const blockedDates = computed(() => {
  return congesStore.calendrierEvents
    .filter(e => e.isBlocked && e.type !== 'weekend') // ← EXCLURE les weekends
    .flatMap(e => {
      const dates: Date[] = []
...
const calculateDays = computed(() => {
  ...
  while (current <= end) {
    // Skip weekends et jours bloqués (mais pas les weekends dans blockedDates maintenant)
...
// ← NOUVEAU: Vérifier si congé urgent (moins d'1 semaine)
const isUrgentRequest = computed(() => {
  ...
  return daysDiff < 7 // Moins d'1 semaine = urgent
})
...
const onDateChange = () => {
  ...
  if (form.value.date_debut) {
    ...
    // Vérifier uniquement les jours bloqués (pas weekends)
    const isBlocked = congesStore.calendrierEvents.some(e => {
      if (!e.isBlocked || e.type === 'weekend') return false // ← Ignorer weekends
