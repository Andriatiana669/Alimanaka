...
export const usePermissionsStore = defineStore('permissions', () => {
  ...
  const approuverPermission = async (id: number) => {
    loading.value = true
    error.value = null
    try {
      const result = await permissionsApi.approuverPermission(id)
      
      // Mettre à jour localement avec vérification de sécurité
      const index = permissions.value.findIndex(p => p.id === id)
      if (index !== -1 && permissions.value[index]) {  // ← Vérifie que l'index existe ET que l'objet existe
        const permission = permissions.value[index]!    // ← Le "!" assure TypeScript que c'est défini
        
        permission.statut = 'approuve'
        // Note: result.valide_par est une string (nom), pas un ID
        // Tu dois recharger les détails complets ou ajuster selon ta réponse API
        permission.date_validation = result.date_validation
        
        // Option 1: Recharger les détails complets pour avoir valide_par_details
        const updated = await permissionsApi.getPermissionDetails(id)
        permissions.value[index] = updated
      }
