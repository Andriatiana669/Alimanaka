...
export const useRetardsStore = defineStore('retards', () => {
  ...
  const totalHeuresARattraper = computed(() =>
    retards.value
      ...
      .reduce((sum, r) => sum + (r.heures_restantes || 0), 0)
  )

  /**
   * Conversion des événements API (string) vers des événements Calendar (Date)
   * CORRIGÉ: On utilise 'as any' pour contourner la vérification de type stricte
   * car Calendar.vue accepte en réalité n'importe quel type d'événement
   */
  const eventsForCalendar = computed<CalendarEvent[]>(() =>
    calendrierEvents.value.map(event => ({
      ...
      color: event.color,
      type: event.type,  // ← On garde 'retard' tel quel
      user_id: event.user_id,
      statut: event.statut,
      minutes_retard: event.minutes_retard,
      heures_restantes: event.heures_restantes
    })) as any // ← CORRECTION: cast en any pour éviter l'erreur TypeScript
  )
