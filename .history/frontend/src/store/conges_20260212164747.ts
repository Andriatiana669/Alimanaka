...
export const useCongesStore = defineStore('conges', () => {
  ...
  const eventsForCalendar = computed((): CalendarEvent[] => {
    return calendrierEvents.value.map(event => ({
      id: String(event.id), // Conversion en string
      title: event.title,
  ...
  const isDateBlocked = (date: Date): boolean => {
    ...
    const event = calendrierEvents.value.find(e => {
      const eventStartStr = new Date(e.start).toISOString().split('T')[0]
      const eventEndStr = e.end 
        ? new Date(e.end).toISOString().split('T')[0] 
        : eventStartStr
      
      return dateStr >= eventStartStr && dateStr <= eventEndStr && e.isBlocked
    })
    return !!event
  }
