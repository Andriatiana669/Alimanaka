...
export const useEquipesStore = defineStore('equipes', () => {
  ...
  const fetchEquipes = async (inclureInactives: boolean = true) => {
    loading.value = true
    error.value = null
    
    try {
      const response = await equipesApi.getAll(inclureInactives)
  ...
  // ← MODIFIER : Ajouter paramètre inclureInactives
  const fetchArbre = async (inclureInactives: boolean = true) => {
    loading.value = true
    error.value = null
    
    try {
      const response = await equipesApi.getArbre(inclureInactives)
  ...
  const changerManager = async (equipeId: number, managerId: number) => {
    ...
    try {
      ...
      await fetchEquipeById(equipeId)
      return response
    } catch (err: any) {
