...
export const useRetardsStore = defineStore('retards', () => {
  ...
  const totalHeuresARattraper = computed(() =>
    retards.value
      ...
      .reduce((sum, r) => sum + r.heures_restantes, 0)
  )

  /**
   * Conversion des événements API (string) vers des événements Calendar (Date)
   */
  const eventsForCalendar = computed<CalendarEvent[]>(() =>  // ← CHANGER ICI
    calendrierEvents.value.map(event => ({
      ...
      color: event.color,
      type: 'retard',  // ← Important: 'retard' est accepté car CalendarEvent attend string
