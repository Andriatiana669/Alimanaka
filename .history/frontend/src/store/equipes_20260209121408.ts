...
export const useEquipesStore = defineStore('equipes', () => {
  ...
  const fetchMembres = async (equipeId: number) => {
    // Vérifie si on charge déjà les mêmes membres
    if (currentEquipe.value?.id === equipeId && membresCurrentEquipe.value.length > 0) {
      console.log('Membres déjà chargés pour cette équipe')
      return membresCurrentEquipe.value
    }
    ...
    try {
      ...
      currentEquipe.value = { id: equipeId } as Equipe // Stocke seulement l'ID
      return response
    } catch (err: any) {
